...
@section('title', __('general_content.gmao_new_maintenance_plan_trans_key'))

@section('content_header')
    <h1>{{ __('general_content.gmao_new_maintenance_plan_trans_key') }}</h1>
@stop
    ...
    <form method="POST" action="{{ route('gmao.maintenance-plans.store') }}">
        ...
        <x-adminlte-card title="{{ __('general_content.gmao_maintenance_plan_trans_key') }}" theme="secondary" maximizable>
            ...
            <x-slot name="footerSlot">
                ...
                <a href="{{ route('gmao.maintenance-plans.index') }}" class="btn btn-secondary float-right">{{ __('general_content.back_trans_key') }}</a>
            </x-slot>
        </x-adminlte-card>
    </form>
@stop
