...
@if (config('adminlte.use_route_url', false))
    @php( $login_url = $login_url ? ltrim(route($login_url, [], false), '/') : '' )
    @php( $register_url = $register_url ? ltrim(route($register_url, [], false), '/') : '' )
    @php( $password_reset_url = $password_reset_url ? ltrim(route($password_reset_url, [], false), '/') : '' )
@else
    @php( $login_url = $login_url ? ltrim($login_url, '/') : '' )
    @php( $register_url = $register_url ? ltrim($register_url, '/') : '' )
    @php( $password_reset_url = $password_reset_url ? ltrim($password_reset_url, '/') : '' )
@endif

@section('auth_header', __('adminlte::adminlte.login_message'))

@section('auth_body')
    <form action="{{ ltrim(route('login.store', [], false), '/') }}" method="post">
        @csrf
        @if (session('message'))
