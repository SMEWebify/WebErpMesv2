<?php

return [

    /* GENERAL TRANS KEY */

    //STATUS
    'awaiting_payment_trans_key'               => 'Awaiting payment',
    'open_trans_key'                           => 'Open',
    'send_trans_key'                           => 'Send',
    'win_trans_key'                            => 'Win',
    'lost_trans_key'                           => 'Lost',
    'closed_trans_key'                         => 'Closed',
    'obsolete_trans_key'                       => 'Obsolete',
    'started_trans_key'                        => 'Started',
    'planned_trans_key'                        => 'Planned',
    'in_progress_trans_key'                    => 'In Progress',
    'finished_trans_key'                       => 'Finished',
    'suspended_trans_key'                      => 'Suspended',
    'outsourced_trans_key'                     => 'Outsourced',
    'rfq_in_progress_trans_key'                => 'RFQ in progress',
    'to_rfq_trans_key'                         => 'To RFQ',
    'finished_task_trans_key'                  => 'Finished (all the tasks are finished)',
    'delivered_trans_key'                      => 'Delivered',
    'partly_delivered_trans_key'               => 'Partly delivered',
    'partly_stored_trans_key'                  => 'Partially stored',
    'not_delivered_trans_key'                  => 'Not delivered',
    'delivered_without_dn_trans_key'           => 'Delivered without delevery note',
    'chargeable_trans_key'                     => 'Chargeable',
    'not_chargeable_trans_key'                 => 'Not chargeable',
    'partly_invoiced_trans_key'                => 'Partly invoiced',
    'invoiced_trans_key'                       => 'Invoiced',
    'not_invoiced_trans_key'                   => 'Not invoiced',
    'ordered_trans_key'                        => 'Ordered',
    'partly_received_trans_key'                => 'Partly received',
    'rceived_trans_key'                        => 'Received',
    'po_partly_created_trans_key'              => 'PO partly created',
    'po_created_trans_key'                     => 'PO Created',
    'canceled_trans_key'                       => 'Canceled',
    'order_canceled_no_document_trans_key'     => 'Order canceled: delivery note and invoice creation are disabled.',
    'stopped_trans_key'                        => 'Stopped',
    'no_task_trans_key'                        => 'No task',
    'created_trans_key'                        => 'Created',
    'to_be_posted_trans_key'                   => 'To be posted',
    'undefined_trans_key'                      => 'Undefined',
    'worker_trans_key'                         => 'worker',
    'employee_trans_key'                       => 'Employee',
    'self_employed_trans_key'                  => 'Self-employed',
    'male_trans_key'                           => 'Male',
    'female_trans_key'                         => 'Female',
    'other_trans_key'                          => 'Other',
    'online_trans_key'                         => 'Online',
    'offline_trans_key'                        => 'Offline',
    'active_trans_key'                         => 'Active',
    'inactive_trans_key'                       => 'Inactive',
    'full_day_absence_trans_key'               => 'Full day absence',
    '1_half_day_absence_trans_key'             => '1 st half day absence',
    '2_half_day_absence_trans_key'             => '2 nd half day absence',
    'absence_in_hours_trans_key'               => 'Absence in hours',
    'calendar_trans_key'                       => 'Calendar',
    'workable_day_trans_key'                   => 'Workable day',
    'worked_day_trans_key'                     => 'Worked day',
    'to_validate_trans_key'                    => 'To validate',
    'validate_trans_key'                       => 'Validate',
    'solved_trans_key'                         => 'Solved',
    'reopen_trans_key'                         => 'Reopen',
    'unvalidate_trans_key'                     => 'Unvalidate',
    'no_traceability_trans_key'                => 'No traceability',
    'with_batch_number_trans_key'              => 'With batch number',
    'with_serial_number_trans_key'             => 'With serial number',
    'sale_trans_key'                           => 'Sale',
    'purchase_stock_trans_key'                 => 'Purchase (stock)',
    'advance_payment_trans_key'                => 'Advance payment',
    'advance_payment_vat_trans_key'            => 'Advance payment (with VAT)',
    'productive_trans_key'                     => 'Productive',
    'raw_material_trans_key'                   => 'Raw material',
    'raw_material_sheet_trans_key'             => 'Raw material (Sheet)',
    'raw_material_profil_trans_key'            => 'Raw material (Profile)',
    'raw_material_block_trans_key'             => 'Raw material (block)',
    'supplies_trans_key'                       => 'Supplies',
    'supplied_trans_key'                       => 'Supplied',
    'sub_contracting_trans_key'                => 'Sub-contracting',
    'composed_component_trans_key'             => 'Composed component',
    'unsed_trans_key'                          => 'Unused',
    'used_trans_key'                           => 'Used',
    'mass_trans_key'                           => 'Mass',
    'length_trans_key'                         => 'Length',
    'aera_trans_key'                           => 'Area',
    'volume_trans_key'                         => 'Volume',
    'on_trial_trans_key'                       => 'On trial',
    'asset_trans_key'                          => 'Asset',
    'manual_stock_recep_trans_key'             => 'Manual Stock reception',
    'add_manual_receipt_line_trans_key'        => 'Add manual receipt line',
    'inventories_trans_key'                    => 'Inventories',
    'task_allocation_trans_key'                => 'Task allocation',
    'purchase_order_reception_trans_key'       => 'Purchase order reception',
    'manufactured_component_entry_trans_key'   => 'Manufactured component entry',
    'inter_stock_mvts_trans_key'               => 'Inter-stock mvts',
    'manual_stock_dispatching_trans_key'       => 'Manual stock dispatching',
    'reservation_cancellation_trans_key'       => 'Reservation cancellation',
    'reservation_component_production_trans_key' => 'Reservation of a component in production',
    'reservation_trans_key'                    => 'Reservation',
    'part_delivery_trans_key'                  => 'Part delivery',
    'in_production_trans_key'                  => 'In production',
    'direct_inventory_trans_key'               => 'Direct inventory',
    'new_trans_key'                            => 'New',
    'converted_trans_key'                      => 'Converted',
    'stop_trans_key'                           => 'Stop',
    'setup_trans_key'                          => 'Setup',
    'run_trans_key'                            => 'Run',
    'off_trans_key'                            => 'Off',
    'internal_trans_key'                       => 'Internal',
    'external_trans_key'                       => 'External',
    'waiting_customer_data_trans_key'          => 'Waiting Customer Data',
    'married_trans_key'                        => 'Married',
    'single_trans_key'                         => 'Single',
    'divorced_trans_key'                       => 'Divorced',
    'widowed_trans_key'                        => 'Widowed',
    'prospect_trans_key'                       => 'Prospect',
    'miss_trans_key'                           => 'Miss',
    'ms_trans_key'                             => 'Ms',
    'mr_trans_key'                             => 'Mr',
    'mrs_trans_key'                            => 'Mrs',
    'burning_trans_key'                        => 'Burning',
    'hot_trans_key'                            => 'Hot',
    'lukewarm_trans_key'                       => 'Lukewarm',
    'cold_trans_key'                           => 'Cold',
    'quote_made_trans_key'                     => 'Quote made',
    'negotiation_trans_key'                    => 'Negotiation',
    'closed_won_trans_key'                     => 'Closed-won',
    'closed_lost_trans_key'                    => 'Closed-lost',
    'informational_trans_key'                  => 'Informational', 
    'activity_maketing_trans_key'              => 'Activity marketing',
    'email_send_trans_key'                     => 'Email Send',
    'pre_sakes_aactivity_trans_key'            => 'Pre-sakes activity',
    'sales_activity_trans_key'                 => 'Sales activity',
    'sales_telephone_call_trans_key'           => 'Sales telephone call',
    'no_start_trans_key'                       => 'No start',
    'internal_meeting_trans_key'               => 'Internal Meeting',
    'onsite_visite_trans_key'                  => 'Onsite visit',
    'sales_meeting_trans_key'                  => 'Sales Meeting',
    'shipped_trans_key'                        => 'Shipped',
    'returned_trans_key'                       => 'Returned',
    'in_stock_trans_key'                       => 'In stock',
    'pending_trans_key'                        => 'Pending',
    'unpaid_trans_key'                         => 'Unpaid',
    'paid_trans_key'                           => 'Paid',
    'done_trans_key'                           => 'Done',
    'to_submit_trans_key'                      => 'To submit',
    'submitted_trans_key'                      => 'Submitted',
    'approved_trans_key'                       => 'Approved',
    'rejected_trans_key'                       => 'Rejected',
    'low_trans_key'                            => 'Low',
    'moderate_trans_key'                       => 'Moderate',
    'high_trans_key'                           => 'High',
    'rare_trans_key'                           => 'Rare',
    'possible_trans_key'                       => 'Possible',
    'probable_trans_key'                       => 'Probable',
    'minor_trans_key'                          => 'Minor',
    'major_trans_key'                          => 'Major',
    'critical_trans_key'                       => 'Critical',
    'expired_trans_key'                        => 'Expired',
    'reports_trans_key'                        => 'Reports',
    'accounting_reports_trans_key'             => 'Accounting reports',
    'total_revenue_trans_key'                  => 'Total revenue',
    'total_expense_trans_key'                  => 'Total expense',
    'total_profit_trans_key'                   => 'Profit',
    'total_product_time_by_service'            => 'Total product time by service',
    'total_setting_time_by_service'            => 'Total setting time by service',
    'total_cost_by_service'                    => 'Total cost by service',
    'total_price_by_service'                   => 'Total price by service',
    'coming_soon_trans_key'                    => 'Coming soon',
    'processing_location_trans_key'            => 'Processing location',
    'select_location_trans_key'                => 'Select location',

    //GENERAL HEADER AND FOOTER TABLE
    'sort_trans_key'                           => 'Sort',
    'id_trans_key'                             => 'ID',
    'code_trans_key'                           => 'Project Ref',
    'label_trans_key'                          => 'Label',
    'customer_trans_key'                       => 'Customer',
    'id_customer_trans_key'                    => 'Id customer',
    'status_trans_key'                         => 'Status',
    'statu_trans_key'                          => 'Statu',
    'total_price_trans_key'                    => 'Total price',
    'lines_count_trans_key'                    => 'Lines count',
    'created_at_trans_key'                     => 'Created at',
    'assigned_trans_key'                       => 'Assigned',
    'action_trans_key'                         => 'Actions',
    'save_trans_key'                           => 'Save',
    'cancel_trans_key'                         => 'Cancel',
    'ordre_trans_key'                          => 'Order',
    'saving_trans_key'                         => 'Saving...',
    'no_results_trans_key'                     => 'No results',
    'user_trans_key'                           => 'User',
    'attendance_trans_key'                     => 'Attendance',
    'attendance_select_user_trans_key'         => 'Select a user',
    'attendance_entry_trans_key'               => 'Entry',
    'attendance_exit_trans_key'                => 'Exit',
    'attendance_success_trans_key'             => 'Attendance recorded.',
    'success_trans_key'                        => 'Operation completed successfully.',
    'status_client_trans_key'                  => 'Status client',
    'status_supplier_trans_key'                => 'Status supplier',
    'color_trans_key'                          => 'Color',
    'picture_trans_key'                        => 'Logo',
    'ressource_trans_key'                      => 'Resource',
    'ressources_trans_key'                     => 'Resources',
    'section_trans_key'                        => 'Section',
    'sections_trans_key'                       => 'Sections',
    'hourly_rate_trans_key'                    => 'Hourly rate',
    'margin_trans_key'                         => 'Margin',
    'mask_time_trans_key'                      => 'Mask time',
    'capacity_trans_key'                       => 'Capacity',
    'hour_capacity_week_trans_key'             => 'Hour capacity by week',
    'etat_trans_key'                           => 'Etat',
    'cost_trans_key'                           => 'Unit Cost',
    'due_date_trans_key'                       => 'Due date',
    
    'order_trans_key'                          => 'Order',
    'external_id_trans_key'                    => 'External ID',
    'description_trans_key'                    => 'Description',
    'price_trans_key'                          => 'Unit Price',
    'customer_price_grid_trans_key'            => 'Customer price grid',
    'automatic_pricing_trans_key'              => 'Automatic pricing',
    'price_source_trans_key'                   => 'Price source',
    'use_price_trans_key'                      => 'Use price',
    'no_price_defined_trans_key'               => 'No customer price defined for this product.',
    'all_customers_trans_key'                  => 'All customers',
    'all_customer_types_trans_key'             => 'All customer types',
    'customer_price_list_create_help_trans_key' => 'Leave customer and type empty to define a general price.',
    'customer_price_list_unique_error_trans_key' => 'A price already exists for this scope and quantity range.',
    'type_trans_key'                           => 'Type',
    'qty_trans_key'                            => 'Qty',
    'unit_trans_key'                           => 'Unit',
    'discount_trans_key'                       => 'Discount',
    'vat_trans_key'                            => 'VAT',
    'delivery_date_trans_key'                  => 'Delivery date',
    'delivery_note_date_from_trans_key'        => 'Delivery note date from',
    'delivery_note_date_to_trans_key'          => 'Delivery note date to',
    'internal_delay_trans_key'                 => 'Internal delay',
    'tasks_status_trans_key'                   => 'Tasks status',
    'delivery_status_trans_key'                => 'Delivery status',
    'invoice_status_trans_key'                 => 'Invoice status',
    'mark_not_chargeable_trans_key'            => 'Mark as not chargeable',
    'confirm_not_chargeable_trans_key'         => 'Mark this line as not chargeable? It will no longer be due to invoice.',
    'line_already_invoiced_trans_key'          => 'This line is already invoiced.',
    'order_invoiced_no_cancel_trans_key'       => 'This order is invoiced and cannot be cancelled. Issue a credit note instead.',
    'delivered_qty_trans_key'                  => 'Delivered qty',
    'remaining_qty_trans_key'                  => 'Remaining qty',
    'addressing_trans_key'                     => 'Addressing',
    
    'supplier_ref_trans_key'                   => 'Supplier ref',
    'supplier_trans_key'                       => 'Supplier',
    'supplier_rate_trans_key'                  => 'Supplier rate',
    'order_rate_trans_key'                     => 'Order rate',
    'qty_reciept_trans_key'                    => 'Qty Receipt',
    'qty_purchase_trans_key'                   => 'Qty purchase',
    'qty_invoice_trans_key'                    => 'Qty Invoice',
    'qty_accepted_trans_key'                   => 'Qty accepted',
    'qty_canceled_trans_key'                   => 'Qty canceled',
    'qty_mini_trans_key'                       => 'Qty mini',
    'qty_total_trans_key'                      => 'Qty total',
    'scum_qty_trans_key'                       => 'Scum qty',
    
    'name_trans_key'                           => 'Name',
    'type_of_contract_trans_key'               => 'Type of contract',
    'weekly_duration_trans_key'                => 'Weekly duration',
    
    'employment_statu_trans_key'               => 'Employment status',
    'job_title_trans_key'                      => 'Job Title',
    'role_trans_key'                           => 'Role',  
    'roles_trans_key'                          => 'Roles',  
    'gender_trans_key'                         => 'Gender',  
    'born_date_trans_key'                      => 'Born date',  
    'type_of_day_trans_key'                    => 'Type of day',  
    'start_date_trans_key'                     => 'Start date',  
    'end_date_trans_key'                       => 'End date',
    'date_time_trans_key'                      => 'Date time',
    'hour_trans_key'                           => 'Hour',
    'hours_trans_key'                          => 'Hours',
    'task_trans_key'                           => 'Task',
    'nc_trans_key'                             => 'NC',
    'failure_trans_key'                        => 'Failure',
    'failure_comment_trans_key'                => 'Failure comment',
    'failure_type_trans_key'                   => 'Failure type',
    'correction_trans_key'                     => 'Correction',
    'correction_comment_trans_key'             => 'Correction comment',
    'correction_type_trans_key'                => 'Correction type',
    'serial_number_trans_key'                  => 'Serial number',
    'companie_concern_trans_key'               => 'Third party concern',
    'city_trans_key'                           => 'City',
    'postal_code_trans_key'                    => 'Postal code',
    'province_trans_key'                       => 'Province',
    'country_trans_key'                        => 'Country',
    'function_trans_key'                       => 'Function',
    'title_trans_key'                          => 'Title',
    'text_trans_key'                           => 'Text',
    
    'rate_trans_key'                           => 'Rate',  
    'code_account_trans_key'                   => 'Code account', 
    'number_of_month_trans_key'                => 'Number of month', 
    'number_of_day_trans_key'                  => 'Number of day', 
    'end_of_month_trans_key'                   => 'End of month', 
    'account_number_trans_key'                 => 'Account number', 
    'vat_account_trans_key'                    => 'VAT account number', 
    
    'total_selling_trans_key'                  => 'Total selling price',
    'total_time_trans_key'                     => 'Total time',
    'unit_time_trans_key'                      => 'Unit time',
    'setting_time_trans_key'                   => 'Setting time',
    'fixed_trans_key'                          => 'Fixed',
    
    'progress_trans_key'                       => 'Progress',
    'sub_total_trans_key'                      => 'Subtotal',
    'total_trans_key'                          => 'Total',
    'tax_trans_key'                            => 'Tax',
    'no_tax_trans_key'                         => 'No tax',
    
    'failure_mode_trans_key'                   => 'Failure Mode',
    'effect_trans_key'                         => 'Effect',
    'severity_trans_key'                       => 'Severity (1 to 10)',
    'occurrence_trans_key'                     => 'Occurrence (1 to 10)',
    'detection_trans_key'                      => 'Detection (1 to 10)',
    'current_control_trans_key'                => 'Current Control',
    'recommended_action_trans_key'             => 'Recommended Action',

    //GENERAL CONTENT TABLE
    'no_data_trans_key'                        => 'No data available in table',

    //TAB
    'opportunity_info_trans_key'               => 'Opportunity info',
    'activities_trans_key'                     => 'Activities',
    'events_trans_key'                         => 'Events',
    'timeline_trans_key'                       => 'TimeLine',

    'quote_info_trans_key'                     => 'Quote info',
    'quote_line_trans_key'                     => 'Quote lines',
    'construction_site_trans_key'              => 'Construction site/layout',
    'modules_trans_key'                        => 'Modules',
    'enable_trans_key'                         => 'Enable',
    'construction_site_enable_help_trans_key'  => 'Shows the Construction site/layout tab on quotes.',
    'guest_page_trans_key'                     => 'Guest page',
    'lines_import_trans_key'                   => 'Lines Import',

    'order_info_trans_key'                     => 'Order info',
    'order_line_trans_key'                     => 'Order lines',
    'quote_lines_related_trans_key'            => 'Quote lines',
    'order_lines_related_trans_key'            => 'Order lines',
    'business_Review_trans_key'                => 'Business Review',

    'delivery_info_trans_key'                  => 'Delivery info',
    'delivery_lines_trans_key'                 => 'Delivery lines',

    'invoice_info_trans_key'                   => 'Invoice info',
    'invoice_lines_trans_key'                  => 'Invoice lines',
    'invoice_draft_trans_key'                  => 'Draft',
    'invoice_draft_no_pdf_trans_key'           => 'A draft invoice cannot be exported as a PDF.',
    'invoice_draft_no_facturx_trans_key'       => 'A draft invoice cannot produce a Factur-X document: it has not been issued yet.',
    'invoice_proforma_no_facturx_trans_key'    => 'A proforma is not an invoice: it cannot produce a Factur-X document.',

    'purchase_quotation_trans_key'             => 'Purchase quotation',
    'purchase_quotation_info_trans_key'        => 'Purchase quotation info',
    'purchase_quotation_lines_trans_key'       => 'Purchase quotation lines',
    'lead_time_trans_key'                      => 'Lead time',
    'score_trans_key'                          => 'Score',

    'purchase_info_trans_key'                  => 'Purchase info',
    'purchase_lines_trans_key'                 => 'Purchase lines',

    'purchase_receipt_trans_key'               => 'Purchase receipt',
    'purchase_receipt_info_trans_key'          => 'Purchase receipt info',
    'purchase_receipt_lines_trans_key'         => 'Purchase receipt lines',
    
    'purchase_invoice_info_trans_key'          => 'Purchase Invoice info',
    'purchase_invoice_lines_trans_key'         => 'Purchase Invoice lines',

    'product_info_trans_key'                   => 'Product detail',
    'tech_bom_trans_key'                       => 'Technical cut + BOM',

    'location_in_workshop_trans_key'           => 'Location in workshop',
    'units_trans_key'                          => 'Units',
    'families_trans_key'                       => 'Families',
    'tools_trans_key'                          => 'Tools',
    'tool_already_booked_trans_key'            => 'The selected tool is already booked for this time slot.',
    'batches_trans_key'                        => 'Batches',

    'absence_trans_key'                        => 'Absence',
    'banck_holiday_trans_key'                  => 'Bank holiday',
    'improduct_time_trans_key'                 => 'Non-productive time',
    'machine_event_trans_key'                  => 'Machine event',

    'profil_setting_trans_key'                 => 'Profile setting',
    'notification_history_trans_key'           => 'Notification history',
    'leave_request_trans_key'                  => 'Leave request',

    'hr_information_trans_key'                 => 'HR information',
    'contract_trans_key'                       => 'Contract',

    'derogations_trans_key'                    => 'Derogations',
    'measuring_devices_trans_key'              => 'Measuring devices',
    'calibrated_at_trans_key'                  => 'Calibrated at',
    'calibration_due_at_trans_key'             => 'Calibration due at',
    'calibration_status_trans_key'             => 'Calibration status',
    'calibration_provider_trans_key'           => 'Calibration provider',
    'calibration_overdue_trans_key'            => 'Calibration overdue',
    'calibration_due_soon_trans_key'           => 'Calibration due soon',
    'calibration_due_soon_description_trans_key' => 'The following devices require calibration within :days days.',
    'capability_index_trans_key'               => 'Capability index',
    'not_available_trans_key'                  => 'N/A',

    'factory_trans_key'                        => 'Factory',
    'factory_settings_trans_key'               => 'Factory settings',
    'announcements_trans_key'                  => 'Announcements',
    'workflow_settings_trans_key'              => 'Workflow settings',
    'permissions_trans_key'                    => 'Permissions',
    'permission_trans_key'                     => 'Permission',
    'role_in_permissions_trans_key'            => 'Roles in permissions',
    'estimated_budget_trans_key'               => 'Estimated Budget',
    'export_fec_trans_key'                     => 'Export FEC',
    'customer_import_trans_key'                => 'Customer import',
    'quotes_import_trans_key'                  => 'Quotes import',
    'orders_import_trans_key'                  => 'Orders import',
    'products_import_trans_key'                => 'Product import',
    'kanban_setting_note_trans_key'            => 'The first line will be used for the default status of new tasks. The last line is used for finished task, ready for delivery note',
    'make_new_role_trans_key'                  => 'Make a new Role',
    'custom_fields_trans_key'                  => 'Custom fields',
    'custom_fields_category_trans_key'         => 'Category',
    'custom_fields_category_placeholder_trans_key' => 'e.g. Technical',
    'custom_fields_placeholder_trans_key'      => 'Enter a value...',
    'custom_fields_default_category_trans_key' => 'General',
    'custom_fields_type_select_trans_key'      => 'List',
    'custom_fields_options_trans_key'          => 'Options',
    'custom_fields_options_placeholder_trans_key' => 'One option per line',
    'custom_fields_options_help_trans_key'     => 'Displayed as selectable values in the list.',
    'custom_fields_select_placeholder_trans_key' => 'Select an option',
    'document_code_templates_trans_key'        => 'Document code templates',
    'new_document_code_template_trans_key'     => 'Nouveau templates',
    'template_trans_key'                       => 'Templates',
    
    // CARD HEADER
    'statistiques_trans_key'                   => 'Statistics',
    'informations_trans_key'                   => 'Information',
    'options_trans_key'                        => 'Options',
    'charts_trans_key'                         => 'Charts',

    // BUTTON
    'view_trans_key'                           => 'View',
    'show_trans_key'                           => 'Show',
    'search_trans_key'                         => 'Search',
    'view_count_trans_key'                     => 'View(s) count',
    'print_trans_key'                          => 'Print',
    'pdf_trans_key'                            => 'PDF',
    'edit_trans_key'                           => 'Edit',
    'delete_trans_key'                         => 'Delete',
    'view_all_trans_key'                       => 'View All',
    'submit_trans_key'                         => 'Submit',
    'update_trans_key'                         => 'Update',
    'success_update_trans_key'                 => 'Record updated successfully.',
    'upload_trans_key'                         => 'Upload',
    'refresh_trans_key'                        => 'Refresh Page',
    'close_trans_key'                          => 'Close',
    'yes_trans_key'                            => 'YES',
    'no_trans_key'                             => 'NO',
    'add_trans_key'                            => 'Add',
    'new_receipt_document_trans_key'           => 'New receipt document',
    'new_empty_receipt_document_trans_key'     => 'New empty receipt',
    'new_invoice_document_trans_key'           => 'New invoice document',
    'new_purchase_document_trans_key'          => 'New purchase document',
    'export_csv_trans_key'                     => 'Export CSV',
    'sheet_metal_global_need_csv_trans_key'    => 'CSV for global sheet metal needs estimate',
    'sheet_metal_global_need_csv_hint_trans_key' => 'Export a CSV to estimate the global sheet metal needs.',
    'open_orders_not_started_trans_key'        => 'Open orders / not started tasks',
    'open_orders_not_started_hint_trans_key'   => 'Export lines from open orders with not started tasks.',
    
    //SELECT
    'select_trans_key'                         => 'Select',
    'select_vat_trans_key'                     => 'Select VAT',
    'select_product_trans_key'                 => 'Select Product',
    'user_management_trans_key'                => 'User management',
    'select_user_trans_key'                    => 'Select user',
    'select_user_management_trans_key'         => 'Select user management',
    'select_company_trans_key'                 => 'Select third party',
    'select_suppliers_trans_key'               => 'Select suppliers',
    'select_address_trans_key'                 => 'Select address',
    'select_contact_trans_key'                 => 'Select contact',
    'select_payement_condition_trans_key'      => 'Select payement condition',
    'select_payement_methods_trans_key'        => 'Select payment method',
    'select_delivery_trans_key'                => 'Select delivery',
    'select_service_trans_key'                 => 'Select service',
    'select_tool_trans_key'                    => 'Select tool',
    'select_ressource_trans_key'               => 'Select ressource',
    'select_family_trans_key'                  => 'Select family',
    'select_unit_trans_key'                    => 'Select unit',
    'select_statu_trans_key'                   => 'Select status',
    'select_type_trans_key'                    => 'Select type',
    'select_control_type_trans_key'            => 'Select control type',
    'select_task_type_trans_key'               => 'Select your task Type',
    'select_component_trans_key'               => 'Select Component',
    'select_year_trans_key'                    => 'Select Year',
    'select_gender_trans_key'                  => 'Select gender',
    'select_marital_status_trans_key'          => 'Select marital status',
    'select_status_trans_key'                  => 'Select status',
    'select_controle_trans_key'                => 'Select controle type',
    'select_document_trans_key'                => 'Select your document',
    'select_task_trans_key'                    => 'Select task',
    'select_order_trans_key'                   => 'Select order',
    'select_order_line_trans_key'              => 'Select order line',
    'select_purchase_order_trans_key'          => 'Select purchase order',
    'select_categorie_trans_key'               => 'Select categorie',
    'reception_control_trans_key'              => 'Reception control',
    
    'no_select_company_trans_key'              => 'No company, please add',
    'no_address_trans_key'                     => 'No address, please add',
    'no_contact_trans_key'                     => 'No contact, please add',
    'no_payment_conditions_trans_key'          => 'No payment conditions, please add in accounting page',
    'no_payment_methods_trans_key'             => 'No payment methods, please add in accounting page',
    'no_delivery_trans_key'                    => 'No delivery type, please add in accounting page',
    'no_service_trans_key'                     => 'No service, please add in methods page',
    'no_family_trans_key'                      => 'No family, please add in methods page',
    'no_unit_trans_key'                        => 'No unit, please add in methods page',
    'no_vat_trans_key'                         => 'Not VAT, please add VAT before',
    'no_section_trans_key'                     => 'No section, please add section before',
    'no_role_trans_key'                        => 'No role, please add before',
    'no_permissions_trans_key'                 => 'No permissions, please add before',
    'no_stock_location_trans_key'              => 'No stock location, please add before',

    //REVIEW & CHANGE
    'review_change_tracking_trans_key'         => 'Review & change tracking',
    'review_timeline_trans_key'                => 'Review timeline',
    'reviewed_by_trans_key'                    => 'Reviewed by',
    'review_date_trans_key'                    => 'Review date',
    'decision_trans_key'                       => 'Decision',
    'change_requested_by_trans_key'            => 'Change requested by',
    'change_reason_trans_key'                  => 'Change reason',
    'change_approved_at_trans_key'             => 'Change approved at',
    'changes_trans_key'                        => 'Changes',

    //LINK
    'back_trans_key'                           => 'Back',
    'back_to_list_trans_key'                   => 'Back to list',
    'previous_trans_key'                       => 'Previous',
    'next_trans_key'                           => 'Next',
    'copie_line_trans_key'                     => 'Copy line',
    'edit_line_trans_key'                      => 'Edit line',
    'delete_line_trans_key'                    => 'Delete line',
    'break_down_task_trans_key'                => 'Break down the article task',
    'create_product_trans_key'                 => 'Create item from this line',
    'create_products_from_selected_lines_trans_key'    => 'Create items from selected lines',
    'create_products_from_selected_lines_success_trans_key' => 'Items created for selected lines',

    //LABEL
    'customer_info_trans_key'                  => 'Customer information',
    'supplier_info_trans_key'                  => 'Supplier information',
    'date_pay_info_trans_key'                  => 'Date & Payment information',
    'documents_trans_key'                      => 'Documents',
    'file_size_trans_key'                      => 'File size',
    'uploaded_at_trans_key'                    => 'Uploaded on',
    'updated_at_trans_key'                     => 'Updated on',
    'uploaded_by_trans_key'                    => 'Uploaded by',
    'related_entities_trans_key'               => 'Linked records',
    'document_search_placeholder_trans_key'    => 'Search by name, original file or comment',
    'hashtag_filter_placeholder_trans_key'     => 'Filter by #tag',
    'reset_filters_trans_key'                  => 'Reset filters',
    'no_documents_found_trans_key'             => 'No documents match your filters',
    'cgv_trans_key'                            => 'General Conditions of Sale',
    'photos_trans_key'                         => 'Photos',
    'choose_file_trans_key'                    => 'Choose file',
    'picture_file_trans_key'                   => 'Picture file',
    'drawing_trans_key'                        => 'Drawing file',
    'stl_file_trans_key'                       => 'STL file',
    'svg_file_trans_key'                       => 'SVG file',
    'viewer_file_trans_key'                    => 'Viewer',
    'attached_file_trans_key'                  => 'Attached files',
    'take_photo_trans_key'                     => 'Take a picture',
    'public_link_trans_key'                    => 'Public secure link',
    'copy_trans_key'                           => 'Copy',
    'add_to_document_trans_key'                => 'Add to new document',
    'preferred_supplier_trans_key'             => 'Preferreds suppliers',
    
    'customer_type_trans_key'                  => 'Customer type',
    'individual_trans_key'                     => 'Individual',
    'legal_entity_trans_key'                   => 'Legal entity',
    'companie_name_trans_key'                  => 'Third party name',
    'adress_name_trans_key'                    => 'Address name',
    'adress_trans_key'                         => 'Address',
    'contact_name_trans_key'                   => 'Contact name',
    'contact_trans_key'                        => 'Contact',
    'contacts_trans_key'                       => 'Contacts',
    'first_name_trans_key'                     => 'First name',
    'civility_trans_key'                       => 'Civility',
    'email_trans_key'                          => 'E-mail',
    'date_trans_key'                           => 'Date',
    'message_trans_key'                        => 'Message',
    'object_trans_key'                         => 'Object',
    'comment_trans_key'                        => 'Comment',
    'hashtags_trans_key'                       => 'Hashtags',
    'internal_comment_trans_key'               => 'Internal comment',
    'external_comment_trans_key'               => 'External comment',
    'identifier_trans_key'                     => 'Identifier',
    'payment_method_trans_key'                 => 'Payment method',
    'payment_methods_trans_key'                => 'Payment Methods',
    'payment_conditions_trans_key'             => 'Payment Conditions',
    'delevery_method_trans_key'                => 'Delivery method',
    'validity_date_trans_key'                  => 'Validity date',
    'longitude_trans_key'                      => 'Longitude',
    'latitude_trans_key'                       => 'Latitude',
    'delivery_constraint_trans_key'            => 'Delivery constraint',
    'no_constraints_trans_key'                 => 'No constraint',
    'no_tolerance_trans_key'                   => 'No tolerance',
    'tolerance_in_days_trans_key'              => 'Tolerance in days',
    'tolerance_days_trans_key'                 => 'Tolerance days',
    'quoted_delivery_note_trans_key'           => 'Quoted delivery note',
    'order_average_note_trans_key'             => 'Average order',
    'customer_processing_cost_trans_key'       => 'Customer processing cost',

    'material_trans_key'                       => 'Material',
    'finishing_trans_key'                      => 'Finishing',
    'thickness_trans_key'                      => 'Thickness',
    'weight_trans_key'                         => 'Net weight',
    'gross_weight_trans_key'                   => 'Gross weight',
    'total_weight_trans_key'                   => 'Total weight',
    'bend_count_trans_key'                     => 'Bend count',
    'x_size_trans_key'                         => 'X size',
    'y_size_trans_key'                         => 'Y size',
    'z_size_trans_key'                         => 'Z size',
    'x_oversize_trans_key'                     => 'X oversize',
    'y_oversize_trans_key'                     => 'Y oversize',
    'z_oversize_trans_key'                     => 'Z oversize',
    'diameter_trans_key'                       => 'Diameter',
    'diameter_oversize_trans_key'              => 'Diameter oversize',
    'material_loss_rate_trans_key'             => 'Material loss rate',
    'section_size_trans_key'                   => 'Section size',

    'delivery_note_number_trans_key'           => 'Delivery note number',
    'delivery_simulation_trans_key'            => 'Delivery lead-time simulation',
    'reference_trans_key'                      => 'Reference',
    'customer_reference_trans_key'             => 'Customer reference',
    
    'absence_type_day_trans_key'               => 'Absence type day',
    'absence_type_trans_key'                   => 'Absence type',
    'year_trans_key'                           => 'Year',
    'days_trans_key'                           => 'Days',
    'amount_trans_key'                         => 'Amount',
    
    'by_default_trans_key'                     => 'By default', 
    'document_type_trans_key'                  => 'Document type',
    'add_export_trans_key'                     => 'Add to export',
    'show_generic_task_trans_key'              => 'Show generic task',
    
    'auxiliary_account_trans_key'              => 'Auxiliary Account',
    'general_account_trans_key'                => 'General Account',
    'client_information_trans_key'             => 'Client information',
    'administrative_information_trans_key'     => 'Administrative information',
    'site_link_trans_key'                      => 'Site link',
    'web_link_trans_key'                       => 'Web site',
    'reg_number_trans_key'                     => 'Reg Number',
    'naf_code_trans_key'                       => 'Naf code',
    'vat_number_trans_key'                     => 'VAT number',
    'siren_trans_key'                          => 'Siren',
    'nat_regis_number_trans_key'               => 'Nat regis number',
    'share_capital_trans_key'                  => 'Share capital',
    'iban_trans_key'                           => 'IBAN',
    'bic_trans_key'                            => 'BIC / SWIFT',
    'custom_trans_key'                         => 'Custom',
    'custom_section_trans_key'                 => 'Custom section',
    'personnal_mail_trans_key'                 => 'Personal E-mail',
    'personnal_phone_trans_key'                => 'Personal phone number',
    'phone_trans_key'                          => 'Phone number',
    'mobile_phone_trans_key'                   => 'Mobile phone number',
    'nationality_trans_key'                    => 'Nationality',
    'user_location_trans_key'                  => 'Location',
    'termination_date_trans_key'               => 'Termination date',
    'supervisor_trans_key'                     => 'Supervisor',
    'section_concern_trans_key'                => 'Section concern',
    'signature_date_trans_key'                 => 'Signature date',
    'duration_trial_trans_key'                 => 'Duration trial period',
    'position_trans_key'                       => 'Position',
    'coefficient_trans_key'                    => 'Coefficient',
    'hourly_gross_salary_trans_key'            => 'Hourly gross salary',
    'minimum_monthly_salary_trans_key'         => 'Minimum monthly salary',
    'annual_gross_salary_trans_key'            => 'Annual gross salary',
    'driving_license_trans_key'                => 'Driving license',
    'driving_license_exp_date_trans_key'       => 'Driving license exp date',
    'ssn_num_trans_key'                        => 'SSN num',
    'nic_num_trans_key'                        => 'NIC num', 
    'move_Type_trans_key'                      => 'Move Type',
    'source_trans_key'                         => 'Source',
    'priority_trans_key'                       => 'Priority',
    'campaign_trans_key'                       => 'Campaign',
    'part_trans_key'                           => 'part(s)',
    'curency_trans_key'                        => 'Currency',
    'repots_setting_trans_key'                 => 'Reports setting',
    'visa_trans_key'                           => 'Visa',
    'select_to_export_trans_key'               => 'Select to export',
    'line_trans_key'                           => 'Line',
    'blocked_unti_trans_key'                   => 'Blocked until',
    
    'problem_description_trans_key'            => 'Problem description',
    'proposal_description_trans_key'           => 'Proposal description',
    'customer_reply_description_trans_key'     => 'Customer reply description',
    'decision_description_trans_key'           => 'Decision description',
    'cause_description_trans_key'              => 'Cause description',
    'action_description_trans_key'             => 'Action description',
    
    'collapse_trans_key'                       => 'Collapse',
    'remove_trans_key'                         => 'Remove',

    'probality_trans_key'                      => 'Probability',
    'budget_trans_key'                         => 'Budget',

    'loaded_date_trans_key'                    => 'Date de chargement',
    'packing_date_trans_key'                   => 'Date d\'emballage',

    //NOTIFICATION
    'unread_list_trans_key'                    => ' Unread List',
    'read_trans_key'                           => ' Read',
    'all_read_trans_key'                       => ' All Read',
    'notif_tab_unread_trans_key'               => 'Unread',
    'notif_tab_history_trans_key'              => 'History',
    'notif_read_success_trans_key'             => 'Marked as read',
    'notif_all_read_success_trans_key'         => 'All notifications marked as read',
    'notif_load_more_trans_key'                => 'Load more',
    'notif_loading_trans_key'                  => 'Loading…',
    'notif_history_empty_trans_key'            => 'No notifications',
    'notif_badge_read_trans_key'               => 'Read',
    'notif_badge_unread_trans_key'             => 'New',
    'notification_type_trans_key'              => 'Type',

    /* SPECIFIC TRANS KEY */

    //DASHBOARD
    'dashboard_trans_key'                      => 'Dashboard',
    'client_trans_key'                         => 'Clients',
    'new_client_trans_key'                     => 'New names',
    'since_time_trans_key'                     => 'Since :time',
    'suppliers_trans_key'                      => 'Suppliers',
    'supplier_evaluations_trans_key'           => 'Supplier evaluations',
    'new_supplier_evaluation_trans_key'        => 'New supplier evaluation',
    'latest_supplier_evaluation_trans_key'     => 'Latest supplier evaluation',
    'supplier_evaluation_history_trans_key'    => 'Supplier evaluation history',
    'no_purchase_order_to_evaluate_trans_key'  => 'No purchase orders available to evaluate this supplier yet.',
    'no_supplier_evaluation_trans_key'         => 'No evaluation has been recorded for this supplier yet.',
    'no_supplier_evaluation_history_trans_key' => 'No supplier evaluation history available yet.',
    'evaluation_quality_score_trans_key'       => 'Quality score',
    'evaluation_logistics_score_trans_key'     => 'Logistics score',
    'evaluation_service_score_trans_key'       => 'Service score',
    'evaluation_composite_score_trans_key'     => 'Composite score',
    'evaluation_average_composite_score_trans_key' => 'Average composite score',
    'evaluation_average_rating_trans_key'      => 'Average rating',
    'evaluation_rating_trans_key'              => 'Rating',
    'evaluation_status_trans_key'              => 'Status',
    'evaluation_last_updated_trans_key'        => 'Last updated',
    'evaluation_next_review_at_trans_key'      => 'Next review at',
    'evaluation_approved_at_trans_key'         => 'Approved at',
    'evaluation_action_plan_trans_key'         => 'Action plan',
    'evaluation_status_pending_trans_key'      => 'Pending',
    'evaluation_status_approved_trans_key'     => 'Approved',
    'evaluation_status_under_review_trans_key' => 'Under review',
    'evaluation_status_rejected_trans_key'     => 'Rejected',
    'supplier_requalification_alert_trans_key' => 'Supplier requalification alert',
    'supplier_requalification_soon_trans_key'  => 'Upcoming supplier requalification',
    'supplier_requalification_overdue_trans_key' => 'The supplier must be requalified since :days day(s).',
    'supplier_next_review_days_trans_key'      => 'Next review scheduled in :days day(s).',
    'suppliers_client_trans_key'               => 'Clients / Suppliers',
    'view_details_trans_key'                   => 'View details',
    'monthly_recap_report_trans_key'           => 'Monthly Recap Report',
    'monthly_recap_task_trans_key'             => 'Monthly Recap Task',
    'chart_months_trans_key'                   => ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'],
    'chart_order_forecast_trans_key'           => 'Order forecast',
    'chart_delivered_revenues_trans_key'       => 'Delivered revenues',
    'chart_invoiced_revenues_trans_key'        => 'Invoiced revenues',
    'chart_purchase_revenues_trans_key'        => 'Purchase revenues',
    'chart_order_targets_trans_key'            => 'Order targets',
    'sales_period_trans_key'                   => 'Sales: 1 Jan, :YEAR - 30 Dec, :YEAR',
    'incoming_orders_trans_key'                => 'Incoming orders',
    'no_coming_orders_trans_key'               => 'No coming orders.',
    'late_orders_trans_key'                    => 'Late orders',
    'no_late_orders_trans_key'                 => 'No late orders.',
    'goal_task_trans_key'                      => 'Goal Task Completion',
    'no_dash_task_trans_key'                   => 'No service go to Methods for add.',
    'total_order_forcasted_trans_key'          => 'TOTAL ORDER FORCASTED',
    'total_order_delivered_trans_key'          => 'TOTAL ORDER DELIVERED',
    'total_invoiced_trans_key'                 => 'TOTAL INVOICED',
    'goal_trans_key'                           => 'GOAL COMPLETIONS',
    'quote_transformation_trans_key'           => 'Quote transformation rate',
    'latest_quotes_trans_key'                  => 'Latest Quotes',
    'no_quote_trans_key'                       => 'No quote, go quote page for add quote',
    'latest_orders_trans_key'                  => 'Latest Orders',
    'no_order_trans_key'                       => 'No order, go order page for add order',
    'latest_products_trans_key'                => 'Recently Added Products',
    'no_product_trans_key'                     => 'No product, go product page for add item',
    'announcement_trans_key'                   => 'Announcements',
    'order_to_be_delivered_trans_key'          => 'Order to be delivered',
    'order_delivered_trans_key'                => 'Order delivered',
    'order_to_be_invoiced_trans_key'           => 'Order to be invoiced',
    'order_invoiced_trans_key'                 => 'Order invoiced',
    'order_waiting_trans_key'                  => 'Wainting orders',
    'delivered_month_in_progress_trans_key'    => 'Total Delivered for the month',
    'remaining_month_trans_key'                => 'Total remaining to deliver',
    'remaining_invoice_month_trans_key'        => 'Total remaining to invoice',
    'pending_orders_trans_key'                => 'Orders in progress',
    'forecast_next_three_months_trans_key'     => 'Forecast for the next 3 months',
    'service_rate_trans_key'                   => 'Service rate',
    'average_order_processing_time_trans_key'  => 'Average order processing time',
    'order_processing_time_trans_key'          => 'Processing time',
    'niko_niko_team_trans_key'                 => 'Niko Niko - Team Moods Today',
    'no_niko_niko_team_trans_key'              => 'No team mood today',
    'mood_tracker_trans_key'                   => 'Niko Niko',
    'my_mood_trans_key'                        => 'My mood today',
    'team_mood_trans_key'                      => 'Team',
    'no_mood_yet_trans_key'                    => 'No votes today',
    'mood_happy_trans_key'                     => 'Good',
    'mood_neutral_trans_key'                   => 'OK',
    'mood_sad_trans_key'                       => 'Hard',
    'average_quote_amount'                     => 'Average Quote Amount',
    'quote_conversion_rate'                    => 'Quote Conversion Rate',
    'quote_response_rate'                       => 'Quote Response Rate',
    
    //COMPANIES
    'companies_trans_key'                      => 'Third party',
    'companie_trans_key'                       => 'Third party',
    'sort_companie_trans_key'                  => 'Sort third party',
    'companies_list_trans_key'                 => 'Third party list',
    'latest_companies_trans_key'               => 'Latest third party ',
    'new_companie_trans_key'                   => 'New third party',
    'name_company_trans_key'                   => 'Name of third party',
    'detail_trans_key'                         => 'Detail',
    'general_information_trans_key'            => 'General information',
    'new_address_trans_key'                    => 'New address',
    'new_contact_trans_key'                    => 'New contact',

    //LEADS
    'leads_trans_key'                          => 'Leads',
    'lead_trans_key'                           => 'Lead',
    'new_leads_trans_key'                      => 'New Lead',
    'lead_count_trans_key'                     => 'Leads count',
    'list_leads_trans_key'                     => 'Lead list',
    
    //OPPORTUNITIES
    'opportunities_trans_key'                  => 'Opportunities',
    'new_opportunities_trans_key'              => 'New opportunity',
    'name_opportunity_trans_key'               => 'Name of opportunity',
    'opportunity_trans_key'                    => 'Opportunity',
    'new_activity_trans_key'                   => 'New activity',
    'new_event_trans_key'                      => 'New event',
    'opportunities_list_trans_key'             => 'Opportunities list',
    'opportunities_count_trans_key'            => 'Opportunities count',
    'total_amount_won_trans_key'               => 'Total amount won',
    'total_amount_lost_trans_key'              => 'Total amount lost',
    'opportunities_by_company_trans_key'       => 'Opportunities by company',
    'opportunities_by_probability_trans_key'   => 'Opportunities by Probability and Amount',

    //QUOTES
    'quote_trans_key'                          => 'Quote',
    'quotes_list_trans_key'                    => 'Quotes list',
    'quotes_lines_list_trans_key'              => 'Quotes lines list',
    'new_quote_trans_key'                      => 'New Quote',
    'select_all_lines_trans_key'               => 'Select all',
    'deselect_all_lines_trans_key'             => 'Deselect all',

    'price_increase_amount_trans_key'          => 'Amount to add',
    'apply_price_increase_to_lines_trans_key'  => 'Add amount to lines',
    'price_increase_applied_trans_key'         => 'Price increase applied to :count line(s).',
    'header_line_ask_trans_key'                => 'Header line ?',
    'quote_not_open_trans_key'                 => 'Quote curently not open',
    'name_quote_trans_key'                     => 'Name of quote',
    'update_detail_information_for_trans_key'  => 'Update detail information for :label',
    'task_detail_for_trans_key'                => 'Task detail for :label',
    'main_features_trans_key'                  => 'Main features',
    'dimensions_xyz_trans_key'                 => 'Dimensions (X, Y, Z)',
    'circular_specs_trans_key'                 => 'Circular specifications',
    'files_trans_key'                          => 'Files',
    'cad_file_trans_key'                       => 'CAD file',
    'cad_file_name_trans_key'                  => 'CAD file name',
    'cam_file_trans_key'                       => 'CAM file',
    'cam_file_name_trans_key'                  => 'CAM file name',
    'cad_file_path_trans_key'                  => 'CAD file path',
    'cam_file_path_trans_key'                  => 'CAM file path',
    'custom_requirements_trans_key'            => 'Custom requirements',
    'no_custom_requirement_trans_key'          => 'No custom requirement added yet.',
    'add_requirement_trans_key'                => 'Add requirement',
    'value_trans_key'                          => 'Value',
    'set_csv_col_trans_key'                    => 'set CSV col number',
    'choose_csv_trans_key'                     => 'Choose a .csv file...',
    'csv_quote_info_trans_key'                 => 'The unit value are defined in the methods section and the value of the default VAT are defined in the accounting section. If there is no discount column, the default value will be 0 %.', 
    'client_requirement_trans_key'             => 'Client Requirements',
    'show_client_requirements_on_pdf_trans_key'=> 'Show Client Requirements on PDF',
    'layout_plan_trans_key'                    => 'Layout Plan',
    'show_layout_on_pdf_trans_key'             => 'Show Layout plan on PDF',
    'materials_finishes_trans_key'             => 'Materials and Finishes',
    'show_materials_on_pdf_trans_key'          => 'Show Materials and Finishes on PDF',
    'logistics_trans_key'                      => 'Logistics',
    'logistics_cost_trans_key'                 => 'Logistics cost',
    'show_logistics_on_pdf_trans_key'          => 'Show Logistics on PDF',
    'coordination_with_contractors_trans_key'  => 'Coordination with contractors',
    'contractors_cost_trans_key'               => 'Contractors cost',
    'show_contractors_on_pdf_trans_key'        => 'Show contractors coordination on PDF',
    'waste_management_trans_key'               => 'Waste management',
    'waste_management_cost_trans_key'          => 'Waste management cost',
    'show_waste_on_pdf_trans_key'              => 'Show waste management on PDF',
    'taxes_and_fees_trans_key'                 => 'Additional taxes and fees',
    'taxes_cost_trans_key'                     => 'Taxes cost',
    'show_taxes_on_pdf_trans_key'              => 'Show taxes on PDF',
    'work_start_date_trans_key'                => 'Work start date',
    'work_end_date_trans_key'                  => 'Work end date',
    'contingency_days_trans_key'               => 'Contingency (days)',

    //ORDERS
    'orders_trans_key'                         => 'Orders',
    'orders_list_trans_key'                    => 'Orders list',
    'orders_lines_list_trans_key'              => 'Orders lines list',
    'order_confirm_trans_key'                  => 'Order Confirm',
    // Order acknowledgement
    'order_confirmations_trans_key'            => 'Order confirmations',
    'revision_trans_key'                       => 'Revision',
    'lines_trans_key'                          => 'Lines',
    'delivery_method_trans_key'                => 'Delivery method',
    'tracking_trans_key'                       => 'Tracking',
    'arc_status_in_progress_trans_key'         => 'In progress',
    'arc_status_sent_trans_key'                => 'Sent',
    'arc_status_accepted_trans_key'            => 'Accepted',
    'arc_status_superseded_trans_key'          => 'Superseded',
    'arc_current_trans_key'                    => 'Current',
    'arc_sent_trans_key'                       => 'Order confirmation sent.',
    'arc_issue_trans_key'                      => 'Issue confirmation',
    'arc_created_trans_key'                    => 'Order confirmation built from the order.',
    'arc_draft_no_pdf_trans_key'               => 'An in-progress confirmation cannot be printed.',
    'arc_differences_trans_key'                => 'Differences',
    'arc_revisions_trans_key'                  => 'Revisions',
    'arc_change_trans_key'                     => 'Change',
    'arc_no_difference_trans_key'              => 'The order matches this confirmation.',
    'arc_diff_only_current_trans_key'          => 'Differences are only computed on the current revision.',
    'arc_diverged_warning_trans_key'           => 'The order changed since confirmation revision :revision. A new revision should be issued.',
    'arc_line_added_trans_key'                 => 'Added',
    'arc_line_removed_trans_key'               => 'Removed',
    'arc_line_modified_trans_key'              => 'Modified',
    'internal_order_trans_key'                 => 'Internal order',
    'new_order_trans_key'                      => 'New order',
    'pre_orders_trans_key'                    => 'Pre-orders',
    'new_return_trans_key'                     => 'New return',
    'new_pre_order_trans_key'                  => 'New pre-order',
    'name_order_trans_key'                     => 'Name of order',
    'historical_trans_key'                     => 'Historical',
    'order_create_from_trans_key'              => 'Order Create from',
    'mnaufacturing_instruction_trans_key'      => 'Manufacturing instruction',
    'order_type_trans_key'                     => 'Order type',
    'customer_type_order_trans_key'            => 'Customer sales order',
    'internal_type_order_trans_key'            => 'Internal sales order',
    'forecast_margin_trans_key'                => 'Forecast margin',
    'current_margin_trans_key'                 => 'Current margin',
    'manufacturing_range_trans_key'            => 'Manufacturing range',
    'accomplished_trans_key'                   => 'Accomplished',
    'gap_trans_key'                            => 'Gap',
    'orders_to_deliver_today_trans_key'        => 'Orders to be delivered today',
    'delivery_scheduled_today_trans_key'       => 'Delivery scheduled for today',
    'no_orders_scheduled_today_trans_key'      => 'No orders scheduled for today.',
    'ready_orders_trans_key'                   => 'Ready orders',
    'no_ready_orders_trans_key'                => 'No ready orders.',
    'days_late_trans_key'                      => ':days days late',
    'to_deliver_today_trans_key'               => 'To be delivered today',
    'delivery_in_days_trans_key'               => 'Delivery in :days days',
    'requested_delivery_date_trans_key'        => 'Requested delivery date',
    'run_simulation_trans_key'                 => 'Run simulation',
    'simulation_possible_trans_key'            => 'Feasible with current load',
    'simulation_not_possible_trans_key'        => 'Not feasible with current load',
    'earliest_possible_date_trans_key'         => 'Earliest possible date',
    'simulation_missing_capacity_trans_key'    => 'Missing capacity (hours)',
    'simulation_no_tasks_trans_key'            => 'No quote tasks available to run the simulation.',
    'simulation_invalid_date_trans_key'        => 'The requested date must be today or later.',
    'simulation_results_trans_key'             => 'Simulation result',
    'simulation_required_hours_trans_key'      => 'Theoretical time (hours)',
    'simulation_service_trans_key'             => 'Service',
    'simulation_remaining_hours_trans_key'     => 'Missing hours',
    'simulation_capacity_per_day_trans_key'    => 'Daily capacity used',
    'simulation_internal_delay_trans_key'      => 'Internal lead time',
    'simulation_internal_deadline_trans_key'   => 'Internal deadline (production end)',
    
    'info_statu_trans_key'                     => 'The document status does not allow adding / modifying / deleting lines.',
    
    'scheduling_trans_key'                     => 'Scheduling',
    'order_calendar_trans_key'                 => 'Order calendar',

    //LOAD PLANNING
    'load_planning_trans_key'                  => 'Load planning',
    'load_planning_info_1_trans_key'           => 'Currently the charge per day is stuck at 16 hours',
    'display_hours_diff_trans_key'             => 'Time difference',

    //NESTING
    'nesting_trans_key'                        => 'Nesting',
    
    //WORKFLOW
    'workflow_trans_key'                       => 'Workflow',
    'workflow_info_1_trans_key'                => 'The views are configured in the \'Your company\' page.',

    //GANTT
    'gantt_trans_key'                          => 'Gantt',
    'gantt_info_1_trans_key'                   => 'Number of Task to be recalculate for date',
    'gantt_info_2_trans_key'                   => 'Number of Task to be recalculate for resource',

    //TASK
    'tasks_trans_key'                          => 'Tasks',
    'tasks_list_trans_key'                     => 'Tasks list',
    'task_type_trans_key'                      => 'Task type',
    'tasks_statu_trans_key'                    => 'Task management',
    'tasks_calendar_trans_key'                 => 'Tasks calendar',
    'add_generic_task_trans_key'               => 'Add generic task',
    'tasks_info_1_trans_key'                   => 'You can add a generic task that is not linked to an order. These are not displayed in the workflow.',
    'no_call_task_trans_key'                   => 'No task call',
    'quote_task_trans_key'                     => 'This is a quote task, no action possible.',
    'task_detail_trans_key'                    => 'Task  Detail',
    'logs_activity_trans_key'                  => 'Logs Activity',
    'no_activity_trans_key'                    => 'No activities.',
    'unknown_activity_trans_key'               => 'Unknown activity',
    'set_to_start_trans_key'                   => 'Task set to start time',
    'set_to_end_trans_key'                     => 'Task set to end time',
    'set_to_finish_trans_key'                  => 'Task set to finish Task',
    'declare_finish_trans_key'                 => 'Declare a finished',
    'declare_rejected_trans_key'               => 'Declare a rejected',
    'quantity_rejected_trans_key'              => 'Rejected quantity',
    'good_rejected_trans_key'                  => 'Good quantity',
    'play_trans_key'                           => 'Play',
    'pause_trans_key'                          => 'Pause',
    'end_trans_key'                            => 'End',
    'finish_part_qty_trans_key'                => 'Finish parts qty',
    'bad_part_qty_trans_key'                   => 'Bad part qty',
    'net_production_qty_trans_key'             => 'Net production',
    'search_task_trans_key'                    => 'Search Task',
    'calculate_task_trans_key'                 => 'Calculate Task(s)',
    'calculate_date_task_trans_key'            => 'Task Calculation Date',
    'calculate_ressource_task_trans_key'       => 'Task Calculation Resource',
    'add_sub_assembly_trans_key'               => 'Add sub assembly',
    'sub_assembly_trans_key'                   => 'Sub assembly',
    'component_trans_key'                      => 'Component',
    'not_this_page_trans_key'                  => 'Not for this page',
    'technical_cut_trans_key'                  => 'Technical cut',
    'bill_of_materials_trans_key'              => 'Bill of materials',
    'bom_trans_key'                            => 'BOM',
    'generic_trans_key'                        => 'Generic',
    'current_count_task_trans_key'             => 'Number of current tasks',
    'average_time_task_trans_key'              => 'Average processing time for OFs',
    'user_productivity_trans_key'              => 'User Productivity',
    'task_count_trans_key'                     => 'Number of tasks',
    'total_hours_per_resource_trans_key'       => 'Total hours per resource',
    'total_hours_per_month_trans_key'          => 'Hours produced over the month',
    'trs_trans_key'                            => 'OEE',
    'trs_per_month_trans_key'                  => 'OEE over the month',
    'active_calculated_price_trans_key'        => 'Activate calculated price',
    'deactivate_calculated_price_trans_key'    => 'Deactivate calculated price',
    'user_choise_trans_key'                    => 'User choice',
    
    //DELIVERY
    'delivery_notes_trans_key'                 => 'Delivery notes',
    'current_month_trans_key'                  => 'Current month',
    'deliverys_notes_request_trans_key'        => 'Deliverys notes request',
    'deliverys_notes_list_trans_key'           => 'Deliverys notes list',
    'name_of_deliverys_notes_trans_key'        => 'Name of Delivery note',
    'new_delivery_note_trans_key'              => 'New delivery note',
    'returns_trans_key'                        => 'Customer returns',
    'tracking_number_note_trans_key'           => 'Tracking number',
    'packaging_trans_key'                      => 'Packaging',
    'new_packaging_trans_key'                  => 'New packaging',

    //INVOICE
    'invoices_trans_key'                       => 'Invoices',    
    'invoice_trans_key'                        => 'Invoice',
    'invoices_request_trans_key'               => 'Invoices request',
    'invoices_list_trans_key'                  => 'Invoices list',
    'invoices_export_trans_key'                => 'Invoice lines export',
    'others_trans_key'                         => 'Others',
    'new_invoice_trans_key'                    => 'New invoice',
    'generate_pending_invoices_trans_key'      => 'Generate pending invoices',
    'number_of_invoice_trans_key'              => 'Total number of invoices',
    'amount_of_invoice_trans_key'              => 'Total amount of invoices',
    'payments_received_of_invoice_trans_key'   => 'Total amount of payments received',
    'bills_paid_trans_key'                     => 'Bills paid',
    'bills_unpaid_trans_key'                   => 'Unpaid bills',
    'top_customers_trans_key'                  => 'Top Customers',
    'top_products_trans_key'                   => 'Top Products',
    'average_payment_time_trans_key'           => 'Average payment time',
    'late_payment_time_trans_key'              => 'Late payment rate',
    'incoterm_trans_key'                       => 'Incoterm',
    'still_invoiced_trans_key'                 => 'Still to be invoiced',

    //CREDIT NOTE
    'credit_notes_trans_key'                   => 'Credit notes',
    'credit_note_trans_key'                    => 'Credit note',
    'credit_notes_list_trans_key'              => 'Credit notes  list',
    'new_credit_note_trans_key'                => 'New credit note',

    //PRODUCT
    'product_trans_key'                        => 'Product',
    'products_trans_key'                       => 'Products',
    'purchased_price_trans_key'                => 'Purchased price',
    'selling_price_trans_key'                  => 'Selling price',
    'purchased_trans_key'                      => 'Purchased',
    'sold_trans_key'                           => 'Sold',
    'tracability_trans_key'                    => 'Tracability',
    'stock_consumption_trans_key'              => 'Stock Consumption',
    'proprieties_trans_key'                    => 'Proprieties',
    'quantite_eco_min_trans_key'               => 'Qty eco min',
    'quantite_min_trans_key'                   => 'Qty min',
    'quantite_eco_max_trans_key'               => 'Qty eco max',
    'quantite_max_trans_key'                   => 'Qty max',
    'duplicate_product_trans_key'              => 'Duplicate product',
    'duplicate_purchase_quotation_trans_key'   => 'Duplicate purchase quotation',
    'other_information_trans_key'              => 'Other information',
    'index_trans_key'                          => 'Index',
    'new_product_trans_key'                    => 'New product',
    'price_by_qty_trans_key'                   => 'Price by quantity',
    'weighted_average_price_trans_key'         => 'Weighted average price',
    'last_purchase_price_price_trans_key'      => 'Last purchase price',

    'serial_numbers_trans_key'                 => 'Serial Numbers',
    'serial_numbers_list_trans_key'            => 'Serial Numbers list',
    
    'product_list_trans_key'                   => 'Product list',
    'stock_trans_key'                          => 'Stock',
    'stock_list_trans_key'                     => 'Stock list',
    'stock_location_trans_key'                 => 'Stocks location',
    'location_trans_key'                       => 'Location',
    'stock_location_list_trans_key'            => 'Stocks location list',
    'stock_location_product_list_trans_key'    => 'Stocks location product list',
    'new_stock_location_product_trans_key'     => 'New stock product line',
    'new_stock_location_trans_key'             => 'New Stock location',
    'new_stock_trans_key'                      => 'New Stock',
    'new_entry_stock_trans_key'                => 'New entry stock line',
    'new_sorting_stock_trans_key'              => 'New sorting stock line',
    'new_transfer_stock_trans_key'             => 'Stock Transfer',
    'transfer_destination_trans_key'           => 'Destination location',
    'transfer_select_destination_trans_key'    => 'Select a destination location',
    'transfer_success_trans_key'               => 'Stock transfer completed successfully',
    'transfer_insufficient_stock_trans_key'    => 'Insufficient stock to perform this transfer',
    'transfer_stock_entry_trans_key'           => 'Inter-stock transfer entry',
    'no_product_in_line_stock_trans_key'       => 'No product attached to the order line',
    'current_stock_trans_key'                  => 'Current stock',
    'entries_stock_trans_key'                  => 'Entries',
    'sortings_stock_trans_key'                 => 'Sortings',
    'total_stock_trans_key'                    => 'Total stock',
    'view_stock_list_trans_key'                => 'View stock list',
    'stock_requested_trans_key'                => 'Show stock/requested',
    'requested_trans_key'                      => 'Requested',
    'remove_from_stock_trans_key'              => 'Remove from stock',
    'remove_component_lines_stock_trans_key'   => 'Remove component lines automatically from stock',
    'create_serial_number_trans_key'           => 'Create automatically serial number if not have',
    'current_stock_note_trans_key'             => 'The display of the requested outstanding includes unfinished order lines without task and tasks whose status is equal to: Open, Started, In progress',
    'current_stock_note_2_trans_key'           => 'Please note, creating an internal order will aim to replenish stock based on outstanding stock, however this may result in overstock if the action is carried out in duplicates with the normal flow.',

    'inventory_trans_key'                      => 'Inventory',

    //PURCHASE
    'purchase_trans_key'                       => 'Purchase',
    'name_purchase_trans_key'                  => 'Name of purchase order',
    'update_valide_trans_key'                  => 'Update valide address & contact',
    'purchase_order_trans_key'                 => 'Purchase Order',
    'top_rated_supplier_trans_key'             => 'Top rated supplier',
    'delevery_time_trans_key'                  => 'Delivery times (avg days)',
    'average_purchase_price_trans_key'         => 'Avg purchase price',
    'most_purchased_products_trans_key'        => 'Most purchased products',
    'suppliers_longest_times_trans_key'        => 'Suppliers with the longest receipt times',
    'suppliers_shortest_times_trans_key'       => 'Suppliers with the shortest receipt times',
    'proposed_purchase_price_trans_key'        => 'Proposed purchase price',
    
    //PURCHASE REQUEST
    'purchase_request_trans_key'               => 'Purchase request',
    'requests_for_quotation_list_trans_key'    => 'Requests for quotation',
    'rfq_group_trans_key'                      => 'RFQ group',
    'name_quote_request_trans_key'             => 'Name of quotation request',
    'compare_rfq_trans_key'                    => 'Compare suppliers',
    
    'purchase_list_trans_key'                  => 'Purchase list',
    'waiting_to_receipt_trans_key'             => 'Waiting to receipt',

    //PURCHASE RECIEPT
    'po_receipt_trans_key'                     => 'PO receipt',
    'name_purchase_reciept_trans_key'          => 'Name of receipt',
    'po_receipt_note_trans_key'                => 'Reception awaiting inspection',
    'validate_control_trans_key'               => 'Validate inspection',
    'inspection_details_trans_key'             => 'Inspection details',
    'inspection_result_trans_key'              => 'Inspection result',
    'inspection_date_trans_key'                => 'Inspection date',
    'inspected_by_trans_key'                   => 'Inspected by',
    'select_option_trans_key'                  => 'Select an option',
    'quick_nc_creation_trans_key'              => 'Quick NC creation',
    'inspection_qty_error_trans_key'           => 'Accepted and rejected quantities cannot exceed the received quantity (:receipt).',
    'inspection_update_success_trans_key'      => 'Inspection data updated successfully.',

    'waiting_to_invoice_trans_key'             => 'Waiting to invoice',
    'incoming_invoices_trans_key'              => 'Received invoices',

    //PURCHASE INVOICE
    'invoice_supplier_trans_key'               => 'Invoice supplier',
    'name_purchase_invoice_trans_key'          => 'Name of invoice supplier',

    //QUALITY
    'quality_trans_key'                        => 'Quality',
    'non_conformities_trans_key'               => 'Non conformities',
    'non_conformitie_trans_key'                => 'Non conformity',
    'settings_trans_key'                       => 'Settings',
    'action_note_trans_key'                    => 'Actions are measures taken to prevent a problem from occurring (preventive actions), to solve an existing problem (corrective actions), or to improve a process or product (improvement requests).',
    'no_nc_trans_key'                          => 'No NC link',
    'problem_trans_key'                        => 'Problem',
    'cause_trans_key'                          => 'Cause',
    'cause_comment_trans_key'                  => 'Cause comment',
    'cause_type_trans_key'                     => 'Cause type',
    'new_cause_trans_key'                      => 'New Cause',
    'new_action_trans_key'                     => 'New action',
    'new_derogation_trans_key'                 => 'New derogation', 
    'new_failure_trans_key'                    => 'New failure',
    'new_correction_trans_key'                 => 'New Correction',
    'new_measuring_devices_trans_key'          => 'New measuring devices',
    'new_non_conformitie_trans_key'            => 'New non conformity',
    'measuring_devices_note_trans_key'         => 'The measuring devices used to measure quality.',
    'non_conformitie_note_trans_key'           => 'Non-conformity sheets are documents summing up data related to a quality issue that arose within your company, with a customer, or with a supplier and the extra costs it generated..',
    'quality_setting_note_trans_key'           => 'To avoid any waste of time, you have the possibility to predefine the defects, causes and/or correction measures that you will use the most. You will then simply have to make the relevant selection when creating a non-conformity sheet or the maintenance interventions of a resource.',
    'amdec_trans_key'                          => 'FMEA',
    'new_amdec_trans_key'                      => 'Nouvelle FMEA',
    'litigation_rate_trans_key'                => 'Litigation rate',
    'resolution_time_trans_key'                => 'Resolution time',

    //METHODS
    'methods_trans_key'                        => 'Methods',
    'service_trans_key'                        => 'Service',
    'additional_services_trans_key'            => 'Additional services',
    'labor_resource_trans_key'                 => 'Labour',
    'times_shifts_trans_key'                 => 'Work shifts',
    'new_shift_pattern_trans_key'              => 'New shift pattern',
    'shift_pattern_info_trans_key'             => 'Shift patterns define the hours worked on each weekday: one slot per day is a single shift, three slots is a 3-shift operation. The resources without a pattern use the default one.',
    'shift_slots_trans_key'                    => 'Slots',
    'shift_no_slot_warning_trans_key'          => 'This pattern has no slot yet. The resources using it keep their weekly capacity spread over five days, as if no pattern was set.',
    'shift_default_trans_key'                  => 'Default',
    'shift_default_hint_trans_key'             => 'Use as the workshop default pattern',
    'shift_apply_to_week_trans_key'            => 'Apply Monday to Friday',
    'shift_crosses_midnight_trans_key'         => 'Crosses midnight (night shift)',
    'shift_pattern_trans_key'                  => 'Work shift',
    'shift_pattern_created_success_trans_key'  => 'Successfully created shift pattern.',
    'shift_pattern_updated_success_trans_key'  => 'Successfully updated shift pattern.',
    'shift_slot_created_success_trans_key'     => 'Successfully created slot.',
    'shift_slot_deleted_success_trans_key'     => 'Successfully deleted slot.',
    'labor_resource_hint_trans_key'            => 'Resource made of people (team, manual station) rather than a machine',
    'labor_ratio_trans_key'                    => 'Operators per hour (0 = none, 0.5 = one operator for two machines)',
    'qualified_users_trans_key'                => 'Qualified users',
    'additional_services_hint_trans_key'       => 'Other operations this resource can perform, in order of preference',
    'resource_not_in_service_trans_key'        => 'This resource cannot perform the service of the task.',
    'new_service_trans_key'                    => 'New Service',
    'new_ressource_trans_key'                  => 'New Resource',
    'new_section_trans_key'                    => 'New Section',
    'new_location_trans_key'                   => 'New location',
    'new_unit_trans_key'                       => 'New unit',
    'new_family_trans_key'                     => 'New family',
    'new_tool_trans_key'                       => 'New tool',
    'family_trans_key'                         => 'Family',
    'new_standard_bom_trans_key'               => 'New standard nomenclature',
    'standard_bom_trans_key'                   => 'Standard nomenclature',
    'service_info_trans_key'                   => 'In the Type scrolling list, indicate whether the service is productive (manufacturing operation) or actually involves the procurement of raw materials or supplies, is a service completed externally, i.e. by a subcontractor, or is a composed component. The service type is then used to filter data when creating a list of tasks, a bill of materials, etc. For raw materials, a distinction is also made between bars, plates and blocks. The screen used to detail raw materials may look different, depending on the raw material type.',
    'ressource_info_trans_key'                 => 'Depending on your working method, resources may be employees or machines or a mixture of both. In any case, you must indicate for each of them the maximum number of working hours per day as well as the services that they can complete.',
    'unit_info_trans_key'                      => 'You must enter all units you may have to work with in your business. This may be Parts, Meters, Kilograms, etc.',
    'family_info_trans_key'                    => 'Defining subgroups per service allows filtering components at later stages.',
    'section_info_trans_key'                   => 'The Sections function allows you to define all the sections making up your company, i.e. the physical zones where work stations and operators are grouped together according to their job and cost.',

    //ACCOUNTING
    'accounting_trans_key'                     => 'Accounting',
    'new_payment_methods_trans_key'            => 'New Payment choice mode',
    'new_payment_conditions_trans_key'         => 'New Payment condition mode',
    'new_delevery_method_trans_key'            => 'New Delivery method mode',
    'new_vat_trans_key'                        => 'New VAT mode',
    'accounting_allocations_trans_key'         => 'Accounting allocations',
    'new_accounting_allocations_trans_key'     => 'New accounting allocation',
    'assets_trans_key'                         => 'Assets',

    //TIME
    'times_setting_trans_key'                  => 'Times setting',  
    'new_machine_event_trans_key'              => 'New machine event type',
    'new_banck_holiday_trans_key'              => 'New bank holiday',
    'new_improduct_time_trans_key'             => 'New non-productive time',
    'machine_statu_trans_key'                  => 'Machine status',
    'resource_required_trans_key'              => 'Resource required',  
    
    //HUMAN RESOURCES
    'human_resources_trans_key'                => 'Human resources',
    'categories_trans_key'                     => 'Categories',
    'payer_trans_key'                          => 'Payer',
    'scan_file_trans_key'                      => 'Scan file',
    'location_expense_trans_key'               => 'Location',
    'expense_categories_trans_key'             => 'Expense categories',
    'new_expense_categories_trans_key'         => 'New expense categories',
    
    //OSH
    'osh_trans_key'                            => 'OSH',
    'incidents_trans_key'                      => 'Incidents',
    'risques_trans_key'                        => 'Risks',
    'formations_trans_key'                     => 'Trainings',
    'conformites_trans_key'                    => 'Conformities',
    'osh_training_List_trans_key'              => 'Training List',
    'type_of_training_trans_key'               => 'Type of Training',
    'training_date_trans_key'                  => 'Training Date',
    'expiration_date_trans_key'                => 'Expiration Date',
    'certification_obtained_trans_key'         => 'Certification Obtained',
    'new_training_trans_key'                   => 'New Training',
    'new_risk_trans_key'                       => 'New Risk',
    'new_conformity_trans_key'                 => 'New Conformity',
    'new_incident_trans_key'                   => 'New Incident',
    'severity_class_trans_key'                 => 'Severity',
    'preventive_measures_class_trans_key'      => 'Preventive Measures',
    'incident_date_class_trans_key'            => 'Incident Date',
    'corrective_actions_class_trans_key'       => 'Corrective Actions',
    
    //USER
    'whiteboard_trans_key'                     => 'Whiteboard',
    'users_trans_key'                          => 'Users',
    'users_list_trans_key'                     => 'Users list',
    'about_trans_key'                          => 'About',
    'new_contract_trans_key'                   => 'New contract',
    'personnal_information_trans_key'          => 'Personnal information',
    'personnal_email_trans_key'                => 'Personnal email',
    'about_you_trans_key'                      => 'About you',
    'about_setup_trans_key'                    => 'Account setup',
    'password_account_trans_key'               => 'Password (only for confirm changes)',
    'marital_status_trans_key'                 => 'Marital status', 
    'adress_section_trans_key'                 => 'Address section',
    
    //YOUR COMPANIE
    'your_company_trans_key'                   => 'Your company',
    'default_value_trans_key'                  => 'Default value',
    'add_value_day_offer_trans_key'            => 'Additional day of validity of the offer',
    'add_value_day_delivery_trans_key'         => 'Additional day delivery delay',
    'day_trans_key'                            => 'Day(s)',
    'header_font_color_trans_key'              => 'Header font Color',
    'pdf_theme_trans_key'                      => 'PDF theme',
    'pdf_theme_fallback_trans_key'             => 'Default',
    'pdf_theme_help_trans_key'                 => 'Choose the theme used to render generated PDFs.',
    'pdf_custom_css_trans_key'                 => 'Custom PDF CSS',
    'pdf_custom_css_placeholder_trans_key'     => 'Enter additional CSS rules applied to the selected PDF theme',
    'pdf_custom_css_help_trans_key'            => 'Leave empty to use the theme styles as-is.',
    'manufacturing_information_trans_key'      => 'Manufacturing information',
    'bare_code_type_trans_key'                 => 'Code bare type for declare task',
    'make_an_announcement_trans_key'           => 'Make an announcement',
    'make_new_permissions_trans_key'           => 'Make a new Permissions',
    'add_role_permission_trans_key'            => 'Add role in Permissions',
    'public_link_cgv_trans_key'                => 'Publish public link for CGV',
    'add_cgv_to_quote_order_trans_key'         => 'Add following the quote or order',
    'show_current_file_trans_key'              => 'Show current file',
    'name_field_trans_key'                     => 'Name field',
    'type_field_trans_key'                     => 'Type field',
    'entity_type_trans_key'                    => 'Type entity',
    'roles_and_permissions_trans_key'          => 'Roles & permissions',
    'import_export_trans_key'                  => 'Imports/Exports',
    'logs_view_trans_key'                      => 'Logs view',
    'template_mail_trans_key'                  => 'Mail template',
    'attached_mail_trans_key'                  => 'Attached mail',
    'template_list_mail_trans_key'             => 'Template list mail',
    'new_template_mail_trans_key'              => 'New template mail',
    'attachment_trans_key'                     => 'Attachment',
    'new_mail_trans_key'                       => 'New E-mail',

    //PROFIL
    'profile_trans_key'                        => 'Profile',
    'information_trans_key'                    => 'Information',
    'pay_grade_trans_key'                      => 'Pay grade',
    'work_station_id_trans_key'                => 'Work station id',
    'joined_date_trans_key'                    => 'Joined date',
    'confirmation_date_trans_key'              => 'Confirmation date',
    'notification_choice_trans_key'            => 'Notification choice',
    'new_user_trans_key'                       => 'New user',
    'new_absence_request_trans_key'            => 'New absence request',
    'expenses_trans_key'                       => 'Expenses',
    'expense_report_trans_key'                 => 'Expense report',
    'new_expense_report_trans_key'             => 'New expense report',
    'expense_validation_trans_key'             => 'Expense validation',
    'attendance_report_trans_key'              => 'Attendance report',
    'total_duration_trans_key'                 => 'Total duration',
    'anomalies_trans_key'                      => 'Anomalies',
    'filter_trans_key'                         => 'Filter',
    'all_trans_key'                            => 'All',

    'licence_trans_key'                        => 'Licence',
    'release_note_trans_key'                   => 'Release notes',
    
    'search_results_trans_key'                 => 'Search results',

    //WORKSHOP
    'workshop_interface_trans_key'             => 'Workshop Interface',
    'to_access_trans_key'                      => 'To access',
    'production_declaration_trans_key'         => 'Production Declaration',
    'note_1_trans_key'                         => 'Consult and plan the tasks to be carried out.',
    'note_2_trans_key'                         => 'Make production declarations.',

    'energy_consumption_trans_key'             => 'Energy consumption',
    'machine_trans_key'                        => 'Machine',
    'kwh_trans_key'                            => 'kWh',
    'cost_per_kwh_trans_key'                   => 'Cost per kWh',
    'total_cost_trans_key'                     => 'Total cost',

    'order_created_log_trans_key'              => 'Order created',
    'order_creation_failed_log_trans_key'      => 'Order creation failed',
    'quote_updated_log_trans_key'              => 'Quote updated',
    'quote_update_failed_log_trans_key'        => 'Quote update failed',
    'quote_update_success_trans_key'           => 'Successfully updated quote',
    'average_supply_delay_trans_key'           => 'Average supply delay',
    'order_site_trans_key'                     => 'Job site',
    'characteristics_trans_key'                => 'Characteristics',
    'contact_info_trans_key'                   => 'Contact info',
    'report_trans_key'                         => 'Report',
    'send_time_trans_key'                      => 'Send time',
    'enabled_trans_key'                        => 'Enabled',
    'automatic_email_reports_trans_key'        => 'Automatic email reports',
    'automatic_email_reports_help_trans_key'   => 'Choose which reports you want to receive and when to receive them.',
    'automatic_email_reports_saved_trans_key'  => 'Automatic email report settings updated successfully.',
    'automatic_email_report_subject_trans_key' => 'Automatic report: :report',
    'automatic_email_reports_intro_trans_key'  => 'Hello :name, here is your scheduled report.',
    'generated_at_trans_key'                   => 'Generated at',
    'overdue_orders_report_trans_key'          => 'Overdue orders',
    'tomorrow_orders_report_trans_key'         => 'Orders due tomorrow',
    'low_stock_report_trans_key'               => 'Low stock items',

    //GMAO
    'gmao_work_orders_page_title_trans_key'        => 'GMAO - Work Orders',
    'gmao_work_orders_card_title_trans_key'        => 'Work Orders',
    'gmao_new_work_order_trans_key'                => 'New Work Order',
    'gmao_edit_work_order_trans_key'               => 'Edit Work Order',
    'gmao_work_order_trans_key'                    => 'Work Order',
    'gmao_work_order_number_trans_key'             => 'Work Order #:id',
    'gmao_technician_trans_key'                    => 'Technician',
    'gmao_select_asset_trans_key'                  => 'Select an asset',
    'gmao_not_specified_trans_key'                 => 'Not specified',
    'gmao_not_available_trans_key'                 => 'Not available',
    'gmao_not_started_trans_key'                   => 'Not started',
    'gmao_not_finished_trans_key'                  => 'Not finished',
    'gmao_not_assigned_trans_key'                  => 'Not assigned',
    'gmao_unassigned_trans_key'                    => 'Unassigned',
    'gmao_assigned_technician_trans_key'           => 'Assigned technician',
    'gmao_requested_at_trans_key'                  => 'Requested at',
    'gmao_scheduled_at_trans_key'                  => 'Scheduled at',
    'gmao_started_at_trans_key'                    => 'Started at',
    'gmao_finished_at_trans_key'                   => 'Finished at',
    'gmao_completed_at_trans_key'                  => 'Completed at',
    'gmao_estimated_duration_minutes_trans_key'    => 'Estimated duration (minutes)',
    'gmao_actual_duration_minutes_trans_key'       => 'Actual duration (minutes)',
    'gmao_estimated_duration_trans_key'            => 'Estimated duration',
    'gmao_actual_duration_trans_key'               => 'Actual duration',
    'gmao_minutes_suffix_trans_key'                => 'min',
    'gmao_actions_performed_trans_key'             => 'Actions performed',
    'gmao_parts_consumed_trans_key'                => 'Parts consumed',
    'gmao_comments_photos_trans_key'               => 'Comments / photos',
    'gmao_severity_trans_key'                      => 'Severity',
    'gmao_machine_stopped_trans_key'               => 'Machine stopped',
    'gmao_failure_started_at_trans_key'            => 'Failure started at',
    'gmao_general_information_trans_key'           => 'General Information',
    'gmao_created_by_trans_key'                    => 'Created by',
    'gmao_schedule_timeline_trans_key'             => 'Schedule & Timeline',
    'gmao_execution_details_trans_key'             => 'Execution Details',
    'gmao_none_recorded_trans_key'                 => 'None recorded',
    'gmao_none_used_trans_key'                     => 'None used',
    'gmao_no_comments_photos_trans_key'            => 'No comments or photos added',
    'gmao_back_to_list_trans_key'                  => 'Back to List',
    'gmao_quick_info_trans_key'                    => 'Quick Info',
    'gmao_dashboard_page_title_trans_key'          => 'GMAO - Dashboard',
    'gmao_kpi_maintenance_card_trans_key'          => '3. 📊 KPI & Maintenance indicators',
    'gmao_kpi_trans_key'                           => 'KPI',
    'gmao_maintenance_plans_page_title_trans_key'  => 'GMAO - Maintenance Plans',
    'gmao_maintenance_plans_card_title_trans_key'  => 'Maintenance Plans',
    'gmao_new_maintenance_plan_trans_key'          => 'New Maintenance Plan',
    'gmao_edit_maintenance_plan_trans_key'         => 'Edit Maintenance Plan',
    'gmao_maintenance_plan_trans_key'              => 'Maintenance Plan',
    'gmao_maintenance_plan_number_trans_key'       => 'Maintenance Plan #:id',
    'gmao_trigger_trans_key'                       => 'Trigger',
    'gmao_trigger_type_trans_key'                  => 'Trigger type',
    'gmao_trigger_value_trans_key'                 => 'Trigger value',
    'gmao_trigger_value_hint_trans_key'            => 'Trigger value (weekly, monthly, 500 hours, 200 cycles...)',
    'gmao_fixed_date_trans_key'                    => 'Fixed date',
    'gmao_required_skill_trans_key'                => 'Required skill',
    'gmao_actions_list_trans_key'                  => 'Actions list',
    'gmao_required_parts_trans_key'                => 'Required parts',
    'gmao_active_trans_key'                        => 'Active',
    'gmao_frequency_trans_key'                     => 'Frequency',
    'gmao_trigger_configuration_trans_key'         => 'Trigger Configuration',
    'gmao_schedule_information_trans_key'          => 'Schedule Information',
    'gmao_next_scheduled_date_trans_key'           => 'Next Scheduled Date',
    'gmao_last_modified_trans_key'                 => 'Last Modified',
    'gmao_execution_resources_trans_key'           => 'Execution Resources',
    'gmao_no_actions_defined_trans_key'            => 'No actions defined',
    'gmao_generate_work_order_trans_key'           => 'Generate Work Order',
    'gmao_edit_plan_trans_key'                     => 'Edit Plan',
    'gmao_plan_summary_trans_key'                  => 'Plan Summary',
    'gmao_duration_trans_key'                      => 'Duration',
    'gmao_next_scheduled_trans_key'                => 'Next Scheduled',
    'gmao_maintenance_work_orders_trans_key'       => 'Maintenance work orders',
    'methods_overview_trans_key'               => 'Methods overview',
    'logs_trans_key'                            => 'Logs',
    'beta_trans_key'                            => 'Beta',
    'info_trans_key'                            => 'Info',
    'customer_disabled_warning_trans_key'       => 'The customer :customer is currently disabled. You cannot change the customer name, contact, or address.',

    //FLASH MESSAGES
    'user_info_updated_success_trans_key' => 'Successfully updated user information.',
    'user_locked_success_trans_key' => 'Successfully locked user.',
    'contract_added_success_trans_key' => 'Successfully added contract.',
    'factory_info_updated_success_trans_key' => 'Successfully updated factory information.',
    'announcement_added_success_trans_key' => 'Successfully added announcement.',
    'announcement_deleted_success_trans_key' => 'Successfully deleted announcement.',
    'stock_line_created_success_trans_key' => 'Successfully created new stock line.',
    'stock_move_created_success_trans_key' => 'Successfully created new stock move.',
    'purchase_order_updated_success_trans_key' => 'Successfully updated purchase order.',
    'purchase_receipt_updated_success_trans_key' => 'Successfully updated receipt.',
    'site_created_success_trans_key' => 'Site created.',
    'site_updated_success_trans_key' => 'Site updated.',
    'site_deleted_success_trans_key' => 'Site deleted.',
    'no_default_address_contact_vat_error_trans_key' => 'No default settings found for address, contact or accounting VAT.',
    'purchase_order_created_success_trans_key' => 'Successfully created new purchase order.',
    'no_supplied_in_progress_status_error_trans_key' => 'No "Supplied" or "In progress" status found in kanban to define progress.',
    'non_conformity_created_success_trans_key' => 'Successfully created non-conformity.',
    'non_conformity_updated_success_trans_key' => 'Successfully updated non-conformity.',
    'role_created_success_trans_key' => 'Role created successfully.',
    'role_updated_success_trans_key' => 'Role updated successfully.',
    'role_deleted_success_trans_key' => 'Role deleted successfully.',
    'supplier_added_success_trans_key' => 'Successfully added supplier.',
    'quantity_price_added_success_trans_key' => 'Successfully added quantity price.',
    'product_updated_success_trans_key' => 'Successfully updated product.',
    'absence_request_created_success_trans_key' => 'Successfully created absence request.',
    'absence_request_updated_success_trans_key' => 'Successfully updated absence request.',
    'preorders_pdf_processed_success_trans_key' => 'PDF(s) sent, Python processing finished, and CSV import completed successfully.',
    'preorders_pdf_uploaded_success_trans_key' => 'PDF(s) successfully uploaded to storage.',
    'preorders_pdf_all_duplicates_trans_key' => 'No new PDF processed: the following file(s) were already imported:',
    'preorders_pdf_some_duplicates_trans_key' => 'Some files were skipped because they were already imported:',
    'preorders_matching_done_success_trans_key' => 'Article matching completed.',
    'email_template_created_success_trans_key' => 'Email template created successfully!',
    'email_template_updated_success_trans_key' => 'Email template updated!',
    'email_template_deleted_success_trans_key' => 'Email template deleted successfully!',
    'resource_created_success_trans_key' => 'Successfully created resource.',
    'resource_updated_success_trans_key' => 'Successfully updated resource.',
    'service_created_success_trans_key' => 'Successfully created service.',
    'service_updated_success_trans_key' => 'Successfully updated service.',
    'tool_created_success_trans_key' => 'Successfully created tool.',
    'tool_updated_success_trans_key' => 'Successfully updated tool.',

    /* ERROR PAGES */
    'error_back_dashboard_trans_key'    => 'Back to dashboard',
    'error_404_title_trans_key'         => 'Page not found',
    'error_404_heading_trans_key'       => 'Oops! Page not found.',
    'error_404_message_trans_key'       => 'We could not find the page you were looking for. You may <a href=":url">return to dashboard</a>.',
    'error_403_title_trans_key'         => 'Access denied',
    'error_403_heading_trans_key'       => 'Oops! Access denied.',
    'error_403_message_trans_key'       => 'You do not have permission to access this page. You may <a href=":url">return to dashboard</a>.',
    'error_419_title_trans_key'         => 'Session expired',
    'error_419_heading_trans_key'       => 'Oops! Session expired.',
    'error_419_message_trans_key'       => 'Your session has expired. Please refresh the page or <a href=":url">return to dashboard</a>.',
    'error_500_title_trans_key'         => 'Server error',
    'error_500_heading_trans_key'       => 'Oops! Internal server error.',
    'error_500_message_trans_key'       => 'An unexpected error occurred. We will work on fixing that right away. Meanwhile, you may <a href=":url">return to dashboard</a>.',
    'error_503_title_trans_key'         => 'Maintenance',
    'error_503_heading_trans_key'       => 'We\'ll be back soon!',
    'error_503_message_trans_key'       => 'Sorry for the inconvenience but we\'re performing some maintenance. We\'ll be back online shortly.',

    // --- Unified document storage (React FileManager component) ---
    'loading_trans_key'                        => 'Loading...',
    'download_trans_key'                       => 'Download',
    'file_uploaded_success_trans_key'          => 'File uploaded successfully.',
    'file_extension_not_allowed_trans_key'     => 'This file format is not accepted.',
    'file_too_large_trans_key'                 => 'File is too large.',
    'file_role_trans_key'                      => 'Document type',
    'file_role_auto_trans_key'                 => 'Automatic (from the format)',
    'file_all_roles_trans_key'                 => 'All types',
    'file_primary_trans_key'                   => 'Primary',
    'file_set_as_primary_trans_key'            => 'Set as primary document',
    'file_dropzone_hint_trans_key'             => 'Drop your files here, or click to browse',
    'file_dropzone_formats_trans_key'          => 'PDF, STL, STEP, IGES, DXF, SVG, images, office files...',
    'file_select_a_file_trans_key'             => 'Select a file to display it',
    'file_no_inline_preview_trans_key'         => 'No preview available for this format.',
    'file_open_new_tab_trans_key'              => 'Open in a tab',
    'file_confirm_delete_trans_key'            => 'Delete the file ":name"?',
    'file_reset_view_trans_key'                => 'Reset',
    'file_triangles_trans_key'                 => ':count triangles',
    'file_entities_trans_key'                  => ':count entities',
    'file_layers_trans_key'                    => ':count layers',
    'file_solids_trans_key'                    => ':count solids',
    'file_ignored_trans_key'                   => 'Ignored',
    'file_no_drawable_entity_trans_key'        => 'No drawable entity in this DXF file.',
    'file_brep_read_failed_trans_key'          => 'Unable to read this CAD file.',
    'file_view_iso_trans_key'                  => 'Iso',
    'file_view_front_trans_key'                => 'Front',
    'file_view_back_trans_key'                 => 'Back',
    'file_view_top_trans_key'                  => 'Top',
    'file_view_bottom_trans_key'               => 'Bottom',
    'file_view_left_trans_key'                 => 'Left',
    'file_view_right_trans_key'                => 'Right',
    'file_role_plan_trans_key'                 => 'Drawing',
    'file_role_modele_3d_trans_key'            => '3D model',
    'file_role_vectoriel_trans_key'            => 'Vector / DXF',
    'file_role_photo_trans_key'                => 'Picture',
    'file_role_cam_trans_key'                  => 'CAM / Machining',
    'file_role_certificat_trans_key'           => 'Material certificate',
    'file_role_controle_trans_key'             => 'Quality control',
    'file_role_autre_trans_key'                => 'Other',

    // Load planning (services x days heat grid)
    'load_planning_today_trans_key'                 => 'Today',
    'load_planning_weekend_trans_key'               => 'Weekend',
    'load_planning_compact_view_trans_key'          => 'Compact view',
    'load_planning_detailed_view_trans_key'         => 'Detailed view',
    'load_planning_legend_trans_key'                => 'Load level',
    'load_planning_level_free_trans_key'            => 'Free',
    'load_planning_level_low_trans_key'             => 'Light',
    'load_planning_level_medium_trans_key'          => 'Balanced',
    'load_planning_level_high_trans_key'            => 'Tight',
    'load_planning_level_over_trans_key'            => 'Overloaded',
    'load_planning_period_total_trans_key'          => 'Total :hours h',
    'load_planning_overloaded_days_trans_key'       => ':count overloaded day(s)',
    'load_planning_tasks_count_trans_key'           => ':count task(s)',
    'load_planning_tasks_processed_trans_key'       => ':count task(s) processed',
    'load_planning_running_trans_key'               => 'Running…',
    'load_planning_finished_trans_key'              => 'Finished',
    'load_planning_default_capacity_trans_key'      => 'Default daily capacity',
    'load_planning_default_capacity_hint_trans_key' => 'services without configured resources — values used to compute the load rate',
    'load_planning_capacity_resources_trans_key'    => 'Capacity from the configured resources',
    'load_planning_capacity_custom_trans_key'       => 'Manually defined capacity (no resource configured)',
    'load_planning_capacity_fallback_trans_key'     => 'No resource configured — 8h/day fallback',
    'load_planning_granularity_trans_key'           => 'Load by',
    'load_planning_by_service_trans_key'            => 'Service',
    'load_planning_by_resource_trans_key'           => 'Resource',
    'load_planning_unassigned_trans_key'            => 'Unassigned tasks',
    'load_planning_capacity_real_trans_key'         => 'Capacity :hours h/day',

    // Inventory workflow (Altior-style counting)
    'new_inventory_trans_key'                       => 'New inventory',
    'inventory_default_label_trans_key'             => 'Inventory',
    'inventory_created_trans_key'                   => 'Inventory created',
    'inventory_already_locked_trans_key'            => 'This inventory is already locked',
    'inventory_stock_changed_since_snapshot_trans_key' => 'Stock changed between inventory creation and validation. Update your file.',
    'inventory_invalid_status_trans_key'            => 'Invalid row status',
    'inventory_unknown_detail_id_trans_key'         => 'Unknown row',
    'inventory_missing_product_or_location_trans_key' => 'Missing product code or location for a new row',
    'inventory_unknown_product_trans_key'           => 'Unknown product',
    'inventory_unknown_location_trans_key'          => 'Unknown location for this product',
    'inventory_counted_below_reserved_trans_key'    => 'Counted quantity is below reserved quantity',
    'inventory_pause_moves_warning_trans_key'       => 'Make sure your stock movements (receipts, deliveries, task declarations) are up to date and paused during the inventory.',
    'inventory_export_step_trans_key'               => 'Export the counting list',
    'inventory_export_normal_trans_key'             => 'Download counting list',
    'inventory_export_blind_trans_key'              => 'Blind version (shop-floor print)',
    'inventory_export_hint_trans_key'               => 'Download the XLSX file, print it or share it with counters. The blind version hides the theoretical quantity to prevent confirmation bias.',
    'inventory_import_step_trans_key'               => 'Import the counted file',
    'inventory_import_hint_trans_key'               => 'Drop the filled XLSX file. The system will check no movement altered the stock since creation.',
    'inventory_summary_step_trans_key'              => 'Summary and validation',
    'inventory_confirm_validate_trans_key'          => 'Confirm validation? Variances will generate irreversible stock movements.',
    'inventory_shortage_trans_key'                  => 'Inventory shortage (exit)',
    'theoretical_qty_trans_key'                     => 'Theoretical qty',
    'reserved_qty_trans_key'                        => 'Reserved qty',
    'counted_qty_trans_key'                         => 'Counted qty',
    'variance_qty_trans_key'                        => 'Qty variance',
    'variance_value_trans_key'                      => 'Value variance',
    'zone_trans_key'                                => 'Zone',
    'batch_trans_key'                               => 'Batch',
    'row_trans_key'                                 => 'Row',
    'errors_found_trans_key'                        => 'Errors found',
    'total_lines_trans_key'                         => 'Total lines',
    'counted_lines_trans_key'                       => 'Counted lines',
    'positive_variance_trans_key'                   => 'Positive variances',
    'negative_variance_trans_key'                   => 'Negative variances',
    'net_variance_trans_key'                        => 'Net variance',
    'download_counting_file_trans_key'              => 'Download counting file',
    'entry_move_trans_key'                          => 'Entry move',
    'exit_move_trans_key'                           => 'Exit move',
    'nb_part_trans_key'                             => 'Nb parts',
    'surface_perc_trans_key'                        => 'Surface %',
    'scope_trans_key'                               => 'Scope',
    'scope_all_trans_key'                           => 'All stock',
    'scope_location_trans_key'                      => 'By location',
    'scope_category_trans_key'                      => 'By category',
    'select_locations_trans_key'                    => 'Select locations',
    'select_categories_trans_key'                   => 'Select categories',
    'status_draft_trans_key'                        => 'Draft',
    'status_exported_trans_key'                     => 'Count imported',
    'status_validated_trans_key'                    => 'Validated',
    'status_cancelled_trans_key'                    => 'Cancelled',
    'confirm_cancel_trans_key'                      => 'Confirm cancellation?',
    'preview_trans_key'                             => 'Preview',
    'drop_xlsx_file_trans_key'                      => 'Drop the XLSX file here',
    'actions_trans_key'                             => 'Actions',
    'create_trans_key'                              => 'Create',
    'created_by_trans_key'                          => 'Created by',
    'import_trans_key'                              => 'Import',
    'validated_at_trans_key'                        => 'Validated on',
    'notes_trans_key'                               => 'Notes',
    'unit_cost_trans_key'                           => 'Unit cost',

    // Inventory show UX (stepper + explanatory blocks)
    'step_draft_trans_key'                          => 'Creation',
    'step_exported_trans_key'                       => 'Counting',
    'step_import_trans_key'                         => 'Import',
    'step_validated_trans_key'                      => 'Validated',
    'xlsx_content_title_trans_key'                  => 'The XLSX file contains:',
    'xlsx_content_line_1_trans_key'                 => 'One row per product x location x batch x geometry (a full sheet and an offcut form two distinct rows).',
    'xlsx_content_line_2_trans_key'                 => 'A blank "Counted qty" column to fill, a "Status" column with dropdown, an auto-computed variance.',
    'xlsx_content_line_3_trans_key'                 => 'X/Y/Z dimensions, part count and surface % on the right - edit them for a new row (found offcut).',
    'export_blind_tooltip_trans_key'                => 'The blind version hides Theoretical qty, Reserved qty, Cost and Variance columns - meant for shop-floor printing without bias.',
    'import_stock_change_warning_trans_key'         => 'If a stock movement occurred after the inventory creation, the import will be rejected with the list of affected products.',
    'analysing_trans_key'                           => 'Analysing…',
    'importing_trans_key'                           => 'Importing…',
    'reimport_hint_trans_key'                       => 'Re-import allowed',
    'errors_hint_trans_key'                         => 'Fix these errors in the XLSX file and re-upload. Only the first 50 errors are displayed.',
    'more_errors_trans_key'                         => 'more errors',
    'preview_ok_title_trans_key'                    => 'File valid - ready to import',
    'preview_ok_hint_trans_key'                     => 'Import will write the counted quantities to the inventory. No stock movement is created yet - you can still cancel before validation.',
    'confirm_import_trans_key'                      => 'Confirm import',
    'validate_incidence_hint_trans_key'             => 'Validation will create an entry move (typ=1) for each positive variance and an exit move (typ=15) for each negative variance. These moves are irreversible.',
    'value_impact_trans_key'                        => 'Value impact',
    'validated_summary_trans_key'                   => 'Inventory validated. Regularisation moves were created and the counting file archived in the GED.',
    'cancelled_summary_trans_key'                   => 'Inventory cancelled. No stock movement was created.',
    'details_step_trans_key'                        => 'Line details',
    'filter_variance_only_trans_key'                => 'Variances only',

    // --- Human resources: leave and documents ---
    'leave_balances_trans_key'                     => 'Leave balances',
    'leave_type_trans_key'                         => 'Leave type',
    'leave_types_trans_key'                        => 'Leave types',
    'leave_new_type_trans_key'                     => 'New leave type',
    'leave_entitled_trans_key'                     => 'Entitlement',
    'leave_carried_over_trans_key'                 => 'Carried over',
    'leave_adjustment_trans_key'                   => 'Adjustment',
    'leave_acquired_trans_key'                     => 'Total credited',
    'leave_taken_trans_key'                        => 'Taken',
    'leave_pending_trans_key'                      => 'Pending',
    'leave_remaining_trans_key'                    => 'Remaining',
    'leave_days_trans_key'                         => 'Days',
    'leave_not_counted_trans_key'                  => 'no balance',
    'leave_period_trans_key'                       => 'Reference period',
    'leave_default_quota_trans_key'                => 'Default annual quota',
    'leave_counts_against_balance_trans_key'       => 'Consumes balance',
    'leave_generate_balances_trans_key'            => 'Generate missing entitlements',
    'leave_pending_hint_trans_key'                 => 'The remaining balance deducts both approved and pending requests.',
    'leave_balance_saved_success_trans_key'        => 'Leave balance saved',
    'leave_balance_generated_success_trans_key'    => ':count entitlement(s) generated',
    'leave_type_created_success_trans_key'         => 'Leave type created',
    'leave_type_updated_success_trans_key'         => 'Leave type updated',
    'hr_documents_confidential_trans_key'          => 'Confidential documents: readable by the employee concerned and by HR only.',
    'hr_documents_self_hint_trans_key'             => 'Your HR documents. Only you and the HR team can access them.',
    'file_role_contrat_trans_key'                  => 'Employment contract',
    'file_role_bulletin_paie_trans_key'            => 'Payslip',
    'file_role_arret_travail_trans_key'            => 'Sick note',
    'file_role_diplome_trans_key'                  => 'Diploma / certification',
    'file_role_identite_trans_key'                 => 'Identity document',
    // --- Human resources: payroll export ---
    'payroll_export_trans_key'                     => 'Payroll export',
    'payroll_variables_trans_key'                  => 'Payroll variable elements',
    'payroll_month_trans_key'                      => 'Month',
    'payroll_matricule_trans_key'                  => 'Employee number',
    'payroll_quantity_trans_key'                   => 'Quantity',
    'payroll_worked_hours_trans_key'               => 'Badged hours',
    'payroll_production_hours_trans_key'           => 'Production hours',
    'payroll_warnings_trans_key'                   => 'To check before sending',
    'payroll_warning_attendance_trans_key'         => ':count attendance anomaly(ies) over the period (unclosed punch)',
    'payroll_warning_matricule_trans_key'          => 'no payroll number set, the internal id will be used',
    'payroll_export_hint_trans_key'                => 'Neutral layout: employee number, period, pay item code, quantity. Mapping the pay item codes is done on the payroll software side.',
    // --- OSH: versatility matrix ---
    'skills_matrix_trans_key'                      => 'Versatility matrix',
    'skills_alerts_trans_key'                      => 'Gaps found',
    'skills_no_alert_trans_key'                    => 'No gap: every assigned task is covered by a valid authorisation.',
    'skills_matrix_advisory_trans_key'             => 'Informative screen: a missing or expired authorisation never blocks an assignment, a task start or the scheduling. The call stays with the workshop.',
    'skills_warning_window_trans_key'              => 'an authorisation is flagged as expiring :days days before its end date',
    'skills_resources_hint_trans_key'              => 'Resources this authorisation is expected on. Used only to report gaps.',
    'skills_status_valid_trans_key'                => 'Valid',
    'skills_status_expiring_trans_key'             => 'Expiring',
    'skills_status_expired_trans_key'              => 'Expired',
    'skills_status_failed_trans_key'               => 'Not obtained',
    'skills_status_missing_trans_key'              => 'Not trained',
    'training_types_trans_key'                     => 'Authorisation types',
    'training_new_type_trans_key'                  => 'New authorisation type',
    'training_type_trans_key'                      => 'Authorisation type',
    'training_validity_trans_key'                  => 'Validity (months)',
    'training_type_created_success_trans_key'      => 'Authorisation type created',
    'training_type_updated_success_trans_key'      => 'Authorisation type updated',

    'mail_config_menu_trans_key'               => 'Email settings',
    'invoice_draft_no_email_trans_key'         => 'This invoice is still a draft — it cannot be sent by email. Issue it first.',
    'invoice_draft_no_email_short_trans_key'   => 'email unavailable',
    'attachment_preview_trans_key'             => 'Attachment preview',
    'attachment_preview_hint_trans_key'        => 'The PDF sent to the recipient is regenerated on the fly at send time — reload this page to reflect the latest document changes.',
    'open_in_new_tab_trans_key'                => 'Open in a new tab',
    'email_pdf_preview_error_trans_key'        => 'The PDF cannot be generated',
    'email_pdf_preview_error_hint_trans_key'   => 'Fix the items listed above (factory or client record) and reload this page. Sending stays blocked until the PDF can be generated.',
    'attach_document_pdf_trans_key'            => 'Attach document PDF',
    'optional_trans_key'                       => 'optional',
    'email_history_trans_key'                  => 'Send history',
    'email_logs_trans_key'                     => 'Email log',
    'email_log_detail_trans_key'               => 'Send detail',
    'email_status_sent_trans_key'              => 'Sent',
    'email_status_failed_trans_key'            => 'Failed',
    'email_status_pending_trans_key'           => 'Pending',
    'email_sent_success_trans_key'             => 'Email sent successfully!',
    'email_send_failed_trans_key'              => 'Send failed:',
    'email_pdf_generation_failed_trans_key'    => 'Could not generate document PDF:',
    'email_resend_trans_key'                   => 'Resend',
    'email_resend_success_trans_key'           => 'Email re-sent.',
    'email_resend_orphan_trans_key'            => 'Cannot resend: the linked document no longer exists.',
    'to_trans_key'                             => 'Recipient',
    'by_trans_key'                             => 'By',
    'document_trans_key'                       => 'Document',
    'error_trans_key'                          => 'Error',
    'details_trans_key'                        => 'Details',
    'search_by_recipient_or_subject_trans_key' => 'Search by recipient or subject…',
    'reset_trans_key'                          => 'Reset',

    // Widget de bascule d'affichage dans la navbar (clair / sombre / pro)
    'theme_light_trans_key'             => 'Display: light mode',
    'theme_dark_trans_key'              => 'Display: dark mode',
    'theme_pro_trans_key'               => 'Display: pro mode (neutral)',
];
