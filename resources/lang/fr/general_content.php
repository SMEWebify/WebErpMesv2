<?php

return [

    /* GRENERAL TRANS KEY */

    //STATUTS
    'awaiting_payment_trans_key'               => 'En attente de paiement',
    'open_trans_key'                           => 'Ouvert',
    'send_trans_key'                           => 'Envoyé',
    'win_trans_key'                            => 'Gagné',
    'lost_trans_key'                           => 'Perdu',
    'closed_trans_key'                         => 'Fermé',
    'obsolete_trans_key'                       => 'Obsolète',
    'started_trans_key'                        => 'Démarer',
    'planned_trans_key'                        => 'Prévue',
    'in_progress_trans_key'                    => 'En cours',
    'finished_trans_key'                       => 'Fini',
    'suspended_trans_key'                      => 'Suspendue',
    'outsourced_trans_key'                     => 'Externalisé',
    'rfq_in_progress_trans_key'                => 'Appel d\'offres en cours',
    'to_rfq_trans_key'                         => 'Vers la demande de prix',
    'finished_task_trans_key'                  => 'Terminé',
    'delivered_trans_key'                      => 'Livré',
    'partly_delivered_trans_key'               => 'Partiellement livré',
    'partly_stored_trans_key'                  => 'Partiellement stocké',
    'not_delivered_trans_key'                  => 'Non livré',
    'delivered_without_dn_trans_key'           => 'Livré sans bon de livraison',
    'chargeable_trans_key'                     => 'Facturable',
    'not_chargeable_trans_key'                 => 'Non facturable',
    'partly_invoiced_trans_key'                => 'Partiellement facturé',
    'invoiced_trans_key'                       => 'Facturé',
    'not_invoiced_trans_key'                   => 'Non facturé',
    'ordered_trans_key'                        => 'En commande',
    'partly_received_trans_key'                => 'Partiellement reçu',
    'rceived_trans_key'                        => 'Reçu',
    'po_partly_created_trans_key'              => 'CA partiellement créé',
    'po_created_trans_key'                     => 'CA créé',
    'canceled_trans_key'                       => 'Annulé',
    'order_canceled_no_document_trans_key'     => 'Commande annulée : création de BL et de facture désactivée.',
    'stopped_trans_key'                        => 'Stoppé',
    'no_task_trans_key'                        => 'Aucune taches',
    'created_trans_key'                        => 'Créer',
    'to_be_posted_trans_key'                   => 'To be posted',
    'undefined_trans_key'                      => 'Indéfinit',
    'worker_trans_key'                         => 'Travailleur',
    'employee_trans_key'                       => 'Employé',
    'self_employed_trans_key'                  => 'Travailleur indépendant',
    'male_trans_key'                           => 'Homme',
    'female_trans_key'                         => 'Femme',
    'other_trans_key'                          => 'Autre',
    'online_trans_key'                         => 'En ligne',
    'offline_trans_key'                        => 'Déconnecté',
    'active_trans_key'                         => 'Actif',
    'inactive_trans_key'                       => 'Inactif',
    'full_day_absence_trans_key'               => 'Jour complet d\'absence',
    '1_half_day_absence_trans_key'             => 'Absence matin',
    '2_half_day_absence_trans_key'             => 'Absence après-midi',
    'absence_in_hours_trans_key'               => 'Absence en heure',
    'calendar_trans_key'                       => 'Calendar',
    'workable_day_trans_key'                   => 'Journée de travail',
    'worked_day_trans_key'                     => 'Journée travaillée',
    'to_validate_trans_key'                    => 'A validée',
    'validate_trans_key'                       => 'Validée',
    'unvalidate_trans_key'                     => 'Non-validée',
    'solved_trans_key'                         => 'Résolue',
    'reopen_trans_key'                         => 'Réouvrir',
    'no_traceability_trans_key'                => 'Aucune tracabilité',
    'with_batch_number_trans_key'              => 'Avec numéro batch',
    'with_serial_number_trans_key'             => 'Avec numéro de lot',
    'sale_trans_key'                           => 'Vente',
    'purchase_stock_trans_key'                 => 'Achat (stock)',
    'advance_payment_trans_key'                => 'Acompte',
    'advance_payment_vat_trans_key'            => 'Acompte avec TVA',
    'productive_trans_key'                     => 'Productive',
    'raw_material_trans_key'                   => 'Matière première',
    'raw_material_sheet_trans_key'             => 'Matière première (Tôle)',
    'raw_material_profil_trans_key'            => 'Matière première (Profile)',
    'raw_material_block_trans_key'             => 'Matière première (Block)',
    'supplies_trans_key'                       => 'Fournitures',
    'supplied_trans_key'                       => 'Fourni',
    'sub_contracting_trans_key'                => 'Sous-traitance',
    'composed_component_trans_key'             => 'Article composé',
    'unsed_trans_key'                          => 'Utilisé',
    'used_trans_key'                           => 'Non-utilisé',
    'mass_trans_key'                           => 'Masse',
    'length_trans_key'                         => 'Longueur',
    'aera_trans_key'                           => 'Aire',
    'volume_trans_key'                         => 'Volume',
    'on_trial_trans_key'                       => 'Essais',
    'asset_trans_key'                          => 'Actif',
    'manual_stock_recep_trans_key'             => 'Réception manuelle des stocks',
    'add_manual_receipt_line_trans_key'        => 'Ajouter une ligne de réception manuelle',
    'inventories_trans_key'                    => 'Inventaires',
    'task_allocation_trans_key'                => 'Répartition des tâches',
    'purchase_order_reception_trans_key'       => 'Reception commande achat',
    'manufactured_component_entry_trans_key'   => 'Composant fabriqué saisie',
    'inter_stock_mvts_trans_key'               => 'Mouvement inter-stock',
    'manual_stock_dispatching_trans_key'       => 'Dispatching Manuel du stock',
    'reservation_cancellation_trans_key'       => 'Annulation de réservation',
    'reservation_component_production_trans_key' => 'Réservation d\'un composant en production',
    'reservation_trans_key'                    => 'Réservation',
    'part_delivery_trans_key'                  => 'Pièce livrée',
    'in_production_trans_key'                  => 'En production',
    'direct_inventory_trans_key'               => 'Inventaire direct',
    'new_trans_key'                            => 'Nouveau',
    'converted_trans_key'                      => 'Converti',
    'stop_trans_key'                           => 'Stop',
    'setup_trans_key'                          => 'Réglage',
    'run_trans_key'                            => 'Run',
    'off_trans_key'                            => 'Off',
    'internal_trans_key'                       => 'Interne',
    'external_trans_key'                       => 'Externe',
    'waiting_customer_data_trans_key'          => 'En attente de données client',
    'married_trans_key'                        => 'Marié',
    'single_trans_key'                         => 'Seul',
    'divorced_trans_key'                       => 'Divorcé',
    'widowed_trans_key'                        => 'Veuve/Veuf',
    'prospect_trans_key'                       => 'Prospect',
    'miss_trans_key'                           => 'Mlle',
    'ms_trans_key'                             => 'Ms',
    'mr_trans_key'                             => 'Mr',
    'mrs_trans_key'                            => 'Mrs',
    'burning_trans_key'                        => 'Bouillant',
    'hot_trans_key'                            => 'Chaud',
    'lukewarm_trans_key'                       => 'Tiède',
    'cold_trans_key'                           => 'Froid',
    'quote_made_trans_key'                     => 'Devis réalisé',
    'negotiation_trans_key'                    => 'Négotiation',
    'closed_won_trans_key'                     => 'Fermé-gagné',
    'closed_lost_trans_key'                    => 'Fermé-perdu',
    'informational_trans_key'                  => 'Informationnel', 
    'activity_maketing_trans_key'              => 'Activité marketing', 
    'email_send_trans_key'                     => 'Envois d\'e-mail',
    'pre_sakes_aactivity_trans_key'            => 'Avant vente',
    'sales_activity_trans_key'                 => 'Activité commercial',
    'sales_telephone_call_trans_key'           => 'Appel téléphonique',
    'no_start_trans_key'                       => 'Non commencé',
    'internal_meeting_trans_key'               => 'Réunion interne',
    'onsite_visite_trans_key'                  => 'Visite sur site',
    'sales_meeting_trans_key'                  => 'Rendez vous commercial',
    'shipped_trans_key'                        => 'Expédié',
    'returned_trans_key'                       => 'Retour',
    'in_stock_trans_key'                       => 'En stock',
    'pending_trans_key'                        => 'En attente',
    'unpaid_trans_key'                         => 'Impayé',
    'paid_trans_key'                           => 'Payé',
    'done_trans_key'                           => 'Fait',
    'to_submit_trans_key'                      => 'Soumettre',
    'submitted_trans_key'                      => 'Soumise',
    'approved_trans_key'                       => 'Approuvée',
    'rejected_trans_key'                       => 'Refusée',
    'low_trans_key'                            => 'Faible',
    'moderate_trans_key'                       => 'Modéré',
    'high_trans_key'                           => 'Élevé',
    'rare_trans_key'                           => 'Rare',
    'possible_trans_key'                       => 'Possible',
    'probable_trans_key'                       => 'Probable',
    'minor_trans_key'                          => 'Mineur',
    'major_trans_key'                          => 'Majeur',
    'critical_trans_key'                       => 'Critique',
    'expired_trans_key'                        => 'Expiré',
    'reports_trans_key'                        => 'Rapports',
    'accounting_reports_trans_key'             => 'Rapports comptables',
    'total_revenue_trans_key'                  => 'Revenus totaux',
    'total_expense_trans_key'                  => 'Dépenses totales',
    'total_profit_trans_key'                   => 'Bénéfice',
    'total_product_time_by_service'            => 'Temps total de production par service',
    'total_setting_time_by_service'            => 'Temps total de réglage par service',
    'total_cost_by_service'                    => 'Coût total par service',
    'total_price_by_service'                   => 'Prix total par service',
    'coming_soon_trans_key'                    => 'Bientôt disponible',
    'processing_location_trans_key'            => 'Emplacement de traitement',
    'select_location_trans_key'                => 'Selectionner emplacement',

    //GENERAL HEADER AND FOOTER TABLE
    'sort_trans_key'                           => 'Tri',
    'id_trans_key'                             => 'ID',
    'code_trans_key'                           => 'Référence projet',
    'label_trans_key'                          => 'Libellé',
    'customer_trans_key'                       => 'Client',
    'id_customer_trans_key'                    => 'Code client',
    'status_trans_key'                         => 'Statuts',
    'statu_trans_key'                          => 'Statut',
    'total_price_trans_key'                    => 'Prix total',
    'lines_count_trans_key'                    => 'Nombre de ligne',
    'created_at_trans_key'                     => 'Créé le',
    'assigned_trans_key'                       => 'Assigné à',
    'action_trans_key'                         => 'Actions',
    'save_trans_key'                           => 'Enregistrer',
    'cancel_trans_key'                         => 'Annuler',
    'ordre_trans_key'                          => 'Ordre',
    'saving_trans_key'                         => 'Enregistrement...',
    'no_results_trans_key'                     => 'Aucun résultat',
    'user_trans_key'                           => 'Utilisateur',
    'attendance_trans_key'                     => 'Pointage',
    'attendance_select_user_trans_key'         => 'Sélectionnez un utilisateur',
    'attendance_entry_trans_key'               => 'Entrée',
    'attendance_exit_trans_key'                => 'Sortie',
    'attendance_success_trans_key'             => 'Pointage enregistré.',
    'success_trans_key'                        => 'Opération réalisée avec succès.',
    'status_client_trans_key'                  => 'Etat client',
    'status_supplier_trans_key'                => 'Etat fournisseur',
    'color_trans_key'                          => 'Couleur',
    'picture_trans_key'                        => 'Logo',
    'ressource_trans_key'                      => 'Ressource',
    'ressources_trans_key'                     => 'Ressources',
    'section_trans_key'                        => 'Section',
    'sections_trans_key'                       => 'Sections',
    'hourly_rate_trans_key'                    => 'Taux horaire',
    'margin_trans_key'                         => 'Marge',
    'mask_time_trans_key'                      => 'Temps masqué',
    'hour_capacity_week_trans_key'             => 'Capacité d\'heure par semaine',
    'capacity_trans_key'                       => 'Capacité',
    'etat_trans_key'                           => 'Etat',
    'cost_trans_key'                           => 'Coût',
    'due_date_trans_key'                       => 'Date d\'échéance',

    'order_trans_key'                          => 'Commande',
    'external_id_trans_key'                    => 'ID externe',
    'description_trans_key'                    => 'Description',
    'price_trans_key'                          => 'Prix U',
    'customer_price_grid_trans_key'            => 'Grille tarifaire client',
    'automatic_pricing_trans_key'              => 'Tarification automatique',
    'price_source_trans_key'                   => 'Source du prix',
    'use_price_trans_key'                      => 'Utiliser ce prix',
    'no_price_defined_trans_key'               => 'Aucun tarif client défini pour ce produit.',
    'all_customers_trans_key'                  => 'Tous les clients',
    'all_customer_types_trans_key'             => 'Tous les types de clients',
    'customer_price_list_create_help_trans_key' => 'Laissez client et type vides pour un tarif général.',
    'customer_price_list_unique_error_trans_key' => 'Un tarif existe déjà pour cette combinaison de périmètre et de quantités.',
    'type_trans_key'                           => 'Type',
    'qty_trans_key'                            => 'Qt',
    'unit_trans_key'                           => 'Unité',
    'discount_trans_key'                       => 'Réduction',
    'vat_trans_key'                            => 'TVA',
    'delivery_date_trans_key'                  => 'Date de livraison',
    'delivery_note_date_from_trans_key'        => 'Date du bon de livraison du',
    'delivery_note_date_to_trans_key'          => 'Date du bon de livraison au',
    'internal_delay_trans_key'                 => 'délai interne',
    'tasks_status_trans_key'                   => 'Statut des taches',
    'delivery_status_trans_key'                => 'Statut de livraison',
    'invoice_status_trans_key'                 => 'Statut de la facture',
    'mark_not_chargeable_trans_key'            => 'Marquer non facturable',
    'confirm_not_chargeable_trans_key'         => 'Marquer cette ligne comme non facturable ? Elle ne sera plus due à facturer.',
    'line_already_invoiced_trans_key'          => 'Cette ligne est déjà facturée.',
    'order_invoiced_no_cancel_trans_key'       => 'Cette commande est facturée : elle ne peut pas être annulée. Émettez un avoir.',
    'delivered_qty_trans_key'                  => 'Qt livrée',
    'remaining_qty_trans_key'                  => 'Qt restante',
    'addressing_trans_key'                     => 'Adresage',

    'supplier_ref_trans_key'                   => 'Référence Vendeur',
    'supplier_trans_key'                       => 'Vendeur',
    'supplier_rate_trans_key'                  => 'Note vendeur',
    'order_rate_trans_key'                     => 'Note commmande',
    'qty_reciept_trans_key'                    => 'Qt reçue',
    'qty_purchase_trans_key'                   => 'Qt achetée',
    'qty_invoice_trans_key'                    => 'Qt facturée',
    'qty_accepted_trans_key'                   => 'Qt acceptée',
    'qty_canceled_trans_key'                   => 'Qt annulée',
    'qty_mini_trans_key'                       => 'Qt mini',
    'qty_total_trans_key'                      => 'Qt total',
    'scum_qty_trans_key'                       => 'Qt restante',

    'name_trans_key'                           => 'Nom',
    'type_of_contract_trans_key'               => 'Type de contra',
    'weekly_duration_trans_key'                => 'Durée hebdomadaire',

    'employment_statu_trans_key'               => 'Statut de l\'employé',
    'job_title_trans_key'                      => 'Poste',
    'role_trans_key'                           => 'Rôle', 
    'roles_trans_key'                          => 'Rôles', 
    'gender_trans_key'                         => 'Genre',  
    'born_date_trans_key'                      => 'Date de naissance',  
    'type_of_day_trans_key'                    => 'Type de jour',  
    'start_date_trans_key'                     => 'Date de début',  
    'end_date_trans_key'                       => 'Date de fin',  
    'date_time_trans_key'                      => 'Date et heure',
    'hour_trans_key'                           => 'Heure',
    'hours_trans_key'                          => 'Heures',
    'task_trans_key'                           => 'Tache',
    'nc_trans_key'                             => 'NC',
    'failure_trans_key'                        => 'Défaillance',
    'failure_comment_trans_key'                => 'Commenataire défaillance',
    'failure_type_trans_key'                   => 'Défaillance type',
    'correction_trans_key'                     => 'Correctif',
    'correction_comment_trans_key'             => 'Commenataire correctif',
    'correction_type_trans_key'                => 'Correctif type',
    'serial_number_trans_key'                  => 'Numéro de série',
    'companie_concern_trans_key'               => 'Tiers concernée',
    'city_trans_key'                           => 'Ville',
    'postal_code_trans_key'                    => 'Code postal',
    'province_trans_key'                       => 'Province',
    'country_trans_key'                        => 'Pays',
    'function_trans_key'                       => 'Fonction',
    'title_trans_key'                          => 'Titre',
    'text_trans_key'                           => 'Texte',

    'rate_trans_key'                           => 'Taux',  
    'code_account_trans_key'                   => 'Code compte', 
    'number_of_month_trans_key'                => 'Nombre de mois', 
    'number_of_day_trans_key'                  => 'Nombre de jours', 
    'end_of_month_trans_key'                   => 'Fin de mois', 
    'account_number_trans_key'                 => 'Numéro du compte', 
    'vat_account_trans_key'                    => 'TVA code compte', 

    'total_selling_trans_key'                  => 'Total prix vendu',
    'total_time_trans_key'                     => 'Temps Total',
    'unit_time_trans_key'                      => 'Temps unitaire',
    'setting_time_trans_key'                   => 'Temps de réglage',
    'fixed_trans_key'                          => 'Fixe',
    
    'progress_trans_key'                       => 'Progrès',
    'sub_total_trans_key'                      => 'Sous total',
    'total_trans_key'                          => 'Total',
    'tax_trans_key'                            => 'Taxe',
    'no_tax_trans_key'                         => 'Aucune taxe',

    'failure_mode_trans_key'                   => 'Mode de défaillance',
    'effect_trans_key'                         => 'Effet',
    'severity_trans_key'                       => 'Sévérité (1 à 10)',
    'occurrence_trans_key'                     => 'Occurrence (1 à 10)',
    'detection_trans_key'                      => 'Détection (1 à 10)',
    'current_control_trans_key'                => 'Contrôle actuel',
    'recommended_action_trans_key'             => 'Action recommandée',

    //GENERAL CONTENT TABLE
    'no_data_trans_key'                        => 'Aucune données',

    //TAB
    'opportunity_info_trans_key'               => 'Oportunité info',
    'activities_trans_key'                     => 'Activités',
    'events_trans_key'                         => 'Evenements',
    'timeline_trans_key'                       => 'TimeLine',

    'quote_info_trans_key'                     => 'Information du devis',
    'quote_line_trans_key'                     => 'Ligne du devis',
    'construction_site_trans_key'              => 'Chantier/agencement',
    'modules_trans_key'                        => 'Modules',
    'enable_trans_key'                         => 'Activer',
    'construction_site_enable_help_trans_key'  => 'Affiche l\'onglet Chantier/agencement sur les devis.',
    'guest_page_trans_key'                     => 'Page publique',
    'lines_import_trans_key'                   => 'Importation de lignes',

    'order_info_trans_key'                     => 'Information de commande',
    'order_line_trans_key'                     => 'nb lignes',
    'quote_lines_related_trans_key'            => 'Lignes de devis',
    'order_lines_related_trans_key'            => 'Lignes de commande',
    'business_Review_trans_key'                => 'Bilan d\'affaire',

    'delivery_info_trans_key'                  => 'Bon de livraison info',
    'delivery_lines_trans_key'                 => 'Lignes du bon de livraison',

    'invoice_info_trans_key'                   => 'Facture info',
    'invoice_lines_trans_key'                  => 'Lignes de facture',
    'invoice_draft_trans_key'                  => 'Brouillon',
    'invoice_draft_no_pdf_trans_key'           => 'Une facture en brouillon ne peut pas être éditée en PDF.',
    'invoice_draft_no_facturx_trans_key'       => 'Une facture en brouillon ne peut pas produire de Factur-X : elle n\'est pas encore émise.',
    'invoice_proforma_no_facturx_trans_key'    => 'Une proforma n\'est pas une facture : elle ne peut pas produire de Factur-X.',

    'purchase_quotation_trans_key'             => 'Demande d\'achat',
    'purchase_quotation_info_trans_key'        => 'Info Devis d\'achat',
    'purchase_quotation_lines_trans_key'       => 'Ligne devis d\'achat',
    'lead_time_trans_key'                      => 'Délai',
    'score_trans_key'                          => 'Score',

    'purchase_info_trans_key'                  => 'Achat info',
    'purchase_lines_trans_key'                 => 'Ligne achat',

    'purchase_receipt_trans_key'               => 'Reception achat',
    'purchase_receipt_info_trans_key'          => 'Reçu d\'achat info',
    'purchase_receipt_lines_trans_key'         => 'Ligne de reçu d\'achat',
    
    'purchase_invoice_info_trans_key'          => 'Info facture fournisseur',
    'purchase_invoice_lines_trans_key'         => 'Ligne de facture fournisseur',

    'product_info_trans_key'                    => 'Détail produit',
    'tech_bom_trans_key'                        => 'Découpage technique et nomenclature',

    'location_in_workshop_trans_key'           => 'Location dans l\'atelier',
    'units_trans_key'                          => 'Unités',
    'families_trans_key'                       => 'Famille',
    'tools_trans_key'                          => 'Outils',
    'tool_already_booked_trans_key'            => "L'outil sélectionné est déjà réservé sur ce créneau.",
    'batches_trans_key'                        => 'Lots',

    'absence_trans_key'                        => 'Absence',
    'banck_holiday_trans_key'                  => 'jour férié',
    'improduct_time_trans_key'                 => 'Temps improductif',
    'machine_event_trans_key'                  => 'Evènement machine',

    'profil_setting_trans_key'                 => 'Profil réglage',
    'notification_history_trans_key'           => 'Notification historique',
    'leave_request_trans_key'                  => 'Demande d\'absence',

    'hr_information_trans_key'                 => 'RH information',
    'contract_trans_key'                       => 'Contra',

    'derogations_trans_key'                    => 'Dérogations',
    'measuring_devices_trans_key'              => 'Appareil de mesure',
    'calibrated_at_trans_key'                  => 'Calibré le',
    'calibration_due_at_trans_key'             => 'Échéance d\'étalonnage',
    'calibration_status_trans_key'             => 'Statut d\'étalonnage',
    'calibration_provider_trans_key'           => 'Prestataire d\'étalonnage',
    'calibration_overdue_trans_key'            => 'Étalonnage en retard',
    'calibration_due_soon_trans_key'           => 'Étalonnage imminent',
    'calibration_due_soon_description_trans_key' => 'Les appareils suivants doivent être étalonnés dans :days jours.',
    'capability_index_trans_key'               => 'Indice de capabilité',
    'not_available_trans_key'                  => 'N/D',

    'factory_trans_key'                        => 'Usine',
    'factory_settings_trans_key'               => 'Réglage usine',
    'announcements_trans_key'                  => 'Annonces',
    'workflow_settings_trans_key'              => 'Workflow réglage',
    'permissions_trans_key'                    => 'Permissions',
    'permission_trans_key'                     => 'Permission',
    'role_in_permissions_trans_key'            => 'Rôles dans les permissions',
    'estimated_budget_trans_key'               => 'Objectif CA',
    'export_fec_trans_key'                     => 'Export FEC',
    'customer_import_trans_key'                => 'Import client',
    'quotes_import_trans_key'                  => 'Import devis',
    'orders_import_trans_key'                  => 'Import commande',
    'products_import_trans_key'                => 'Import produit',
    'kanban_setting_note_trans_key'            => 'La première ligne sera utilisée pour le statut par défaut des nouvelles tâches. La dernière ligne est utilisée pour la tâche terminée, prête pour le bon de livraison',
    'make_new_role_trans_key'                  => 'Créer un rôle',
    'custom_fields_trans_key'                  => 'Champs personalisés',
    'custom_fields_category_trans_key'         => 'Catégorie',
    'custom_fields_category_placeholder_trans_key' => 'ex. Technique',
    'custom_fields_placeholder_trans_key'      => 'Saisissez une valeur...',
    'custom_fields_default_category_trans_key' => 'Général',
    'custom_fields_type_select_trans_key'      => 'Liste',
    'custom_fields_options_trans_key'          => 'Options',
    'custom_fields_options_placeholder_trans_key' => 'Une option par ligne',
    'custom_fields_options_help_trans_key'     => 'Affichées comme valeurs sélectionnables dans la liste.',
    'custom_fields_select_placeholder_trans_key' => 'Sélectionnez une option',
    'document_code_templates_trans_key'        => 'Document code templates',
    'new_document_code_template_trans_key'     => 'Nouveau templates',
    'template_trans_key'                       => 'Templates',
    
    // CARD HEADER
    'statistiques_trans_key'                   => 'Statistiques',
    'informations_trans_key'                   => 'Informations',
    'options_trans_key'                        => 'Options',
    'charts_trans_key'                         => 'Graphiques',

    // BUTTON    
    'view_trans_key'                           => 'Voir',
    'show_trans_key'                           => 'Afficher',
    'search_trans_key'                         => 'Rechercher',
    'view_count_trans_key'                     => 'Nombre de vue(s)',
    'print_trans_key'                          => 'Imprimer',
    'pdf_trans_key'                            => 'PDF',
    'edit_trans_key'                           => 'Modifier',
    'delete_trans_key'                         => 'Supprimer',
    'view_all_trans_key'                       => 'Tout voir',
    'submit_trans_key'                         => 'Soumettre',
    'update_trans_key'                         => 'Mettre à jours',
    'success_update_trans_key'                 => 'Fiche mise à jour avec succès.',
    'upload_trans_key'                         => 'Télécharger',
    'refresh_trans_key'                        => 'Rafraichir la page',
    'close_trans_key'                          => 'Fermer',
    'yes_trans_key'                            => 'YES',
    'no_trans_key'                             => 'NO',
    'add_trans_key'                            => 'Ajout',
    'new_receipt_document_trans_key'           => 'Nouveau reçu',
    'new_empty_receipt_document_trans_key'     => 'Nouveau reçu vierge',
    'new_invoice_document_trans_key'           => 'Nouvelle facture',
    'new_purchase_document_trans_key'          => 'Nouvelle commande d\'achat',
    'export_csv_trans_key'                     => 'Exporter CSV',
    'sheet_metal_global_need_csv_trans_key'    => 'CSV pour estimation du besoin de tôle globale',
    'sheet_metal_global_need_csv_hint_trans_key' => 'Exporter un CSV pour estimer le besoin global en tôle.',
    'open_orders_not_started_trans_key'        => 'Commandes ouvertes / tâches non commencées',
    'open_orders_not_started_hint_trans_key'   => 'Exporter les lignes des commandes ouvertes avec tâches non commencées.',

    //SELECT
    'select_trans_key'                         => 'Selectionner',
    'select_vat_trans_key'                     => 'Selectionner TVA',
    'select_product_trans_key'                 => 'Selectionner produit',
    'user_management_trans_key'                => 'Utilisateur responsable',
    'select_user_trans_key'                    => 'Sélectionner un utilisateur',
    'select_user_management_trans_key'         => 'Selectionner l\'utilisateur',
    'select_company_trans_key'                 => 'Selectionner tiers',
    'select_suppliers_trans_key'               => 'Sélectionner des fournisseurs',
    'select_address_trans_key'                 => 'Selectionner adresse',
    'select_contact_trans_key'                 => 'Selectionner contact',
    'select_payement_condition_trans_key'      => 'Selectionner condition de paiement',
    'select_payement_methods_trans_key'        => 'Selectionner méthode de paiement',
    'select_delivery_trans_key'                => 'Selectionner mode de livraison',
    'select_service_trans_key'                 => 'Selectionner service',
    'select_tool_trans_key'                    => 'Sélectionner outil',
    'select_ressource_trans_key'               => 'Selectionner ressource',
    'select_family_trans_key'                  => 'Selectionner famille',
    'select_unit_trans_key'                    => 'Selectionner unité',
    'select_statu_trans_key'                   => 'Selectionner statut',
    'select_type_trans_key'                    => 'Selectionner type',
    'select_control_type_trans_key'            => 'Selectionner type de controle',
    'select_task_type_trans_key'               => 'Selectionner type de tâche',
    'select_component_trans_key'               => 'Selectionner composant',
    'select_year_trans_key'                    => 'Selectionner année',
    'select_gender_trans_key'                  => 'Selectionner genre',
    'select_marital_status_trans_key'          => 'Selectionner l\'état civil',
    'select_status_trans_key'                  => 'Selectionner statuts',
    'select_controle_trans_key'                => 'Selectionner type de contrôle',
    'select_document_trans_key'                => 'Selectionner le document',
    'select_task_trans_key'                    => 'Selectionner la tâche',
    'select_order_trans_key'                   => 'Selectionner la commande',
    'select_order_line_trans_key'              => 'Selectionner une ligne de commande',
    'select_purchase_order_trans_key'          => 'Selectionner la commande achat',
    'select_categorie_trans_key'               => 'Selectionner la categorie',
    'reception_control_trans_key'              => 'Controle de reception',

    'no_select_company_trans_key'              => 'Aucun tiers, ajouter un tiers',
    'no_address_trans_key'                     => 'Aucune adresse, ajouter une adresse',
    'no_contact_trans_key'                     => 'Aucun contact, ajouter un contact',
    'no_payment_conditions_trans_key'          => 'Aucune condition de paiement, veuillez ajouter la page de comptabilité',
    'no_payment_methods_trans_key'             => 'Aucune méthode de paiement, veuillez ajouter la page de comptabilité',
    'no_delivery_trans_key'                    => 'Aucun mode de livraison, veuillez ajouter la page de comptabilité',
    'no_service_trans_key'                     => 'Aucun service, veuillez ajouter la page de méthode',
    'no_family_trans_key'                      => 'Aucune famille, veuillez ajouter la page de méthode',
    'no_unit_trans_key'                        => 'Aucune unité, veuillez ajouter la page de méthode',
    'no_vat_trans_key'                         => 'Aucune TVA, veuillez ajouter une TVA',
    'no_section_trans_key'                     => 'Aucune section, veuillez ajouter une section',
    'no_role_trans_key'                        => 'Aucun role, veuillez ajouter un role',
    'no_permissions_trans_key'                 => 'Aucune permission, veuillez ajouter une permission',
    'no_stock_location_trans_key'              => 'Aucune location de stock',

    //REVIEW & CHANGE
    'review_change_tracking_trans_key'         => 'Suivi des revues et changements',
    'review_timeline_trans_key'                => 'Historique de revue',
    'reviewed_by_trans_key'                    => 'Revu par',
    'review_date_trans_key'                    => 'Date de revue',
    'decision_trans_key'                       => 'Décision',
    'change_requested_by_trans_key'            => 'Changement demandé par',
    'change_reason_trans_key'                  => 'Motif du changement',
    'change_approved_at_trans_key'             => 'Changement approuvé le',
    'changes_trans_key'                        => 'Modifications',

    //LINK
    'back_trans_key'                           => 'Retour',
    'back_to_list_trans_key'                   => 'Retour à la list',
    'previous_trans_key'                       => 'Précédent',
    'next_trans_key'                           => 'Suivant',
    'copie_line_trans_key'                     => 'Copier la ligne',
    'edit_line_trans_key'                      => 'Editer la ligne',
    'delete_line_trans_key'                    => 'Effacer la ligne',
    'break_down_task_trans_key'                => 'Décomposer les taches de l\'article',
    'create_product_trans_key'                 => 'Créer article à partir de cette ligne',
    'create_products_from_selected_lines_trans_key'    => 'Créer les articles des lignes sélectionnées',
    'create_products_from_selected_lines_success_trans_key' => 'Articles créés pour les lignes sélectionnées',

    //LABEL
    'customer_info_trans_key'                  => 'Information client',
    'supplier_info_trans_key'                  => 'Information fournisseur',
    'date_pay_info_trans_key'                  => 'Date & paiement information',
    'documents_trans_key'                      => 'Documents',
    'file_size_trans_key'                      => 'Taille du fichier',
    'uploaded_at_trans_key'                    => 'Ajouté le',
    'updated_at_trans_key'                     => 'Mis à jour le',
    'uploaded_by_trans_key'                    => 'Ajouté par',
    'related_entities_trans_key'               => 'Lié à',
    'document_search_placeholder_trans_key'    => 'Rechercher par nom, fichier original ou commentaire',
    'hashtag_filter_placeholder_trans_key'     => 'Filtrer par #tag',
    'reset_filters_trans_key'                  => 'Réinitialiser les filtres',
    'no_documents_found_trans_key'             => 'Aucun document ne correspond à vos filtres',
    'cgv_trans_key'                            => 'Conditions générale de ventes',
    'photos_trans_key'                         => 'Photos',
    'choose_file_trans_key'                    => 'Choisir fichier',
    'picture_file_trans_key'                   => 'Fichier image',
    'drawing_trans_key'                        => 'Mise en plan',
    'stl_file_trans_key'                       => 'Fichier STL',
    'svg_file_trans_key'                       => 'Fichier SVG',
    'viewer_file_trans_key'                    => 'Visioneuse',
    'attached_file_trans_key'                  => 'Fichier(s) attaché(s)',
    'take_photo_trans_key'                     => 'Télécharger la photo',
    'public_link_trans_key'                    => 'Lien publique sécurisé',
    'copy_trans_key'                           => 'Copier',
    'add_to_document_trans_key'                => 'Ajouter au nouveau document',
    'preferred_supplier_trans_key'             => 'Fournisseurs préférés',

    'customer_type_trans_key'                  => 'Type de client',
    'individual_trans_key'                     => 'Particulier',
    'legal_entity_trans_key'                   => 'Entité juridique',
    'companie_name_trans_key'                  => 'Nom du tiers',
    'adress_name_trans_key'                    => 'Nom de l\'adresse',
    'adress_trans_key'                         => 'Adresse',
    'contact_name_trans_key'                   => 'Nom du contact',
    'contact_trans_key'                        => 'Contact',
    'contacts_trans_key'                       => 'Contacts',
    'first_name_trans_key'                     => 'Prénom',
    'civility_trans_key'                       => 'Civilité',
    'email_trans_key'                          => 'E-mail',
    'date_trans_key'                           => 'Date',
    'message_trans_key'                        => 'Message',
    'object_trans_key'                         => 'Objet',
    'comment_trans_key'                        => 'Commentaire',
    'hashtags_trans_key'                       => 'Hashtags',
    'internal_comment_trans_key'               => 'Commentaire interne',
    'external_comment_trans_key'               => 'Commentaire externe',
    'identifier_trans_key'                     => 'Référence',
    'payment_method_trans_key'                 => 'Mode de paiement',
    'payment_methods_trans_key'                => 'Méthode de paiement',
    'payment_conditions_trans_key'             => 'Condition de paiement',
    'delevery_method_trans_key'                => 'Mode de livraison',
    'validity_date_trans_key'                  => 'Date de validité',
    'longitude_trans_key'                      => 'Longitude',
    'latitude_trans_key'                       => 'Latitude',
    'delivery_constraint_trans_key'            => 'Contrainte de livraison',
    'no_constraints_trans_key'                 => 'Sans contrainte',
    'no_tolerance_trans_key'                   => 'Sans tolérance',
    'tolerance_in_days_trans_key'              => 'Tolérance en jours',
    'tolerance_days_trans_key'                 => 'Nombre de jours de tolérance',
    'quoted_delivery_note_trans_key'           => 'Note de livraison chiffré',
    'order_average_note_trans_key'             => 'Commande moyenne',
    'customer_processing_cost_trans_key'       => 'Coût de traitement client',

    'material_trans_key'                       => 'Matière',
    'thickness_trans_key'                      => 'Epaisseur',
    'finishing_trans_key'                      => 'Finition',
    'weight_trans_key'                         => 'Poids net',
    'gross_weight_trans_key'                   => 'Poids brut',
    'total_weight_trans_key'                   => 'Poids total',
    'bend_count_trans_key'                     => 'Nombre de plis',
    'x_size_trans_key'                         => 'Taille X',
    'y_size_trans_key'                         => 'Taille Y',
    'z_size_trans_key'                         => 'Taille Z',
    'x_oversize_trans_key'                     => 'Dépassement X',
    'y_oversize_trans_key'                     => 'Dépassement Y',
    'z_oversize_trans_key'                     => 'Dépassement Z',
    'diameter_trans_key'                       => 'Diamètre',
    'diameter_oversize_trans_key'              => 'Dépassement diamètre',
    'material_loss_rate_trans_key'             => 'Taux de perte de matière',
    'section_size_trans_key'                   => 'Taille des sections',

    'delivery_note_number_trans_key'           => 'Nom du bon de livraison',
    'delivery_simulation_trans_key'            => 'Simulation de délai de livraison',
    'reference_trans_key'                      => 'Référence',
    'customer_reference_trans_key'             => 'Référence client',

    'absence_type_day_trans_key'               => 'Type de jour d\'absence',
    'absence_type_trans_key'                   => 'Type d\'absence',
    'year_trans_key'                           => 'Année',
    'days_trans_key'                           => 'jours',
    'amount_trans_key'                         => 'Montant',

    'by_default_trans_key'                     => 'Par défaut',
    'document_type_trans_key'                  => 'Type de document',
    'add_export_trans_key'                     => 'Ajouter à l\'export',
    'show_generic_task_trans_key'              => 'Afficher les tâches générique',

    'auxiliary_account_trans_key'              => 'Compte auxiliaire',
    'general_account_trans_key'                => 'Compte général',
    'client_information_trans_key'             => 'Information client',
    'administrative_information_trans_key'     => 'Information Administrative',
    'site_link_trans_key'                      => 'Lien sites',
    'web_link_trans_key'                       => 'Site web',
    'reg_number_trans_key'                     => 'CRN',
    'naf_code_trans_key'                       => 'CODE NAF',
    'vat_number_trans_key'                     => 'Numéro TVA',
    'siren_trans_key'                          => 'Siren',
    'nat_regis_number_trans_key'               => 'Numéro de registre national',
    'share_capital_trans_key'                  => 'Partage le capital',
    'iban_trans_key'                           => 'IBAN',
    'bic_trans_key'                            => 'BIC / SWIFT',
    'custom_trans_key'                         => 'Custom',
    'custom_section_trans_key'                 => 'Custom section',
    'personnal_mail_trans_key'                 => 'E-mail personnel',
    'personnal_phone_trans_key'                => 'Numéro de téléphone personnel',
    'phone_trans_key'                          => 'Numéro de téléphone',
    'mobile_phone_trans_key'                   => 'Numéro de téléphone mobile',
    'nationality_trans_key'                    => 'Nationalité',
    'user_location_trans_key'                  => 'Emplacement',
    'termination_date_trans_key'               => 'Date de résiliation',
    'supervisor_trans_key'                     => 'Superviseur',
    'section_concern_trans_key'                => 'Section concernée',
    'signature_date_trans_key'                 => 'Date de signature',
    'duration_trial_trans_key'                 => 'Durée de la période d\'essai',
    'position_trans_key'                       => 'Position',
    'coefficient_trans_key'                    => 'Coefficient',
    'hourly_gross_salary_trans_key'            => 'Salaire brut horaire',
    'minimum_monthly_salary_trans_key'         => 'Salaire mensuel minimum',
    'annual_gross_salary_trans_key'            => 'Salaire brut annuel',
    'driving_license_trans_key'                => 'Permis de conduire',
    'driving_license_exp_date_trans_key'       => 'Date d\'expiration du permis de conduire',
    'ssn_num_trans_key'                        => 'Numéro SSN',
    'nic_num_trans_key'                        => 'NIC Numéro', 
    'move_Type_trans_key'                      => 'Type de mouvement',
    'source_trans_key'                         => 'Source',
    'priority_trans_key'                       => 'Priorité',
    'campaign_trans_key'                       => 'Campagne',
    'part_trans_key'                           => 'pièce(s)',
    'curency_trans_key'                        => 'Devise',
    'repots_setting_trans_key'                 => 'Paramètre de rapports',
    'visa_trans_key'                           => 'Visa',
    'select_to_export_trans_key'               => 'Sélectionnez pour exporter',
    'line_trans_key'                           => 'Ligne',
    'blocked_unti_trans_key'                   => 'Bloqué jusqu\'au',

    'problem_description_trans_key'            => 'Description du problème',
    'proposal_description_trans_key'           => 'Description de la proposition',
    'customer_reply_description_trans_key'     => 'Description de la réponse du client',
    'decision_description_trans_key'           => 'Description de la décision',
    'cause_description_trans_key'              => 'Cause description',
    'action_description_trans_key'             => 'Action description',
    
    'collapse_trans_key'                       => 'Réduire',
    'remove_trans_key'                         => 'Retirer',

    'probality_trans_key'                      => 'Probabilité',
    'budget_trans_key'                         => 'Budget',

    'loaded_date_trans_key'                    => 'Loaded Date',
    'packing_date_trans_key'                   => 'Packing Date',

    //NOTIFICATION
    'unread_list_trans_key'                    => ' Liste non lus',
    'read_trans_key'                           => ' Lus',
    'all_read_trans_key'                       => ' Tout vu',
    'notif_tab_unread_trans_key'               => 'Non lues',
    'notif_tab_history_trans_key'              => 'Historique',
    'notif_read_success_trans_key'             => 'Marqué comme lu',
    'notif_all_read_success_trans_key'         => 'Toutes les notifications marquées comme lues',
    'notif_load_more_trans_key'                => 'Charger plus',
    'notif_loading_trans_key'                  => 'Chargement…',
    'notif_history_empty_trans_key'            => 'Aucune notification',
    'notif_badge_read_trans_key'               => 'Lu',
    'notif_badge_unread_trans_key'             => 'Nouveau',
    'notification_type_trans_key'              => 'Type',

    /* SPECIFIC TRANS KEY */

    //DASHBOARD
    'dashboard_trans_key'                      => 'Tableau de bord',
    'client_trans_key'                         => 'Clients',
    'new_client_trans_key'                     => 'Nouveaux client',
    'since_time_trans_key'                     => 'Depuis :time',
    'suppliers_trans_key'                      => 'Fournisseurs',
    'supplier_evaluations_trans_key'           => 'Évaluations fournisseurs',
    'new_supplier_evaluation_trans_key'        => 'Nouvelle évaluation fournisseur',
    'latest_supplier_evaluation_trans_key'     => 'Dernière évaluation fournisseur',
    'supplier_evaluation_history_trans_key'    => 'Historique des évaluations fournisseur',
    'no_purchase_order_to_evaluate_trans_key'  => 'Aucune commande d\'achat disponible pour évaluer ce fournisseur.',
    'no_supplier_evaluation_trans_key'         => 'Aucune évaluation enregistrée pour ce fournisseur.',
    'no_supplier_evaluation_history_trans_key' => 'Aucun historique d\'évaluation fournisseur disponible.',
    'evaluation_quality_score_trans_key'       => 'Note qualité',
    'evaluation_logistics_score_trans_key'     => 'Note logistique',
    'evaluation_service_score_trans_key'       => 'Note service',
    'evaluation_composite_score_trans_key'     => 'Score composite',
    'evaluation_average_composite_score_trans_key' => 'Score composite moyen',
    'evaluation_average_rating_trans_key'      => 'Note moyenne',
    'evaluation_rating_trans_key'              => 'Note',
    'evaluation_status_trans_key'              => 'Statut',
    'evaluation_last_updated_trans_key'        => 'Dernière mise à jour',
    'evaluation_next_review_at_trans_key'      => 'Prochaine revue le',
    'evaluation_approved_at_trans_key'         => 'Approuvé le',
    'evaluation_action_plan_trans_key'         => 'Plan d\'action',
    'evaluation_status_pending_trans_key'      => 'En attente',
    'evaluation_status_approved_trans_key'     => 'Approuvé',
    'evaluation_status_under_review_trans_key' => 'En cours de revue',
    'evaluation_status_rejected_trans_key'     => 'Rejeté',
    'supplier_requalification_alert_trans_key' => 'Alerte de requalification fournisseur',
    'supplier_requalification_soon_trans_key'  => 'Requalification fournisseur à venir',
    'supplier_requalification_overdue_trans_key' => 'Le fournisseur doit être requalifié depuis :days jour(s).',
    'supplier_next_review_days_trans_key'      => 'Prochaine revue prévue dans :days jour(s).',
    'suppliers_client_trans_key'               => 'Client / Fournisseurs',
    'view_details_trans_key'                   => 'Voir détail',
    'monthly_recap_report_trans_key'           => 'Rapport récapitulatif mensuel',
    'monthly_recap_task_trans_key'             => 'Récapitulation de Tâche',
    'chart_months_trans_key'                   => ['Janvier', 'Février', 'Mars', 'Avril', 'Mai', 'Juin', 'Juillet', 'Août', 'Septembre', 'Octobre', 'Novembre', 'Décembre'],
    'chart_order_forecast_trans_key'           => 'Prévisions de commandes',
    'chart_delivered_revenues_trans_key'       => 'Revenus livrés',
    'chart_invoiced_revenues_trans_key'        => 'Revenus facturés',
    'chart_purchase_revenues_trans_key'        => 'Revenus achats',
    'chart_order_targets_trans_key'            => 'Objectifs de commandes',
    'sales_period_trans_key'                   => 'Ventes: 1 Jan, :YEAR - 30 Dec, :YEAR',
    'incoming_orders_trans_key'                => 'Commandes à venir',
    'no_coming_orders_trans_key'               => 'Aucune.',
    'late_orders_trans_key'                    => 'Comandes en retards',
    'no_late_orders_trans_key'                 => 'Aucune.',
    'goal_task_trans_key'                      => 'Achèvement des tâches',
    'no_dash_task_trans_key'                   => 'Aucun service, accédez aux méthodes d\'ajout.',
    'total_order_forcasted_trans_key'          => 'COMMANDE TOTALE PRÉVUE',
    'total_order_delivered_trans_key'          => 'COMMANDE TOTALE LIVREE',
    'total_invoiced_trans_key'                 => 'TOTAL FACTURÉ',
    'goal_trans_key'                           => 'RÉALISATION DES OBJECTIFS',
    'quote_transformation_trans_key'           => 'Taux de transformation des devis',
    'latest_quotes_trans_key'                  => 'Derniers devis',
    'no_quote_trans_key'                       => 'Pas de devis, rendez-vous sur la page de devis pour ajouter un devis',
    'latest_orders_trans_key'                  => 'Dernières commande',
    'no_order_trans_key'                       => 'Pas de commande, allez sur la page de commande pour ajouter une commande',
    'latest_products_trans_key'                => 'Articles récemment ajoutés',
    'no_product_trans_key'                     => 'Aucun produit, accédez à la page du produit pour ajouter un article',
    'announcement_trans_key'                   => 'Annonces',
    'order_to_be_delivered_trans_key'          => 'Commandes à livrée(s)',
    'order_delivered_trans_key'                => 'Commandes livrée(s)',
    'order_to_be_invoiced_trans_key'           => 'Commandes à facturée(s)',
    'order_invoiced_trans_key'                 => 'Commandes facturée(s)',
    'order_waiting_trans_key'                  => 'Commandes en attente(s)',
    'delivered_month_in_progress_trans_key'    => 'Total livré du mois',
    'remaining_month_trans_key'                => 'Total restant à livrer',
    'remaining_invoice_month_trans_key'        => 'Total restant à facturer',
    'pending_orders_trans_key'                => 'Commandes en cours',
    'forecast_next_three_months_trans_key'     => 'Prévision pour les 3 prochains mois',
    'service_rate_trans_key'                   => 'Taux de service',
    'average_order_processing_time_trans_key'  => 'Temps moyen de traitement',
    'order_processing_time_trans_key'          => 'Temps de traitement',
    'niko_niko_team_trans_key'                 => 'Niko Niko - Humeur de l\'équipe aujourd\'hui',
    'no_niko_niko_team_trans_key'              => 'Aucune humeur aujourd\'hui',
    'mood_tracker_trans_key'                   => 'Niko Niko',
    'my_mood_trans_key'                        => 'Mon humeur aujourd\'hui',
    'team_mood_trans_key'                      => 'Équipe',
    'no_mood_yet_trans_key'                    => 'Aucun vote aujourd\'hui',
    'mood_happy_trans_key'                     => 'Bien',
    'mood_neutral_trans_key'                   => 'Moyen',
    'mood_sad_trans_key'                       => 'Difficile',
    'average_quote_amount'                     => 'Montant moyen par devis',
    'quote_conversion_rate'                    => 'Taux de conversion des devis',
    'quote_response_rate'                      => 'Taux de réponse des devis',

    //COMPANIES
    'companies_trans_key'                      => 'Tiers',
    'companie_trans_key'                       => 'Tier',
    'sort_companie_trans_key'                  => 'Trie Tiers',
    'companies_list_trans_key'                 => 'Liste des Tiers',
    'latest_companies_trans_key'               => 'Tiers récents',
    'new_companie_trans_key'                   => 'Ajouter un tiers',
    'name_company_trans_key'                   => 'Nom du tiers',
    'detail_trans_key'                         => 'Détail',
    'general_information_trans_key'            => 'Informations générale',
    'new_address_trans_key'                    => 'Nouvelle adresse',
    'new_contact_trans_key'                    => 'Nouveau contact',

    //LEADS
    'leads_trans_key'                          => 'Leads',
    'lead_trans_key'                           => 'Lead',
    'new_leads_trans_key'                      => 'Nouveau Lead',
    'lead_count_trans_key'                     => 'Nombre de leads',
    'list_leads_trans_key'                     => 'Liste des Lead',

    //OPPORTUNITIES
    'opportunities_trans_key'                  => 'Oportunités',
    'new_opportunities_trans_key'              => 'Nouvelle Oportunité',
    'name_opportunity_trans_key'               => 'Nom de l\'oportunité',
    'opportunity_trans_key'                    => 'Oportunité',
    'new_activity_trans_key'                   => 'Nouvelle activité',
    'new_event_trans_key'                      => 'Nouveau événement',
    'opportunities_list_trans_key'             => 'Liste des opportunités',
    'opportunities_count_trans_key'            => 'Nombre d\'opp',
    'total_amount_won_trans_key'               => 'Montant total gagné',
    'total_amount_lost_trans_key'              => 'Montant total perdu',
    'opportunities_by_company_trans_key'       => 'Opportunités par entreprise',
    'opportunities_by_probability_trans_key'   => 'Opportunités par probabilité et montant',

    //QUOTES
    'quote_trans_key'                          => 'Devis',
    'quotes_list_trans_key'                    => 'Liste des devis',
    'quotes_lines_list_trans_key'              => 'Lignes de devis',
    'new_quote_trans_key'                      => 'Nouveau devis',
    'select_all_lines_trans_key'               => 'Tout sélectionner',
    'deselect_all_lines_trans_key'             => 'Tout désélectionner',

    'price_increase_amount_trans_key'          => 'Montant à ajouter',
    'apply_price_increase_to_lines_trans_key'  => 'Ajouter montant aux lignes',
    'price_increase_applied_trans_key'         => 'Majoration appliquée à :count ligne(s).',
    'header_line_ask_trans_key'                => 'En-tête ?',
    'quote_not_open_trans_key'                 => 'Devis actuellement non ouvert',
    'name_quote_trans_key'                     => 'Nom du devis',
    'update_detail_information_for_trans_key'  => 'Mettre à jour les détails pour :label',
    'task_detail_for_trans_key'                => 'Détail de la tâche pour :label',
    'main_features_trans_key'                  => 'Caractéristiques principales',
    'dimensions_xyz_trans_key'                 => 'Dimensions (X, Y, Z)',
    'circular_specs_trans_key'                 => 'Spécifications circulaires',
    'files_trans_key'                          => 'Fichiers',
    'cad_file_trans_key'                       => 'Fichier CAO',
    'cad_file_name_trans_key'                  => 'Nom du fichier CAO',
    'cam_file_trans_key'                       => 'Fichier FAO',
    'cam_file_name_trans_key'                  => 'Nom du fichier FAO',
    'cad_file_path_trans_key'                  => 'Chemin du fichier CAO',
    'cam_file_path_trans_key'                  => 'Chemin du fichier FAO',
    'custom_requirements_trans_key'            => 'Exigences personnalisées',
    'no_custom_requirement_trans_key'          => 'Aucune exigence personnalisée ajoutée.',
    'add_requirement_trans_key'                => 'Ajouter une exigence',
    'value_trans_key'                          => 'Valeur',
    'set_csv_col_trans_key'                    => 'Choisir numéro de colonne CSV',
    'choose_csv_trans_key'                     => 'Choisir un fichier csv ...',
    'csv_quote_info_trans_key'                 => 'La valeur unitaire est définie dans la section méthodes et la valeur de la TVA par défaut est définie dans la section comptabilité. S\'il n\'y a pas de colonne de remise, la valeur par défaut sera 0 %.', 
    'client_requirement_trans_key'             => 'Exigences du client',
    'show_client_requirements_on_pdf_trans_key'=> 'Afficher les exigences du client sur le PDF',
    'layout_plan_trans_key'                    => 'Plan de mise en page',
    'show_layout_on_pdf_trans_key'             => 'Afficher le plan de mise en page sur le PDF',
    'materials_finishes_trans_key'             => 'Matériaux et Finitions',
    'show_materials_on_pdf_trans_key'          => 'Afficher les matériaux et finitions sur le PDF',
    'logistics_trans_key'                      => 'Logistique',
    'logistics_cost_trans_key'                 => 'Coût de la logistique',
    'show_logistics_on_pdf_trans_key'          => 'Afficher la logistique sur le PDF',
    'coordination_with_contractors_trans_key'  => 'Coordination avec les entrepreneurs',
    'contractors_cost_trans_key'               => 'Coût des entrepreneurs',
    'show_contractors_on_pdf_trans_key'        => 'Afficher la coordination des entrepreneurs sur le PDF',
    'waste_management_trans_key'               => 'Gestion des déchets',
    'waste_management_cost_trans_key'          => 'Coût de la gestion des déchets',
    'show_waste_on_pdf_trans_key'              => 'Afficher la gestion des déchets sur le PDF',
    'taxes_and_fees_trans_key'                 => 'Taxes et Frais Additionnels',
    'taxes_cost_trans_key'                     => 'Coût des taxes',
    'show_taxes_on_pdf_trans_key'              => 'Afficher les taxes sur le PDF',
    'work_start_date_trans_key'                => 'Date de début des travaux',
    'work_end_date_trans_key'                  => 'Date de fin des travaux',
    'contingency_days_trans_key'               => 'Marge de manœuvre (jours)',

    //ORDERS
    'orders_trans_key'                         => 'Commandes',
    'orders_list_trans_key'                    => 'Commandes',
    'orders_lines_list_trans_key'              => 'Lignes de commande',
    'order_confirm_trans_key'                  => 'Confirmation de commande',
    // ARC — accusé de réception de commande
    'order_confirmations_trans_key'            => 'Confirmations de commande',
    'revision_trans_key'                       => 'Indice',
    'lines_trans_key'                          => 'Lignes',
    'delivery_method_trans_key'                => 'Mode de livraison',
    'tracking_trans_key'                       => 'Suivi',
    'arc_status_in_progress_trans_key'         => 'En cours',
    'arc_status_sent_trans_key'                => 'Envoyé',
    'arc_status_accepted_trans_key'            => 'Accepté',
    'arc_status_superseded_trans_key'          => 'Remplacé',
    'arc_current_trans_key'                    => 'En vigueur',
    'arc_sent_trans_key'                       => 'Confirmation de commande envoyée.',
    'arc_issue_trans_key'                      => 'Émettre l\'ARC',
    'arc_created_trans_key'                    => 'ARC constitué depuis la commande.',
    'arc_draft_no_pdf_trans_key'               => 'Un ARC en cours ne peut pas être imprimé.',
    'arc_differences_trans_key'                => 'Écarts',
    'arc_revisions_trans_key'                  => 'Indices',
    'arc_change_trans_key'                     => 'Modification',
    'arc_no_difference_trans_key'              => 'La commande est conforme à cet ARC.',
    'arc_diff_only_current_trans_key'          => 'Les écarts ne sont calculés que sur l\'indice en vigueur.',
    'arc_diverged_warning_trans_key'           => 'La commande a été modifiée depuis l\'ARC indice :revision. Un avenant est à émettre.',
    'arc_line_added_trans_key'                 => 'Ajoutée',
    'arc_line_removed_trans_key'               => 'Supprimée',
    'arc_line_modified_trans_key'              => 'Modifiée',
    'internal_order_trans_key'                 => 'Commande interne',
    'new_order_trans_key'                      => 'Nouvelle commande',
    'pre_orders_trans_key'                    => 'Pré-commandes',
    'new_return_trans_key'                     => 'Nouveau retour',
    'new_pre_order_trans_key'                  => 'Nouvelle pré-commande',
    'name_order_trans_key'                     => 'Nom de la commande',
    'historical_trans_key'                     => 'Historique',
    'order_create_from_trans_key'              => 'Commande créée à partir de ',
    'mnaufacturing_instruction_trans_key'      => 'Instruction de fabrication',
    'order_type_trans_key'                     => 'Type de commande',
    'customer_type_order_trans_key'            => 'Commande client',
    'internal_type_order_trans_key'            => 'Commande interne',
    'forecast_margin_trans_key'                => 'Marge prévisionnelle',
    'current_margin_trans_key'                 => 'Marge courante',
    'manufacturing_range_trans_key'            => 'Gamme',
    'accomplished_trans_key'                   => 'Réalisé',
    'gap_trans_key'                            => 'Ecart',
    'orders_to_deliver_today_trans_key'        => 'Commandes à livrer aujourd\'hui',
    'delivery_scheduled_today_trans_key'       => 'Livraison prévue aujourd\'hui',
    'no_orders_scheduled_today_trans_key'      => 'Aucune commande prévue aujourd\'hui.',
    'ready_orders_trans_key'                   => 'Commandes prêtes',
    'no_ready_orders_trans_key'                => 'Aucune commande prête.',
    'days_late_trans_key'                      => ':days jr de retard',
    'to_deliver_today_trans_key'               => 'À livrer aujourd’hui',
    'delivery_in_days_trans_key'               => 'Livraison dans :days jr',
    'requested_delivery_date_trans_key'        => 'Date de livraison souhaitée',
    'run_simulation_trans_key'                 => 'Lancer la simulation',
    'simulation_possible_trans_key'            => 'Faisable avec la charge actuelle',
    'simulation_not_possible_trans_key'        => 'Non faisable avec la charge actuelle',
    'earliest_possible_date_trans_key'         => 'Date au plus tôt',
    'simulation_missing_capacity_trans_key'    => 'Manque de capacité (heures)',
    'simulation_no_tasks_trans_key'            => 'Aucune tâche de devis pour lancer la simulation.',
    'simulation_invalid_date_trans_key'        => 'La date souhaitée doit être supérieure ou égale à aujourd\'hui.',
    'simulation_results_trans_key'             => 'Résultat de la simulation',
    'simulation_required_hours_trans_key'      => 'Temps théorique (heures)',
    'simulation_service_trans_key'             => 'Service',
    'simulation_remaining_hours_trans_key'     => 'Heures manquantes',
    'simulation_capacity_per_day_trans_key'    => 'Capacité journalière utilisée',
    
    'info_statu_trans_key'                     => 'Le statut du document ne permet pas d\'ajouter/modifier/supprimer des lignes.',

    'scheduling_trans_key'                     => 'Scheduling',
    'order_calendar_trans_key'                 => 'Calendrier des commandes',
    
    //LOAD PLANNING
    'load_planning_trans_key'                  => 'Planning de charge',
    'load_planning_info_1_trans_key'           => 'Actuellement, la charge journalière est bloquée à 16 heures.',
    'display_hours_diff_trans_key'             => 'Différence de temps',

    //NESTING
    'nesting_trans_key'                        => 'Nesting',

    //WORKFLOW
    'workflow_trans_key'                       => 'Workflow',
    'workflow_info_1_trans_key'                => 'Les vues sont paramétrées dans la page -Votre entreprise-.',

    //GANTT
    'gantt_trans_key'                          => 'Gantt',
    'gantt_info_1_trans_key'                   => 'Nombre de date de tâches à recalculer',
    'gantt_info_2_trans_key'                   => 'Nombre de ressources de tâches à recalculer',

    //TASK
    'tasks_trans_key'                          => 'Tâches',
    'tasks_list_trans_key'                     => 'Liste des tâches',
    'task_type_trans_key'                      => 'Type de tâche',
    'tasks_statu_trans_key'                    => 'Management des tâches',
    'tasks_calendar_trans_key'                 => 'Calendrier des tâches',
    'add_generic_task_trans_key'               => 'Ajouter une tache générique',
    'tasks_info_1_trans_key'                   => 'Vous pouvez ajouter une tâche générique qui n\'est pas liée à une commande. Celle-ci ne sont pas affichés dans le workflow.',
    'no_call_task_trans_key'                   => 'Aucun appel de tâche',
    'quote_task_trans_key'                     => 'Il s\'agit d\'une tâche de devis, aucune action possible.',
    'task_detail_trans_key'                    => 'Détail de la tâche',
    'logs_activity_trans_key'                  => 'Journal d\'activité',
    'no_activity_trans_key'                    => 'Aucune activité.',
    'unknown_activity_trans_key'               => 'Activité inconnue',
    'set_to_start_trans_key'                   => 'Tâche démarée',
    'set_to_end_trans_key'                     => 'Tâche arrêtée',
    'set_to_finish_trans_key'                  => 'Tâche finie',
    'declare_finish_trans_key'                 => 'Déclarer une qt finie',
    'declare_rejected_trans_key'               => 'Déclarer une qt rejeté',
    'quantity_rejected_trans_key'              => 'Quantité rejetée',
    'good_rejected_trans_key'                  => 'Quantité acceptée',
    'play_trans_key'                           => 'Play',
    'pause_trans_key'                          => 'Pause',
    'end_trans_key'                            => 'Fin',
    'finish_part_qty_trans_key'                => 'Nombre de pièce finie',
    'bad_part_qty_trans_key'                   => 'Nombre de pièce mauvaise',
    'net_production_qty_trans_key'             => 'Production net',
    'search_task_trans_key'                    => 'Recherche une tâche',
    'calculate_task_trans_key'                 => 'Calculer les tâches',
    'calculate_date_task_trans_key'            => 'Calcul des dates des tâches',
    'calculate_ressource_task_trans_key'       => 'Calcul des ressources des tâches',
    'add_sub_assembly_trans_key'               => 'Ajouter un sous-assemblage',
    'sub_assembly_trans_key'                   => 'Sous-assemblage',
    'component_trans_key'                      => 'Composant',
    'not_this_page_trans_key'                  => 'Pas pour cette page',
    'technical_cut_trans_key'                  => 'Découpage technique',
    'bill_of_materials_trans_key'              => 'Nomenclature',
    'bom_trans_key'                            => 'BOM',
    'generic_trans_key'                        => 'Générique',
    'current_count_task_trans_key'             => 'Nombre d\'OF en cours',
    'average_time_task_trans_key'              => 'Average processing time for OFs',
    'user_productivity_trans_key'              => 'Productivité des utilisateurs',
    'task_count_trans_key'                     => 'Nombre de tâches',
    'total_hours_per_resource_trans_key'       => 'Heures totales par ressource',
    'total_hours_per_month_trans_key'          => 'Heures produites sur le mois',
    'trs_trans_key'                            => 'TRS',
    'trs_per_month_trans_key'                  => 'TRS sur le mois',
    'active_calculated_price_trans_key'        => 'Activer le prix calculé',
    'deactivate_calculated_price_trans_key'    => 'Désactiver le prix calculé',
    'user_choise_trans_key'                    => 'Choix de l\'utilisateur',
    
    //DELIVERY
    'delivery_notes_trans_key'                 => 'Bon de livraison',
    'current_month_trans_key'                  => 'Mois en cours',
    'deliverys_notes_request_trans_key'        => 'Attente de bons de livraison',
    'deliverys_notes_list_trans_key'           => 'Liste des bons de livraisons',
    'name_of_deliverys_notes_trans_key'        => 'Nom du bon de livraison',
    'new_delivery_note_trans_key'              => 'Nouveau bon de livraison',
    'returns_trans_key'                        => 'Retours clients',
    'tracking_number_note_trans_key'           => 'Numéro de suivis',
    'packaging_trans_key'                      => 'Packaging',
    'new_packaging_trans_key'                  => 'Nouveau packaging',

    //INVOICE
    'invoices_trans_key'                       => 'Factures',
    'invoice_trans_key'                        => 'Facture',
    'invoices_request_trans_key'               => 'Attente de facture',
    'invoices_list_trans_key'                  => 'Factures liste',
    'invoices_export_trans_key'                => 'Exporter ligne de facture',
    'others_trans_key'                         => 'Autres',
    'new_invoice_trans_key'                    => 'Nouvelle facture',
    'generate_pending_invoices_trans_key'      => 'Générer les factures en attente',
    'number_of_invoice_trans_key'              => 'Nombre total de factures',
    'amount_of_invoice_trans_key'              => 'Montant total des factures',
    'payments_received_of_invoice_trans_key'   => 'Montant total des paiements reçus',
    'bills_paid_trans_key'                     => 'Factures payées',
    'bills_unpaid_trans_key'                   => 'Factures impayées',
    'top_customers_trans_key'                  => 'Meilleurs clients',
    'top_products_trans_key'                   => 'Meilleurs produits',
    'average_payment_time_trans_key'           => 'Délai moyen de paiement',
    'late_payment_time_trans_key'              => 'Taux de retard de paiement',
    'incoterm_trans_key'                       => 'Incoterm',
    'still_invoiced_trans_key'                 => 'Reste à facturer',

    //CREDIT NOTE
    'credit_notes_trans_key'                   => 'Avoirs',
    'credit_note_trans_key'                    => 'Avoir',
    'credit_notes_list_trans_key'              => 'Liste des avoirs',
    'new_credit_note_trans_key'                => 'Nouvel avoir',

    //PRODUCT
    'product_trans_key'                        => 'Produit',
    'products_trans_key'                       => 'Produits',
    'purchased_price_trans_key'                => 'Prix d\'achat',
    'selling_price_trans_key'                  => 'Prix de vente ',
    'purchased_trans_key'                      => 'Acheté',
    'sold_trans_key'                           => 'Vendu',
    'tracability_trans_key'                    => 'Tracabilité',
    'stock_consumption_trans_key'              => 'Consommations de stock',
    'proprieties_trans_key'                    => 'Propriétés',
    'quantite_eco_min_trans_key'               => 'Quantité eco min',
    'quantite_min_trans_key'                   => 'Quantité min',
    'quantite_eco_max_trans_key'               => 'Quantité eco max',
    'quantite_max_trans_key'                   => 'Quantité max',
    'duplicate_product_trans_key'              => 'Dupliqué produit',
    'duplicate_purchase_quotation_trans_key'   => 'Dupliquer la demande de devis',
    'other_information_trans_key'              => 'Autre information',
    'index_trans_key'                          => 'Ind',
    'new_product_trans_key'                    => 'Nouveau produit',
    'price_by_qty_trans_key'                   => 'Prix par quantité',
    'weighted_average_price_trans_key'         => 'Prix ​​moyen pondéré',
    'last_purchase_price_price_trans_key'      => 'Dernier prix d\'achat',

    'serial_numbers_trans_key'                 => 'Numéros de série',
    'serial_numbers_list_trans_key'            => 'Liste des numéros de série',

    'product_list_trans_key'                   => 'Liste des produits',
    'stock_trans_key'                          => 'Stock',
    'stock_list_trans_key'                     => 'Stock liste',
    'stock_location_trans_key'                 => 'Stocks emplacement',
    'stock_location_list_trans_key'            => 'Stocks emplacement liste',
    'location_trans_key'                       => 'emplacement',
    'stock_location_product_list_trans_key'    => 'Stocks emplacement produit liste',
    'new_stock_location_product_trans_key'     => 'Nouvelle ligne de stock produit ',
    'new_stock_location_trans_key'             => 'Nouveau emplacement de stock',
    'new_stock_trans_key'                      => 'Nouveau stock',
    'new_entry_stock_trans_key'                => 'Nouvelle ligne entré stock',
    'new_sorting_stock_trans_key'              => 'Nouvelle ligne sortie stock',
    'new_transfer_stock_trans_key'             => 'Transfert de stock',
    'transfer_destination_trans_key'           => 'Emplacement destination',
    'transfer_select_destination_trans_key'    => 'Sélectionner un emplacement destination',
    'transfer_success_trans_key'               => 'Transfert de stock effectué avec succès',
    'transfer_insufficient_stock_trans_key'    => 'Stock insuffisant pour effectuer ce transfert',
    'transfer_stock_entry_trans_key'           => 'Entrée transfert inter-stock',
    'no_product_in_line_stock_trans_key'       => 'Aucun produit rataché à la ligne de commande',
    'current_stock_trans_key'                  => 'Stock courant',
    'entries_stock_trans_key'                  => 'Entrées stock',
    'sortings_stock_trans_key'                 => 'sorties stock',
    'total_stock_trans_key'                    => 'Stock total',
    'view_stock_list_trans_key'                => 'Afficher la liste des stocks',
    'stock_requested_trans_key'                => 'Afficher stock/demandé',
    'requested_trans_key'                      => 'Demandé',
    'remove_from_stock_trans_key'              => 'Retirer du stock',
    'remove_component_lines_stock_trans_key'   => 'Supprimer automatiquement les lignes de composants du stock',
    'create_serial_number_trans_key'           => 'Créer automatiquement un numéro de série si vous n\'en avez pas',
    'current_stock_note_trans_key'             => 'L\'affichage des encours demandés comprend les lignes de commandes non terminées sans tâche et les tâches dont le statuts est égale à : Ouvert, Commencé, Progression',
    'current_stock_note_2_trans_key'           => 'Attention, créer une commande interne aura pour but de refaire du stock basé sur l\'encours, cependant cela peux réaliser du surstock si l\'action est réalisée en doublons avec le flux normale.',

    'inventory_trans_key'                      => 'Inventory',

    //PURCHASE
    'purchase_trans_key'                       => 'Achats',
    'name_purchase_trans_key'                  => 'Nom de la commande d\'achat',
    'update_valide_trans_key'                  => 'Mettre à jour l\'adresse et un contact valides',
    'purchase_order_trans_key'                 => 'Commande achat',
    'top_rated_supplier_trans_key'             => 'Top fournisseurr',
    'delevery_time_trans_key'                  => 'Délais de reception (moy jours)',
    'average_purchase_price_trans_key'         => 'Prix ​​d\'achat moyen',
    'most_purchased_products_trans_key'        => 'Produits les plus achetés',
    'suppliers_longest_times_trans_key'        => 'Fournisseurs avec les délais de réception les plus longs',
    'suppliers_shortest_times_trans_key'       => 'Fournisseurs avec les délais de réception les plus courts',
    'proposed_purchase_price_trans_key'        => 'Prix d\'achat proposé',

    //PURCHASE REQUEST
    'purchase_request_trans_key'               => 'Demande d\'achat',
    'requests_for_quotation_list_trans_key'    => 'Demande de devis',
    'rfq_group_trans_key'                      => 'Groupe RFQ',
    'compare_rfq_trans_key'                    => 'Comparer les fournisseurs',
    'name_quote_request_trans_key'             => 'Nom de la demande d\'achat',

    'purchase_list_trans_key'                  => 'Liste commande d\'achat',
    'waiting_to_receipt_trans_key'             => 'Attente de réception achat',

    //PURCHASE RECIEPT
    'po_receipt_trans_key'                     => 'CA reçue',
    'name_purchase_reciept_trans_key'          => 'Nom du reçu',
    'po_receipt_note_trans_key'                => 'Reception en attente de controle',
    'validate_control_trans_key'               => 'Valider le contrôle',
    'inspection_details_trans_key'             => "Détails d'inspection",
    'inspection_result_trans_key'              => "Résultat d'inspection",
    'inspection_date_trans_key'                => "Date d'inspection",
    'inspected_by_trans_key'                   => 'Inspecté par',
    'select_option_trans_key'                  => 'Sélectionner une option',
    'quick_nc_creation_trans_key'              => "Création rapide d'une NC",
    'inspection_qty_error_trans_key'           => 'Les quantités acceptées et rejetées ne peuvent pas dépasser la quantité reçue (:receipt).',
    'inspection_update_success_trans_key'      => "Les données d'inspection ont été mises à jour avec succès.",

    'waiting_to_invoice_trans_key'             => 'Attente de facturation',
    'incoming_invoices_trans_key'              => 'Factures reçues',

    //PURCHASE INVOICE
    'invoice_supplier_trans_key'               => 'Facture fournisseur',
    'name_purchase_invoice_trans_key'          => 'Nom de la facture fournisseur',

    //QUALITY
    'quality_trans_key'                        => 'Qualité',
    'non_conformities_trans_key'               => 'Non-conformités',
    'non_conformitie_trans_key'                => 'Non-conformité',   
    'settings_trans_key'                       => 'Réglage',
    'action_note_trans_key'                    => 'Les actions sont des mesures prises pour éviter qu\'un problème ne survienne (actions préventives), pour résoudre un problème existant (actions correctives) ou pour améliorer un processus ou un produit (demandes d\'amélioration).',
    'no_nc_trans_key'                          => 'Pas de lien avec NC',
    'problem_trans_key'                        => 'Problème',
    'cause_trans_key'                          => 'Cause',
    'cause_comment_trans_key'                  => 'Commentaire cause',
    'cause_type_trans_key'                     => 'Cause type',
    'new_cause_trans_key'                      => 'Nouvelle cause',
    'new_action_trans_key'                     => 'Nouvelle action',
    'new_derogation_trans_key'                 => 'Nouvelle dérogation', 
    'new_failure_trans_key'                    => 'Nouvelle défaillance',
    'new_correction_trans_key'                 => 'Nouvelle correction',
    'new_measuring_devices_trans_key'          => 'Nouveau appareil de mesure',
    'new_non_conformitie_trans_key'            => 'Nouvelle non conformité',
    'measuring_devices_note_trans_key'         => 'Les appareils de mesure utilisés pour mesurer la qualité.',
    'non_conformitie_note_trans_key'           => 'Les fiches de non-conformité sont des documents récapitulant les données liées à un problème qualité survenu au sein de votre entreprise, chez un client ou chez un fournisseur et les surcoûts que cela a engendré.',
    'quality_setting_note_trans_key'           => 'Pour éviter toute perte de temps, vous avez la possibilité de prédéfinir les défauts, causes et/ou mesures de correction que vous utiliserez le plus. Il vous suffira alors de faire le choix pertinent lors de la création d\'une fiche de non-conformité ou des interventions de maintenance d\'une ressource.',
    'amdec_trans_key'                          => 'AMDEC',
    'new_amdec_trans_key'                      => 'Nouvelle AMDEC',
    'litigation_rate_trans_key'                => 'Taux de litige',
    'resolution_time_trans_key'                => 'Temps de résolution',

    //METHODS
    'methods_trans_key'                        => 'Méthodes',
    'service_trans_key'                        => 'Service',
    'additional_services_trans_key'            => 'Services complémentaires',
    'labor_resource_trans_key'                 => "Main-d'œuvre",
    'times_shifts_trans_key'                 => 'Horaires de travail',
    'new_shift_pattern_trans_key'              => "Nouveau régime horaire",
    'shift_pattern_info_trans_key'             => "Un régime horaire définit les plages travaillées de chaque jour de la semaine : une plage par jour pour du 1×8, trois plages pour du 3×8 — la nuit franchissant minuit. Les ressources sans régime propre utilisent celui marqué par défaut ; sans régime par défaut, les horaires historiques 8h-18h du lundi au vendredi restent appliqués.",
    'shift_slots_trans_key'                    => 'Plages',
    'shift_no_slot_warning_trans_key'          => "Ce régime n'a encore aucune plage. Les ressources qui l'utilisent conservent leur capacité hebdomadaire lissée sur cinq jours, comme si aucun régime n'était choisi.",
    'shift_default_trans_key'                  => 'Par défaut',
    'shift_default_hint_trans_key'             => "Utiliser comme régime par défaut de l'atelier",
    'shift_apply_to_week_trans_key'            => 'Appliquer du lundi au vendredi',
    'shift_crosses_midnight_trans_key'         => 'Franchit minuit (poste de nuit)',
    'shift_pattern_trans_key'                  => 'Régime horaire',
    'shift_pattern_created_success_trans_key'  => 'Régime horaire créé avec succès.',
    'shift_pattern_updated_success_trans_key'  => 'Régime horaire mis à jour avec succès.',
    'shift_slot_created_success_trans_key'     => 'Plage créée avec succès.',
    'shift_slot_deleted_success_trans_key'     => 'Plage supprimée avec succès.',
    'labor_resource_hint_trans_key'            => "Ressource composée de personnes (équipe, poste manuel) et non d'une machine",
    'labor_ratio_trans_key'                    => 'Opérateurs par heure (0 = aucun, 0,5 = un opérateur pour deux machines)',
    'qualified_users_trans_key'                => 'Personnes habilitées',
    'additional_services_hint_trans_key'       => 'Autres opérations réalisables par cette ressource, par ordre de préférence',
    'resource_not_in_service_trans_key'        => 'Cette ressource ne réalise pas le service de la tâche.',
    'new_service_trans_key'                    => 'Nouveau service',
    'new_ressource_trans_key'                  => 'Nouvelle ressource',
    'new_section_trans_key'                    => 'Nouvelle section',
    'new_location_trans_key'                   => 'Nouvelle location',
    'new_unit_trans_key'                       => 'Nouvelle unité',
    'new_family_trans_key'                     => 'Nouvelle famille',
    'new_tool_trans_key'                       => 'Nouveau outil',
    'new_standard_bom_trans_key'               => 'Nouvelle nomenclature standard',
    'standard_bom_trans_key'                   => 'Nomenclature standard',
    'family_trans_key'                         => 'Famille',
    'service_info_trans_key'                   => 'Dans la liste déroulante Type, indiquez si la prestation est productive (opération de fabrication) ou implique effectivement l\'approvisionnement en matières premières ou fournitures, est une prestation réalisée en externe, c\'est-à-dire par un sous-traitant, ou est une composante composée. Le type de prestation permet ensuite de filtrer les données lors de la création d\'une liste de tâches, d\'une nomenclature, etc. Pour les matières premières, on distingue également les barres, les plaques et les blocs. L\'écran utilisé pour détailler les matières premières peut être différent selon le type de matière première.',
    'ressource_info_trans_key'                 => 'Selon votre méthode de travail, les ressources peuvent être des employés, des machines ou un mélange des deux. Dans tous les cas, vous devez indiquer pour chacun d’eux le nombre maximum d’heures de travail par jour ainsi que les prestations qu’ils peuvent réaliser.',
    'unit_info_trans_key'                      => 'Vous devez saisir toutes les unités avec lesquelles vous pourriez avoir à travailler dans votre entreprise. Il peut s\'agir de pièces, de mètres, de kilogrammes, etc..',
    'family_info_trans_key'                    => 'La définition de sous-groupes par service permet de filtrer les composants à des étapes ultérieures.',
    'section_info_trans_key'                   => 'La fonction Sections vous permet de définir toutes les sections composant votre entreprise, c\'est-à-dire les zones physiques où sont regroupés les postes de travail et les opérateurs en fonction de leur métier et de leur coût.',

    //ACCOUNTING
    'accounting_trans_key'                     => 'Comptabilité',
    'new_payment_methods_trans_key'            => 'Nouveau mode de paiement',
    'new_payment_conditions_trans_key'         => 'Nouvelle condition de paiement',
    'new_delevery_method_trans_key'            => 'Noueau mode de livraison',
    'new_vat_trans_key'                        => 'Nouvelle TVA',
    'accounting_allocations_trans_key'         => 'Allocation comptable',
    'new_accounting_allocations_trans_key'     => 'Nouvelle allocation comptable',
    'assets_trans_key'                         => 'Immobilisations',

    //TIME
    'times_setting_trans_key'                  => 'Réglage temps',  
    'new_machine_event_trans_key'              => 'Nouveau type d\'événement machine',
    'new_banck_holiday_trans_key'              => 'Nouveau jour férié',
    'new_improduct_time_trans_key'             => 'Nouveau temps improductif',
    'machine_statu_trans_key'                  => 'Statut machine',  
    'resource_required_trans_key'              => 'Ressource requise',  
    
    //HUMAN RESOURCES
    'human_resources_trans_key'                => 'Ressources humaines',   
    'expense_categories_trans_key'             => 'Catégorie de dépense',
    'categories_trans_key'                     => 'Catégorie',
    'payer_trans_key'                          => 'Payeur',
    'scan_file_trans_key'                      => 'Fichier scanné',
    'location_expense_trans_key'               => 'Localisation',
    'new_expense_categories_trans_key'         => 'Nouvelle catégorie de dépense',
    
    //OSH
    'osh_trans_key'                            => 'Risque et santé',
    'incidents_trans_key'                      => 'Incidents',
    'risques_trans_key'                        => 'Risques',
    'formations_trans_key'                     => 'Formations',
    'conformites_trans_key'                    => 'Conformités',
    'osh_training_List_trans_key'              => 'Liste des Formations',
    'type_of_training_trans_key'               => 'Type de Formation',
    'training_date_trans_key'                  => 'Date de Formation',
    'expiration_date_trans_key'                => 'Date d\'Expiration',
    'certification_obtained_trans_key'         => 'Certification Obtenue',
    'new_training_trans_key'                   => 'Nouvelle Formation',
    'new_risk_trans_key'                       => 'Nouveau Risque',
    'new_conformity_trans_key'                 => 'Nouvelle Conformité',
    'new_incident_trans_key'                   => 'Nouvel Incident',
    'severity_class_trans_key'                 => 'Gravité',
    'preventive_measures_class_trans_key'      => 'Mesures Préventives',
    'incident_date_class_trans_key'            => 'Date de l\'Incident',
    'corrective_actions_class_trans_key'       => 'Actions Correctives',

    //USER
    'whiteboard_trans_key'                     => 'Tableau blanc',
    'users_trans_key'                          => 'Utilisateur',
    'users_list_trans_key'                     => 'Liste des utilisateurs',
    'about_trans_key'                          => 'A propos',
    'new_contract_trans_key'                   => 'Nouveau contrat',
    'personnal_information_trans_key'          => 'Informations personnelles',
    'personnal_email_trans_key'                => 'E-mail personnel',
    'about_you_trans_key'                      => 'Au propos de vous',
    'about_setup_trans_key'                    => 'Réglage compte',
    'password_account_trans_key'               => 'Mot de passe (uniquement pour confirmer les modifications)',
    'marital_status_trans_key'                 => 'État civil', 
    'adress_section_trans_key'                 => 'Section Adresse',
    
    //YOUR COMPANIE
    'your_company_trans_key'                   => 'Votre entreprise',
    'default_value_trans_key'                  => 'Valeur par défaut',
    'add_value_day_offer_trans_key'            => 'Jour supplémentaire de validité de l\'offre',
    'add_value_day_delivery_trans_key'         => 'Délai de livraison d\'un jour supplémentaire',
    'day_trans_key'                            => 'Jour(s)',
    'header_font_color_trans_key'              => 'Couleur d’en-tête',
    'pdf_theme_trans_key'                      => 'Thème PDF',
    'pdf_theme_fallback_trans_key'             => 'Par défaut',
    'pdf_theme_help_trans_key'                 => 'Choisissez le thème utilisé pour générer les PDF.',
    'pdf_custom_css_trans_key'                 => 'CSS PDF personnalisé',
    'pdf_custom_css_placeholder_trans_key'     => 'Saisissez des règles CSS supplémentaires appliquées au thème PDF sélectionné',
    'pdf_custom_css_help_trans_key'            => 'Laissez vide pour utiliser uniquement le style du thème.',
    'manufacturing_information_trans_key'      => 'Informations de fabrication',
    'bare_code_type_trans_key'                 => 'Type de code pour la tâche de déclaration',
    'make_an_announcement_trans_key'           => 'Faire une annonce',
    'make_new_permissions_trans_key'           => 'Créer une nouvelle autorisation',
    'add_role_permission_trans_key'            => 'Ajouter un rôle dans les autorisations',
    'public_link_cgv_trans_key'                => 'Lien publique pour les CGV',
    'add_cgv_to_quote_order_trans_key'         => 'Ajouter les CGV à la suite des devis ou commande',
    'show_current_file_trans_key'              => 'Voir le ficher courant',
    'name_field_trans_key'                     => 'Nom du champs',
    'type_field_trans_key'                     => 'Type de champs',
    'entity_type_trans_key'                    => 'Type associé',
    'roles_and_permissions_trans_key'          => 'Rôles & permissions',
    'import_export_trans_key'                  => 'Imports/Exports',
    'logs_view_trans_key'                      => 'Logs view',
    'template_mail_trans_key'                  => 'Template de mail',
    'attached_mail_trans_key'                  => 'Email attachés',
    'template_list_mail_trans_key'             => 'Liste des modèles de mails',
    'new_template_mail_trans_key'              => 'Nouveau modèle d\'email',
    'attachment_trans_key'                     => 'Pièce jointe',
    'new_mail_trans_key'                       => 'Nouvel E-mail',
    
    //PROFIL
    'profile_trans_key'                        => 'Profil',
    'information_trans_key'                    => 'Information',
    'pay_grade_trans_key'                      => 'Niveau de rémunération',
    'work_station_id_trans_key'                => 'Identifiant du poste de travail',
    'joined_date_trans_key'                    => 'Date d\'adhésion',
    'confirmation_date_trans_key'              => 'Date de confirmation',
    'notification_choice_trans_key'            => 'Choix des notifications',
    'new_user_trans_key'                       => 'Nouvel utilisateur',
    'new_absence_request_trans_key'            => 'Nouvelle demande d\'absence',
    'expenses_trans_key'                       => 'Note de frais',
    'expense_report_trans_key'                 => 'Note de frais',
    'new_expense_report_trans_key'             => 'Nouvelle note de frais',
    'expense_validation_trans_key'             => 'Validation des note de frais',
    'attendance_report_trans_key'              => 'Rapport de présence',
    'total_duration_trans_key'                 => 'Durée totale',
    'anomalies_trans_key'                      => 'Anomalies',
    'filter_trans_key'                         => 'Filtrer',
    'all_trans_key'                            => 'Tous',

    'licence_trans_key'                        => 'Licence',
    'release_note_trans_key'                   => 'Notes de version',

    'search_results_trans_key'                 => 'Résulta de la recherche',

    //WORKSHOP
    'workshop_interface_trans_key'             => 'Workshop Interface',
    'to_access_trans_key'                      => 'Accéder',
    'production_declaration_trans_key'         => 'Déclaration de Production',
    'note_1_trans_key'                         => 'Consult and plan the tasks to be carried out.',
    'note_2_trans_key'                         => 'Faites des déclarations de production',

    'energy_consumption_trans_key'             => "Consommation d'énergie",
    'machine_trans_key'                        => 'Machine',
    'kwh_trans_key'                            => 'kWh',
    'cost_per_kwh_trans_key'                   => 'Coût par kWh',
    'total_cost_trans_key'                     => 'Coût total',

    'order_created_log_trans_key'              => 'Commande créée',
    'order_creation_failed_log_trans_key'      => 'Échec de création de la commande',
    'quote_updated_log_trans_key'              => 'Devis mis à jour',
    'quote_update_failed_log_trans_key'        => 'Échec de mise à jour du devis',
    'quote_update_success_trans_key'           => 'Devis mis à jour avec succès',
    'average_supply_delay_trans_key'           => "Délai moyen d'approvisionnement",
    'order_site_trans_key'                     => 'Chantier',
    'characteristics_trans_key'                => 'Caractéristiques',
    'contact_info_trans_key'                   => 'Informations de contact',
    'report_trans_key'                         => 'Rapport',
    'send_time_trans_key'                      => 'Heure d\'envoi',
    'enabled_trans_key'                        => 'Activé',
    'automatic_email_reports_trans_key'        => 'E-mails automatiques',
    'automatic_email_reports_help_trans_key'   => 'Choisissez les rapports à recevoir et l\'heure d\'envoi.',
    'automatic_email_reports_saved_trans_key'  => 'Paramètres des e-mails automatiques mis à jour.',
    'automatic_email_report_subject_trans_key' => 'Rapport automatique : :report',
    'automatic_email_reports_intro_trans_key'  => 'Bonjour :name, voici votre rapport planifié.',
    'generated_at_trans_key'                   => 'Généré le',
    'overdue_orders_report_trans_key'          => 'Commandes en retard',
    'tomorrow_orders_report_trans_key'         => 'Commandes à livrer demain',
    'low_stock_report_trans_key'               => 'Articles en stock bas',

    //GMAO
    'gmao_work_orders_page_title_trans_key'        => 'GMAO - Ordres de travail',
    'gmao_work_orders_card_title_trans_key'        => 'Ordres de travail',
    'gmao_new_work_order_trans_key'                => 'Nouvel ordre de travail',
    'gmao_edit_work_order_trans_key'               => 'Modifier l\'ordre de travail',
    'gmao_work_order_trans_key'                    => 'Ordre de travail',
    'gmao_work_order_number_trans_key'             => 'Ordre de travail n°:id',
    'gmao_technician_trans_key'                    => 'Technicien',
    'gmao_select_asset_trans_key'                  => 'Sélectionner un équipement',
    'gmao_not_specified_trans_key'                 => 'Non spécifié',
    'gmao_not_available_trans_key'                 => 'Non disponible',
    'gmao_not_started_trans_key'                   => 'Non démarré',
    'gmao_not_finished_trans_key'                  => 'Non terminé',
    'gmao_not_assigned_trans_key'                  => 'Non assigné',
    'gmao_unassigned_trans_key'                    => 'Non affecté',
    'gmao_assigned_technician_trans_key'           => 'Technicien assigné',
    'gmao_requested_at_trans_key'                  => 'Demandé le',
    'gmao_scheduled_at_trans_key'                  => 'Planifié le',
    'gmao_started_at_trans_key'                    => 'Démarré le',
    'gmao_finished_at_trans_key'                   => 'Terminé le',
    'gmao_completed_at_trans_key'                  => 'Clôturé le',
    'gmao_estimated_duration_minutes_trans_key'    => 'Durée estimée (minutes)',
    'gmao_actual_duration_minutes_trans_key'       => 'Durée réelle (minutes)',
    'gmao_estimated_duration_trans_key'            => 'Durée estimée',
    'gmao_actual_duration_trans_key'               => 'Durée réelle',
    'gmao_minutes_suffix_trans_key'                => 'min',
    'gmao_actions_performed_trans_key'             => 'Actions réalisées',
    'gmao_parts_consumed_trans_key'                => 'Pièces consommées',
    'gmao_comments_photos_trans_key'               => 'Commentaires / photos',
    'gmao_severity_trans_key'                      => 'Gravité',
    'gmao_machine_stopped_trans_key'               => 'Machine arrêtée',
    'gmao_failure_started_at_trans_key'            => 'Début de la panne',
    'gmao_general_information_trans_key'           => 'Informations générales',
    'gmao_created_by_trans_key'                    => 'Créé par',
    'gmao_schedule_timeline_trans_key'             => 'Planning et chronologie',
    'gmao_execution_details_trans_key'             => 'Détails d\'exécution',
    'gmao_none_recorded_trans_key'                 => 'Aucune saisie',
    'gmao_none_used_trans_key'                     => 'Aucune utilisée',
    'gmao_no_comments_photos_trans_key'            => 'Aucun commentaire ou photo ajouté',
    'gmao_back_to_list_trans_key'                  => 'Retour à la liste',
    'gmao_quick_info_trans_key'                    => 'Infos rapides',
    'gmao_dashboard_page_title_trans_key'          => 'GMAO - Tableau de bord',
    'gmao_kpi_maintenance_card_trans_key'          => '3. 📊 KPI & Indicateurs maintenance',
    'gmao_kpi_trans_key'                           => 'KPI',
    'gmao_maintenance_plans_page_title_trans_key'  => 'GMAO - Plans de maintenance',
    'gmao_maintenance_plans_card_title_trans_key'  => 'Plans de maintenance',
    'gmao_new_maintenance_plan_trans_key'          => 'Nouveau plan de maintenance',
    'gmao_edit_maintenance_plan_trans_key'         => 'Modifier le plan de maintenance',
    'gmao_maintenance_plan_trans_key'              => 'Plan de maintenance',
    'gmao_maintenance_plan_number_trans_key'       => 'Plan de maintenance n°:id',
    'gmao_trigger_trans_key'                       => 'Déclencheur',
    'gmao_trigger_type_trans_key'                  => 'Type de déclencheur',
    'gmao_trigger_value_trans_key'                 => 'Valeur du déclencheur',
    'gmao_trigger_value_hint_trans_key'            => 'Valeur du déclencheur (hebdomadaire, mensuel, 500 heures, 200 cycles...)',
    'gmao_fixed_date_trans_key'                    => 'Date fixe',
    'gmao_required_skill_trans_key'                => 'Compétence requise',
    'gmao_actions_list_trans_key'                  => 'Liste des actions',
    'gmao_required_parts_trans_key'                => 'Pièces requises',
    'gmao_active_trans_key'                        => 'Actif',
    'gmao_frequency_trans_key'                     => 'Fréquence',
    'gmao_trigger_configuration_trans_key'         => 'Configuration du déclencheur',
    'gmao_schedule_information_trans_key'          => 'Informations de planification',
    'gmao_next_scheduled_date_trans_key'           => 'Prochaine date planifiée',
    'gmao_last_modified_trans_key'                 => 'Dernière modification',
    'gmao_execution_resources_trans_key'           => 'Ressources d\'exécution',
    'gmao_no_actions_defined_trans_key'            => 'Aucune action définie',
    'gmao_generate_work_order_trans_key'           => 'Générer un ordre de travail',
    'gmao_edit_plan_trans_key'                     => 'Modifier le plan',
    'gmao_plan_summary_trans_key'                  => 'Résumé du plan',
    'gmao_duration_trans_key'                      => 'Durée',
    'gmao_next_scheduled_trans_key'                => 'Prochaine planification',
    'gmao_maintenance_work_orders_trans_key'       => 'Ordres de travail de maintenance',
    'methods_overview_trans_key'               => 'Vue d\'ensemble méthodes',
    'logs_trans_key'                            => 'Journaux',
    'beta_trans_key'                            => 'Bêta',
    'info_trans_key'                            => 'Info',
    'customer_disabled_warning_trans_key'       => 'Le client :customer est actuellement désactivé. Vous ne pouvez pas modifier le nom, le contact ou l\'adresse.',

    //FLASH MESSAGES
    'user_info_updated_success_trans_key' => 'Informations utilisateur mises à jour avec succès.',
    'user_locked_success_trans_key' => 'Utilisateur verrouillé avec succès.',
    'contract_added_success_trans_key' => 'Contrat ajouté avec succès.',
    'factory_info_updated_success_trans_key' => 'Informations de l\'usine mises à jour avec succès.',
    'announcement_added_success_trans_key' => 'Annonce ajoutée avec succès.',
    'announcement_deleted_success_trans_key' => 'Annonce supprimée avec succès.',
    'stock_line_created_success_trans_key' => 'Nouvelle ligne de stock créée avec succès.',
    'stock_move_created_success_trans_key' => 'Nouveau mouvement de stock créé avec succès.',
    'purchase_order_updated_success_trans_key' => 'Commande d\'achat mise à jour avec succès.',
    'purchase_receipt_updated_success_trans_key' => 'Réception d\'achat mise à jour avec succès.',
    'site_created_success_trans_key' => 'Site créé.',
    'site_updated_success_trans_key' => 'Site mis à jour.',
    'site_deleted_success_trans_key' => 'Site supprimé.',
    'no_default_address_contact_vat_error_trans_key' => 'Aucun paramétrage par défaut trouvé pour l\'adresse, le contact ou la TVA comptable.',
    'purchase_order_created_success_trans_key' => 'Nouvelle commande d\'achat créée avec succès.',
    'no_supplied_in_progress_status_error_trans_key' => 'Aucun statut "Approvisionné" ou "En cours" trouvé dans le kanban pour définir l\'avancement.',
    'non_conformity_created_success_trans_key' => 'Non-conformité créée avec succès.',
    'non_conformity_updated_success_trans_key' => 'Non-conformité mise à jour avec succès.',
    'role_created_success_trans_key' => 'Rôle créé avec succès.',
    'role_updated_success_trans_key' => 'Rôle mis à jour avec succès.',
    'role_deleted_success_trans_key' => 'Rôle supprimé avec succès.',
    'supplier_added_success_trans_key' => 'Fournisseur ajouté avec succès.',
    'quantity_price_added_success_trans_key' => 'Prix par quantité ajouté avec succès.',
    'product_updated_success_trans_key' => 'Produit mis à jour avec succès.',
    'absence_request_created_success_trans_key' => 'Demande d\'absence créée avec succès.',
    'absence_request_updated_success_trans_key' => 'Demande d\'absence mise à jour avec succès.',
    'preorders_pdf_processed_success_trans_key' => 'PDF(s) envoyé(s), traitement Python terminé et import CSV exécuté avec succès.',
    'preorders_pdf_uploaded_success_trans_key' => 'PDF(s) envoyé(s) dans le stockage avec succès.',
    'preorders_pdf_all_duplicates_trans_key' => 'Aucun nouveau PDF traité : ce(s) fichier(s) ont déjà été importés :',
    'preorders_pdf_some_duplicates_trans_key' => 'Certains fichiers ont été ignorés car déjà importés :',
    'preorders_matching_done_success_trans_key' => 'Matching des articles terminé.',
    'email_template_created_success_trans_key' => 'Modèle de mail créé avec succès !',
    'email_template_updated_success_trans_key' => 'Modèle de mail mis à jour !',
    'email_template_deleted_success_trans_key' => 'Modèle supprimé avec succès !',
    'resource_created_success_trans_key' => 'Ressource créée avec succès.',
    'resource_updated_success_trans_key' => 'Ressource mise à jour avec succès.',
    'service_created_success_trans_key' => 'Service créé avec succès.',
    'service_updated_success_trans_key' => 'Service mis à jour avec succès.',
    'tool_created_success_trans_key' => 'Outil créé avec succès.',
    'tool_updated_success_trans_key' => 'Outil mis à jour avec succès.',

    /* ERROR PAGES */
    'error_back_dashboard_trans_key'    => 'Retour au tableau de bord',
    'error_404_title_trans_key'         => 'Page introuvable',
    'error_404_heading_trans_key'       => 'Oups ! Page introuvable.',
    'error_404_message_trans_key'       => 'La page que vous recherchez est introuvable. Vous pouvez <a href=":url">retourner au tableau de bord</a>.',
    'error_403_title_trans_key'         => 'Accès refusé',
    'error_403_heading_trans_key'       => 'Oups ! Accès refusé.',
    'error_403_message_trans_key'       => 'Vous n\'avez pas les droits nécessaires pour accéder à cette page. Vous pouvez <a href=":url">retourner au tableau de bord</a>.',
    'error_419_title_trans_key'         => 'Session expirée',
    'error_419_heading_trans_key'       => 'Oups ! Session expirée.',
    'error_419_message_trans_key'       => 'Votre session a expiré. Veuillez rafraîchir la page ou <a href=":url">retourner au tableau de bord</a>.',
    'error_500_title_trans_key'         => 'Erreur serveur',
    'error_500_heading_trans_key'       => 'Oups ! Erreur interne du serveur.',
    'error_500_message_trans_key'       => 'Une erreur inattendue s\'est produite. Nous travaillons à la résoudre. En attendant, vous pouvez <a href=":url">retourner au tableau de bord</a>.',
    'error_503_title_trans_key'         => 'Maintenance en cours',
    'error_503_heading_trans_key'       => 'Nous revenons bientôt !',
    'error_503_message_trans_key'       => 'Désolé pour la gêne occasionnée, nous effectuons une maintenance. Nous serons de retour en ligne très prochainement.',

    // --- Gestion documentaire unifiée (composant React FileManager) ---
    'loading_trans_key'                        => 'Chargement...',
    'download_trans_key'                       => 'Télécharger',
    'file_uploaded_success_trans_key'          => 'Fichier déposé avec succès.',
    'file_extension_not_allowed_trans_key'     => 'Ce format de fichier n\'est pas accepté.',
    'file_too_large_trans_key'                 => 'Fichier trop volumineux.',
    'file_role_trans_key'                      => 'Type de document',
    'file_role_auto_trans_key'                 => 'Automatique (selon le format)',
    'file_all_roles_trans_key'                 => 'Tous les types',
    'file_primary_trans_key'                   => 'Principal',
    'file_set_as_primary_trans_key'            => 'Définir comme document principal',
    'file_dropzone_hint_trans_key'             => 'Glissez vos fichiers ici, ou cliquez pour parcourir',
    'file_dropzone_formats_trans_key'          => 'PDF, STL, STEP, IGES, DXF, SVG, images, bureautique...',
    'file_select_a_file_trans_key'             => 'Sélectionnez un fichier pour l\'afficher',
    'file_no_inline_preview_trans_key'         => 'Aucun aperçu disponible pour ce format.',
    'file_open_new_tab_trans_key'              => 'Ouvrir dans un onglet',
    'file_confirm_delete_trans_key'            => 'Supprimer le fichier « :name » ?',
    'file_reset_view_trans_key'                => 'Réinitialiser',
    'file_triangles_trans_key'                 => ':count triangles',
    'file_entities_trans_key'                  => ':count entités',
    'file_layers_trans_key'                    => ':count calques',
    'file_solids_trans_key'                    => ':count solides',
    'file_ignored_trans_key'                   => 'Ignoré',
    'file_no_drawable_entity_trans_key'        => 'Aucune entité affichable dans ce fichier DXF.',
    'file_brep_read_failed_trans_key'          => 'Lecture du fichier CAO impossible.',
    'file_view_iso_trans_key'                  => 'Iso',
    'file_view_front_trans_key'                => 'Face',
    'file_view_back_trans_key'                 => 'Arrière',
    'file_view_top_trans_key'                  => 'Dessus',
    'file_view_bottom_trans_key'               => 'Dessous',
    'file_view_left_trans_key'                 => 'Gauche',
    'file_view_right_trans_key'                => 'Droite',
    'file_role_plan_trans_key'                 => 'Plan',
    'file_role_modele_3d_trans_key'            => 'Modèle 3D',
    'file_role_vectoriel_trans_key'            => 'Vectoriel / DXF',
    'file_role_photo_trans_key'                => 'Photo',
    'file_role_cam_trans_key'                  => 'FAO / Usinage',
    'file_role_certificat_trans_key'           => 'Certificat matière',
    'file_role_controle_trans_key'             => 'Contrôle qualité',
    'file_role_autre_trans_key'                => 'Autre',

    // Plan de charge (grille services x jours)
    'load_planning_today_trans_key'                 => "Aujourd'hui",
    'load_planning_weekend_trans_key'               => 'Week-end',
    'load_planning_compact_view_trans_key'          => 'Vue compacte',
    'load_planning_detailed_view_trans_key'         => 'Vue détaillée',
    'load_planning_legend_trans_key'                => 'Niveau de charge',
    'load_planning_level_free_trans_key'            => 'Libre',
    'load_planning_level_low_trans_key'             => 'Faible',
    'load_planning_level_medium_trans_key'          => 'Équilibré',
    'load_planning_level_high_trans_key'            => 'Tendu',
    'load_planning_level_over_trans_key'            => 'Surcharge',
    'load_planning_period_total_trans_key'          => 'Total :hours h',
    'load_planning_overloaded_days_trans_key'       => ':count jour(s) en surcharge',
    'load_planning_tasks_count_trans_key'           => ':count tâche(s)',
    'load_planning_tasks_processed_trans_key'       => ':count tâche(s) traitée(s)',
    'load_planning_running_trans_key'               => 'En cours…',
    'load_planning_finished_trans_key'              => 'Terminé',
    'load_planning_default_capacity_trans_key'      => 'Capacité journalière par défaut',
    'load_planning_default_capacity_hint_trans_key' => 'services sans ressources configurées — valeurs utilisées pour le calcul du taux de charge',
    'load_planning_capacity_resources_trans_key'    => 'Capacité issue des ressources configurées',
    'load_planning_capacity_custom_trans_key'       => 'Capacité définie manuellement (aucune ressource configurée)',
    'load_planning_capacity_fallback_trans_key'     => 'Aucune ressource configurée — fallback 8h/j',
    'load_planning_granularity_trans_key'           => 'Charge par',
    'load_planning_by_service_trans_key'            => 'Service',
    'load_planning_by_resource_trans_key'           => 'Ressource',
    'load_planning_unassigned_trans_key'            => 'Tâches non affectées',
    'load_planning_capacity_real_trans_key'         => 'Capacité :hours h/j',

    // Inventory workflow (Altior-style counting)
    'new_inventory_trans_key'                       => 'Nouvel inventaire',
    'inventory_default_label_trans_key'             => 'Inventaire',
    'inventory_created_trans_key'                   => 'Inventaire créé',
    'inventory_already_locked_trans_key'            => 'Cet inventaire est déjà verrouillé',
    'inventory_stock_changed_since_snapshot_trans_key' => 'Le stock a changé entre la création de l\'inventaire et sa validation. Mettez à jour votre fichier.',
    'inventory_invalid_status_trans_key'            => 'Statut de ligne invalide',
    'inventory_unknown_detail_id_trans_key'         => 'Ligne inconnue',
    'inventory_missing_product_or_location_trans_key' => 'Code produit ou emplacement manquant pour une nouvelle ligne',
    'inventory_unknown_product_trans_key'           => 'Produit inconnu',
    'inventory_unknown_location_trans_key'          => 'Emplacement inconnu pour ce produit',
    'inventory_counted_below_reserved_trans_key'    => 'La quantité comptée est inférieure à la quantité réservée',
    'inventory_pause_moves_warning_trans_key'       => 'Assurez-vous que vos mouvements de stock (réceptions, livraisons, déclarations d\'OF) soient réalisés et mis en pause pendant l\'inventaire.',
    'inventory_export_step_trans_key'               => 'Exporter la liste à compter',
    'inventory_export_normal_trans_key'             => 'Télécharger la liste à compter',
    'inventory_export_blind_trans_key'              => 'Version aveugle (impression terrain)',
    'inventory_export_hint_trans_key'               => 'Téléchargez le fichier XLSX, imprimez-le ou partagez-le avec les compteurs. La version aveugle masque la quantité théorique pour éviter le biais de confirmation.',
    'inventory_import_step_trans_key'               => 'Importer le fichier compté',
    'inventory_import_hint_trans_key'               => 'Déposez le fichier XLSX rempli. Le système vérifiera qu\'aucun mouvement n\'a modifié le stock depuis la création.',
    'inventory_summary_step_trans_key'              => 'Résumé et validation',
    'inventory_confirm_validate_trans_key'          => 'Confirmer la validation ? Les écarts génèreront des mouvements de stock irréversibles.',
    'inventory_shortage_trans_key'                  => 'Écart inventaire (sortie)',
    'theoretical_qty_trans_key'                     => 'Qté théorique',
    'reserved_qty_trans_key'                        => 'Qté réservée',
    'counted_qty_trans_key'                         => 'Qté comptée',
    'variance_qty_trans_key'                        => 'Écart qté',
    'variance_value_trans_key'                      => 'Écart valeur',
    'zone_trans_key'                                => 'Zone',
    'batch_trans_key'                               => 'Lot',
    'row_trans_key'                                 => 'Ligne',
    'errors_found_trans_key'                        => 'Erreurs détectées',
    'total_lines_trans_key'                         => 'Lignes totales',
    'counted_lines_trans_key'                       => 'Lignes comptées',
    'positive_variance_trans_key'                   => 'Écarts positifs',
    'negative_variance_trans_key'                   => 'Écarts négatifs',
    'net_variance_trans_key'                        => 'Écart net',
    'download_counting_file_trans_key'              => 'Télécharger le fichier de comptage',
    'entry_move_trans_key'                          => 'Mouvement d\'entrée',
    'exit_move_trans_key'                           => 'Mouvement de sortie',
    'nb_part_trans_key'                             => 'Nb pièces',
    'surface_perc_trans_key'                        => 'Surface %',
    'scope_trans_key'                               => 'Périmètre',
    'scope_all_trans_key'                           => 'Tout le stock',
    'scope_location_trans_key'                      => 'Par emplacement',
    'scope_category_trans_key'                      => 'Par catégorie',
    'select_locations_trans_key'                    => 'Sélectionner les emplacements',
    'select_categories_trans_key'                   => 'Sélectionner les catégories',
    'status_draft_trans_key'                        => 'Brouillon',
    'status_exported_trans_key'                     => 'Comptage importé',
    'status_validated_trans_key'                    => 'Validé',
    'status_cancelled_trans_key'                    => 'Annulé',
    'confirm_cancel_trans_key'                      => 'Confirmer l\'annulation ?',
    'preview_trans_key'                             => 'Aperçu',
    'drop_xlsx_file_trans_key'                      => 'Déposez le fichier XLSX ici',
    'actions_trans_key'                             => 'Actions',
    'create_trans_key'                              => 'Créer',
    'created_by_trans_key'                          => 'Créé par',
    'import_trans_key'                              => 'Importer',
    'validated_at_trans_key'                        => 'Validé le',
    'notes_trans_key'                               => 'Notes',
    'unit_cost_trans_key'                           => 'Coût unitaire',

    // Inventory show UX (stepper + explanatory blocks)
    'step_draft_trans_key'                          => 'Création',
    'step_exported_trans_key'                       => 'Comptage',
    'step_import_trans_key'                         => 'Import',
    'step_validated_trans_key'                      => 'Validé',
    'xlsx_content_title_trans_key'                  => 'Le fichier XLSX contient :',
    'xlsx_content_line_1_trans_key'                 => 'Une ligne par produit × emplacement × lot × géométrie (une tôle pleine et une chute forment 2 lignes distinctes).',
    'xlsx_content_line_2_trans_key'                 => 'Une colonne « Qté comptée » vide à remplir, une colonne « Statut » avec liste déroulante, un écart calculé automatiquement.',
    'xlsx_content_line_3_trans_key'                 => 'Les dimensions X/Y/Z, nombre de pièces et surface % à droite du fichier — modifiez-les pour une nouvelle ligne (chute retrouvée).',
    'export_blind_tooltip_trans_key'                => 'La version aveugle masque les colonnes Qté théorique, Qté réservée, Coût, Écart — à imprimer pour un comptage terrain sans biais.',
    'import_stock_change_warning_trans_key'         => 'Si un mouvement de stock a eu lieu depuis la création de l\'inventaire, l\'import sera rejeté avec la liste des produits concernés.',
    'analysing_trans_key'                           => 'Analyse en cours…',
    'importing_trans_key'                           => 'Import en cours…',
    'reimport_hint_trans_key'                       => 'Réimport possible',
    'errors_hint_trans_key'                         => 'Corrigez ces erreurs dans le fichier XLSX puis retentez l\'import. Les 50 premières erreurs sont affichées.',
    'more_errors_trans_key'                         => 'autres erreurs',
    'preview_ok_title_trans_key'                    => 'Fichier valide — prêt à importer',
    'preview_ok_hint_trans_key'                     => 'L\'import écrira les quantités comptées sur l\'inventaire. Aucun mouvement de stock n\'est encore créé — vous pourrez encore annuler avant de valider.',
    'confirm_import_trans_key'                      => 'Confirmer l\'import',
    'validate_incidence_hint_trans_key'             => 'La validation créera un mouvement d\'entrée (typ=1) pour chaque écart positif et un mouvement de sortie (typ=15) pour chaque écart négatif. Ces mouvements sont irréversibles.',
    'value_impact_trans_key'                        => 'Impact valorisé',
    'validated_summary_trans_key'                   => 'Inventaire validé. Les mouvements de régularisation ont été créés et le fichier de comptage archivé dans la GED.',
    'cancelled_summary_trans_key'                   => 'Inventaire annulé. Aucun mouvement de stock n\'a été créé.',
    'details_step_trans_key'                        => 'Détail des lignes',
    'filter_variance_only_trans_key'                => 'Écarts uniquement',

    // --- Ressources humaines : congés et GED ---
    'leave_balances_trans_key'                     => 'Soldes de congés',
    'leave_type_trans_key'                         => 'Nature du congé',
    'leave_types_trans_key'                        => 'Natures de congé',
    'leave_new_type_trans_key'                     => 'Nouvelle nature de congé',
    'leave_entitled_trans_key'                     => 'Droit acquis',
    'leave_carried_over_trans_key'                 => 'Report N-1',
    'leave_adjustment_trans_key'                   => 'Régularisation',
    'leave_acquired_trans_key'                     => 'Total acquis',
    'leave_taken_trans_key'                        => 'Pris',
    'leave_pending_trans_key'                      => 'En attente',
    'leave_remaining_trans_key'                    => 'Solde restant',
    'leave_days_trans_key'                         => 'Jours',
    'leave_not_counted_trans_key'                  => 'hors solde',
    'leave_period_trans_key'                       => 'Période de référence',
    'leave_default_quota_trans_key'                => 'Quota annuel par défaut',
    'leave_counts_against_balance_trans_key'       => 'Décompte du solde',
    'leave_generate_balances_trans_key'            => 'Générer les droits manquants',
    'leave_pending_hint_trans_key'                 => 'Le solde restant déduit les demandes validées et les demandes en attente.',
    'leave_balance_saved_success_trans_key'        => 'Solde de congés enregistré',
    'leave_balance_generated_success_trans_key'    => ':count droit(s) généré(s)',
    'leave_type_created_success_trans_key'         => 'Nature de congé créée',
    'leave_type_updated_success_trans_key'         => 'Nature de congé mise à jour',
    'hr_documents_confidential_trans_key'          => 'Documents confidentiels : accessibles au salarié concerné et aux RH uniquement.',
    'hr_documents_self_hint_trans_key'             => 'Vos documents RH. Seuls vous et le service RH y ont accès.',
    'file_role_contrat_trans_key'                  => 'Contrat de travail',
    'file_role_bulletin_paie_trans_key'            => 'Bulletin de paie',
    'file_role_arret_travail_trans_key'            => 'Arrêt de travail',
    'file_role_diplome_trans_key'                  => 'Diplôme / habilitation',
    'file_role_identite_trans_key'                 => 'Pièce d\'identité',
    // --- Ressources humaines : export paie ---
    'payroll_export_trans_key'                     => 'Export paie',
    'payroll_variables_trans_key'                  => 'Variables de paie',
    'payroll_month_trans_key'                      => 'Mois',
    'payroll_matricule_trans_key'                  => 'Matricule',
    'payroll_quantity_trans_key'                   => 'Quantité',
    'payroll_worked_hours_trans_key'               => 'Heures badgées',
    'payroll_production_hours_trans_key'           => 'Heures de production',
    'payroll_warnings_trans_key'                   => 'À vérifier avant transmission',
    'payroll_warning_attendance_trans_key'         => ':count anomalie(s) de pointage sur la période (badgeage non refermé)',
    'payroll_warning_matricule_trans_key'          => 'aucun matricule paie renseigné, l\'identifiant interne sera utilisé',
    'payroll_export_hint_trans_key'                => 'Format neutre : matricule, période, code rubrique, quantité. Le mapping des codes rubrique se fait côté logiciel de paie.',
    // --- QSE : matrice de polyvalence ---
    'skills_matrix_trans_key'                      => 'Matrice de polyvalence',
    'skills_alerts_trans_key'                      => 'Écarts constatés',
    'skills_no_alert_trans_key'                    => 'Aucun écart : toutes les tâches affectées sont couvertes par une habilitation valide.',
    'skills_matrix_advisory_trans_key'             => 'Écran informatif : aucune habilitation manquante ou périmée ne bloque une affectation, un lancement de tâche ou un ordonnancement. La décision reste à l\'atelier.',
    'skills_warning_window_trans_key'              => 'une habilitation est signalée « bientôt échue » :days jours avant sa date de fin',
    'skills_resources_hint_trans_key'              => 'Ressources sur lesquelles cette habilitation est attendue. Sert uniquement à signaler les écarts.',
    'skills_status_valid_trans_key'                => 'Valide',
    'skills_status_expiring_trans_key'             => 'Bientôt échue',
    'skills_status_expired_trans_key'              => 'Périmée',
    'skills_status_failed_trans_key'               => 'Non obtenue',
    'skills_status_missing_trans_key'              => 'Non formé',
    'training_types_trans_key'                     => 'Types d\'habilitation',
    'training_new_type_trans_key'                  => 'Nouveau type d\'habilitation',
    'training_type_trans_key'                      => 'Type d\'habilitation',
    'training_validity_trans_key'                  => 'Validité (mois)',
    'training_type_created_success_trans_key'      => 'Type d\'habilitation créé',
    'training_type_updated_success_trans_key'      => 'Type d\'habilitation mis à jour',

    'mail_config_menu_trans_key'               => 'Configuration e-mail',
    'invoice_draft_no_email_trans_key'         => 'Cette facture est encore au brouillon, elle ne peut pas être envoyée par e-mail. Émettez-la d\'abord.',
    'invoice_draft_no_email_short_trans_key'   => 'e-mail indisponible',
    'attachment_preview_trans_key'             => 'Aperçu de la pièce jointe',
    'attachment_preview_hint_trans_key'        => 'Le PDF envoyé au destinataire est régénéré à la volée au moment de l\'envoi — un rechargement de cette page reflète les dernières modifications du document.',
    'open_in_new_tab_trans_key'                => 'Ouvrir dans un nouvel onglet',
    'email_pdf_preview_error_trans_key'        => 'Le PDF ne peut pas être généré',
    'email_pdf_preview_error_hint_trans_key'   => 'Corrigez les points listés ci-dessus (fiche société ou fiche client), puis rechargez cette page. L\'envoi reste bloqué tant que le PDF n\'est pas généré.',
    'attach_document_pdf_trans_key'            => 'Joindre le PDF du document',
    'optional_trans_key'                       => 'facultatif',
    'email_history_trans_key'                  => 'Historique des envois',
    'email_logs_trans_key'                     => 'Journal des e-mails',
    'email_log_detail_trans_key'               => 'Détail de l\'envoi',
    'email_status_sent_trans_key'              => 'Envoyé',
    'email_status_failed_trans_key'            => 'Échec',
    'email_status_pending_trans_key'           => 'En attente',
    'email_sent_success_trans_key'             => 'E-mail envoyé avec succès !',
    'email_send_failed_trans_key'              => 'Échec de l\'envoi :',
    'email_pdf_generation_failed_trans_key'    => 'Impossible de générer le PDF du document :',
    'email_resend_trans_key'                   => 'Renvoyer',
    'email_resend_success_trans_key'           => 'E-mail renvoyé.',
    'email_resend_orphan_trans_key'            => 'Impossible de renvoyer : le document lié n\'existe plus.',
    'to_trans_key'                             => 'Destinataire',
    'by_trans_key'                             => 'Par',
    'document_trans_key'                       => 'Document',
    'error_trans_key'                          => 'Erreur',
    'details_trans_key'                        => 'Détails',
    'search_by_recipient_or_subject_trans_key' => 'Rechercher par destinataire ou objet…',
    'reset_trans_key'                          => 'Réinitialiser',

    // Widget de bascule d'affichage dans la navbar (clair / sombre / pro)
    'theme_light_trans_key'             => 'Affichage : mode clair',
    'theme_dark_trans_key'              => 'Affichage : mode sombre',
    'theme_pro_trans_key'               => 'Affichage : mode pro (neutre)',
];
