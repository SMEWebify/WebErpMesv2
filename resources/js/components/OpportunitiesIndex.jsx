...
function OpportunitiesTable({ items, trans, onSort, sortField, sortAsc, currency, locale }) {
    ...
    const filtered = items.filter(o =>
        Object.entries(colFilters).every(([col, val]) => {
            ...
            if (col === 'label')    return (o.label   ?? '').toLowerCase().includes(v);
            if (col === 'companie') return (o.companie?.label ?? '').toLowerCase().includes(v);
            if (col === 'statu')    return String(o.statu).includes(v);
            if (col === 'user')     return (o.user?.name ?? '').toLowerCase().includes(v);
            return true;
        })
    );
            ...
            <div className="table-responsive">
                <table className="table table-hover table-sm mb-0">
                    <thead>
                        ...
                        <tr>
                            {visibleCols.map(colId => {
                                const filterable = ['label', 'companie', 'user'].includes(colId);
                                return (
